...
| M2-02 | Implement wrapper class skeletons in `stdlib/src/com/aden/lang` for `Byte`, `Short`, `Integer`, `Long`, `Float`, `Double`, `Char`, `Boolean` | M2-01 | Codex | 2026-03-09 | Done | All classes compile and are importable |
...
| M2-G5 | Native wrapper runtime fixtures | `adenc build` + run fixture executables with stable output | Done |
| M2-G6 | Freeze-doc lock | docs updated and cross-checked with tests | Done |
| M2-G7 | Casting/literal conformance lock suites | `cargo test -p adenc --test stage_locks_casting` and `cargo test -p adenc --test stage_locks_casting_failures` are green, including negative truncation + boxed literal mapping (`Long a = 20`) | Done (Locked) |
...
| M3-15 | Add real multi-file fixture suites for collections/io/time/math/object-interface mixes | M3-08, M3-12, M3-13 | Codex | 2026-03-10 | Done | Fixtures run via `adenc check` and `adenc build` |
...
| Gate ID | Gate | Command / Evidence | Status |
|---|---|---|---|
| M3-G1 | Fixture project coverage | `cargo test -p adenc --test stage_locks gate_m3_g1_fixture_project_coverage -- --exact` and `cargo test -p adenc --test fixture_projects` pass | Done |
| M3-G2 | Collections behavior coverage | `cargo test -p adenc --test stage_locks gate_m3_g2_collections_behavior_coverage -- --exact` passes | Done |
| M3-G3 | IO and stream behavior coverage | `cargo test -p adenc --test stage_locks gate_m3_g3_io_stream_behavior_coverage -- --exact` passes | Done |
| M3-G4 | Time and math behavior coverage | `cargo test -p adenc --test stage_locks gate_m3_g4_time_math_determinism_coverage -- --exact` passes | Done |
