...
- Aden Lang is now the Rust-hosted bootstrap compiler and language/runtime workbench for a multi-target self-sustained path
...
- Windows x64 is the current executable target lane; AdenOS, generic Linux, and macOS Apple Silicon remain preserved later target lanes
...
- new backend/runtime work below the IR boundary should prioritize explicit target-adapter boundaries that keep Windows x64 targetable while preserving clean integration room for later AdenOS, Linux, and macOS targets
- `adenos-x64` now means a distinct AdenOS platform target contract
- `linux-x64` remains a distinct later public target
- `native-x64` is now retired from the public target surface; current CLI, manifest, and scaffold surfaces use only the canonical public targets (`windows-x64`, `adenos-x64`, `linux-x64`, `macos-arm64`)
- compiler-owned packaging/install generation has been removed from `adenc`; downstream tools own installers/packages if needed
...
- the actual target is self-sustained hosting, meaning Aden should own everything it realistically can
- `stdlib` remains the ordinary user-facing library
- `adklib` is the privileged Aden-owned extension library for compiler/runtime/toolchain authoring work
- the compiler must always be able to resolve `adklib` for compiler/runtime builds even when a user project does not enable it as a normal dependency
- the import/prelude model must stop depending on hand-maintained builtin class/package registries; `aden.*` / `adk.*` resolution should derive from the real stdlib tree, the default-import contract must be documented explicitly, and the implicit prelude package list must have one compiler-owned source of truth
- the future Aden-side compiler/runtime test model is not a copy of Rust bring-up locks: Rust keeps the broad board/roadmap/doc lock burden while Aden-side tests should center feature/behavior locks, ABI/runtime/stdlib contracts, diagnostics, and failure modes
- a dedicated pre-Phase-G `F-B` inventory pass now exists to classify what still has to move from Rust into Aden before the Phase G transition can be claimed honestly
...
- when that later self-sustained work is real but not correctly sequenced yet, it must be written into [FB_CARRY_FORWARD_TRACKER.md](/G:/Programming/Rust/Aden Lang/docs/FB_CARRY_FORWARD_TRACKER.md) so `F-B` starts with a lighter audit load
- the first concrete Aden-side feature-lock homes now exist in:
  - [prelude_contract.rs](/G:/Programming/Rust/Aden Lang/crates/adenc-core/src/tests/prelude_contract.rs)
  - [aden_feature_locks.rs](/G:/Programming/Rust/Aden Lang/crates/adenc-cli/tests/aden_feature_locks.rs)
...
- the current public stdlib surface already lives under:
  ...
  - `aden.util`
  - `aden.rt`
- the current practical utility baseline already includes reusable parser/text helpers such as `aden.util.TextCursor`
- the public project/tooling surface is real and must be treated as contract:
  - `adenc new|check|build|test|help|version`
  - canonical public targets `windows-x64`, `adenos-x64`, `linux-x64`, `macos-arm64`
  - public manifest v1 sections `[package]`, `[sources]`, `[build]`, `[toolchain]`, `[workspace]`, and `[adklib]`
...
- prefer extending or routing through existing `aden.*` / `adk.*` surface over parallel rewrites
- the current `adk.project.*` seed now includes manifest parsing/validation, layout resolution, manifest lookup/loading, workspace-member validation, entry discovery, test-file discovery, and the first Aden-owned check/test invocation-shaping slice instead of stopping at passive model types
- that same `adk.project.*` seed now also includes bridge-first workspace context ownership through:
...
- the first Aden-owned build invocation/config slice now also exists under `adk.build.*` (`BuildConfig`, `BuildInvocation`, `BuildInvocationResolver`)
- `adk.build.*` now also owns workspace-build execution/result contracts through:
...
- the first Aden-owned toolchain precedence/config slice now also exists under `adk.toolchain.*` (`ToolchainConfig`, `ToolchainResolver`)
- the first Aden-owned toolchain candidate-planning slice now also exists there (`ToolchainCandidatePlan`, `ToolchainResolver.planCandidates(...)`)
- the first Aden-owned toolchain command-shaping slice now also exists there (`ToolchainCommandPlan`, Windows link-plan helpers, response-file rendering helpers)
- the first Aden-owned toolchain env/process planning slice now also exists there through the public `aden.process.*` surface (`ProcessEnvironmentVariable`, `ProcessEnvironment`, `ProcessPlan`, `ToolchainResolver.mergeEnvironment(...)`, `ToolchainResolver.planResponseFileProcess(...)`, `ToolchainResolver.planWindowsMasmAssembleProcess(...)`)
- the first Aden-owned toolchain host-filesystem execution slice now also exists there (`ToolchainHostFilesystem.writeResponseFile(...)`)
- the first public host-filesystem materialization seam is now also real under `aden.io.HostFiles`
- `adk.toolchain.*` now crosses into both the public `aden.process.*` surface and the public `aden.io.HostFiles` surface instead of keeping private first-homes for them
...
- that build publication/materialization lift now also includes binary-safe filtered artifact staging through `adk.build.BuildPublicationWriter.copyRecursiveMatchingExtension(...)`, so normal `asm` / `obj` staging is no longer Rust-owned in the hot path
- that outward move is now real under `aden.process.*`:
  - `ProcessEnvironmentVariable`
  - `ProcessEnvironment`
  - `ProcessPlan`
  - `ProcessResult`
  - `Processes.execute(...)`
- `adk.toolchain.*` now consumes that public process/env surface instead of owning a private first-home for it
- the adjacent remaining public-stdlib gap is host-filesystem breadth, not first-home ownership:
  - `aden.io.*` is now the public file/path/materialization surface
...
- the Rust build-resolution donor path has also been partially realigned to that Aden-owned contract:
  ...
  - the build-resolution bridge from Rust into the Aden-owned resolver is now live and cached; Rust remains the backend/tool execution safety net under that bridge rather than the policy owner
...
- the new env/process slice also forced two more compiler truths:
  - IR lowering must preserve compile-time generic return information long enough to substitute owner type parameters for nested member-call inference; otherwise valid adklib chains like `values.get(index).name().toLowerCase()` collapse to `unknown` too early
  - large boolean contract expressions in adklib feature locks must not blow the Rust compiler stack during semantic analysis; logical `&&` / `||` handling is now flattened in semantics/nullability/lowering instead of recursing one frame per binary node
...
- [Tool Inventory](/D:/Programming/codex/Aden Lang/docs/TOOLS.md) records the currently available local command-line tools and the shell `PATH` refresh step
- [F1 Task Board](/D:/Programming/codex/Aden Lang/docs/F1_TASK_BOARD.md) is now the active execution board
- [Backend Adapter Registry](/D:/Programming/codex/Aden Lang/docs/BACKEND_ADAPTER_REGISTRY.md) is the current source of truth for the `RB-08` adapter-selection seam
- [Windows x64 Adapter Scope](/D:/Programming/codex/Aden Lang/docs/WINDOWS_X64_ADAPTER_SCOPE.md) is the current source of truth for the `RB-09` retained Windows lane boundary
- [Target-Neutral Planning Boundary](/D:/Programming/codex/Aden Lang/docs/TARGET_NEUTRAL_PLANNING_BOUNDARY.md) is the current source of truth for the `RB-10` planning-structure boundary
- [Runtime Intrinsics Partition](/D:/Programming/codex/Aden Lang/docs/RUNTIME_INTRINSICS_PARTITION.md) is the current source of truth for the `RB-11` portable-vs-adapter runtime split
- [Host Bootstrap Runtime Contract](/D:/Programming/codex/Aden Lang/docs/HOST_BOOTSTRAP_RUNTIME_CONTRACT.md) is the current source of truth for the `RB-12` minimum retained Rust-host bootstrap runtime seam
- [AdenOS Runtime Service ABI Slice](/D:/Programming/codex/Aden Lang/docs/PULSEOS_RUNTIME_SERVICE_ABI_SLICE.md) is the current source of truth for the `RB-13` requested-target AdenOS runtime-service slice
- [Runtime Ownership Rebase](/D:/Programming/codex/Aden Lang/docs/RUNTIME_OWNERSHIP_REBASE.md) is the current source of truth for the `RB-14` three-way ownership split
- [Target Artifact Contract Split](/D:/Programming/codex/Aden Lang/docs/TARGET_ARTIFACT_CONTRACT_SPLIT.md) is the current source of truth for the `RB-15` artifact split between Windows x64 and AdenOS
- [AdenOS Startup, Loader, and Publication Contract](/D:/Programming/codex/Aden Lang/docs/PULSEOS_STARTUP_LOADER_PUBLICATION_CONTRACT.md) is the current source of truth for the `RB-16` startup/publication split between the retained Windows x64 lane and the requested-target AdenOS lane
- [AdenOS Executable-Loading Proof Target](/D:/Programming/codex/Aden Lang/docs/PULSEOS_EXECUTABLE_LOADING_PROOF_TARGET.md) is the current source of truth for the `RB-17` contract-level definition of what counts as a AdenOS-targetable executable-loading artifact
...
- [Compiler Packaging Removal](/D:/Programming/codex/Aden Lang/docs/COMPILER_PACKAGING_REMOVAL.md) is the current source of truth for `RB-17.1` and the rule that `adenc` publishes build outputs only
- [Rebase Validation Layering](/D:/Programming/codex/Aden Lang/docs/REBASE_VALIDATION_LAYERING.md) is the current source of truth for `RB-18` and the rule that target-neutral, Windows x64 adapter, and AdenOS requested-target validation must keep explicit homes
- [Windows x64 Scope / Freeze Policy](/D:/Programming/codex/Aden Lang/docs/WINDOWS_X64_SCOPE_FREEZE_POLICY.md) is the current source of truth for `RB-19` and the rule that real Windows x64 target support must stay separate from frozen Windows-only productization scope
- [Rebase Planning Spine Alignment](/D:/Programming/codex/Aden Lang/docs/REBASE_PLANNING_SPINE_ALIGNMENT.md) is the current source of truth for `RB-20` and the rule that the planning spine stays aligned now that the rebase is closed
- [Rebase Closure Checklist](/D:/Programming/codex/Aden Lang/docs/REBASE_CLOSURE_CHECKLIST.md) and [Rebase Evidence Index](/D:/Programming/codex/Aden Lang/docs/REBASE_EVIDENCE_INDEX.md) are the current source of truth for the `RB-21` closure package
- [F1 Task Board](/D:/Programming/codex/Aden Lang/docs/F1_TASK_BOARD.md) is now the active language/stdlib execution board after the closed rebase
...
1. Read the active rebase-planning docs first:
   - [REBASE Task Board](/D:/Programming/codex/Aden Lang/docs/REBASE_TAKS_BOARD.md)
   - [Rebase Scope and Return-to-F1 Policy](/D:/Programming/codex/Aden Lang/docs/REBASE_SCOPE_AND_RETURN_POLICY.md)
   - [Windows-Coupled Assumption Inventory](/D:/Programming/codex/Aden Lang/docs/REBASE_WINDOWS_ASSUMPTION_INVENTORY.md)
   - [Windows-Coupled Assumption Classification Matrix](/D:/Programming/codex/Aden Lang/docs/REBASE_WINDOWS_ASSUMPTION_CLASSIFICATION.md)
   - [Target Taxonomy and Naming Policy](/D:/Programming/codex/Aden Lang/docs/TARGET_TAXONOMY_AND_NAMING.md)
   - [Backend Layer Architecture](/D:/Programming/codex/Aden Lang/docs/BACKEND_LAYER_ARCHITECTURE.md)
   - [Runtime Intrinsics Partition](/D:/Programming/codex/Aden Lang/docs/RUNTIME_INTRINSICS_PARTITION.md)
   - [Host Bootstrap Runtime Contract](/D:/Programming/codex/Aden Lang/docs/HOST_BOOTSTRAP_RUNTIME_CONTRACT.md)
   - [AdenOS Runtime Service ABI Slice](/D:/Programming/codex/Aden Lang/docs/PULSEOS_RUNTIME_SERVICE_ABI_SLICE.md)
   - [Runtime Ownership Rebase](/D:/Programming/codex/Aden Lang/docs/RUNTIME_OWNERSHIP_REBASE.md)
   - [Target Artifact Contract Split](/D:/Programming/codex/Aden Lang/docs/TARGET_ARTIFACT_CONTRACT_SPLIT.md)
   - [AdenOS Startup, Loader, and Publication Contract](/D:/Programming/codex/Aden Lang/docs/PULSEOS_STARTUP_LOADER_PUBLICATION_CONTRACT.md)
   - [AdenOS Executable-Loading Proof Target](/D:/Programming/codex/Aden Lang/docs/PULSEOS_EXECUTABLE_LOADING_PROOF_TARGET.md)
   - [PULSEOS_LINUX_BACKED_TARGET_DIRECTION.md](/D:/Programming/codex/Aden Lang/docs/PULSEOS_LINUX_BACKED_TARGET_DIRECTION.md)
   - [Compiler Packaging Removal](/D:/Programming/codex/Aden Lang/docs/COMPILER_PACKAGING_REMOVAL.md)
   - [Rebase Validation Layering](/D:/Programming/codex/Aden Lang/docs/REBASE_VALIDATION_LAYERING.md)
   - [Windows x64 Scope / Freeze Policy](/D:/Programming/codex/Aden Lang/docs/WINDOWS_X64_SCOPE_FREEZE_POLICY.md)
   - [Rebase Planning Spine Alignment](/D:/Programming/codex/Aden Lang/docs/REBASE_PLANNING_SPINE_ALIGNMENT.md)
   - [Rebase Closure Checklist](/D:/Programming/codex/Aden Lang/docs/REBASE_CLOSURE_CHECKLIST.md)
   - [Rebase Evidence Index](/D:/Programming/codex/Aden Lang/docs/REBASE_EVIDENCE_INDEX.md)
   - [Standalone Roadmap](/D:/Programming/codex/Aden Lang/docs/STANDALONE_ROADMAP.md)
   - [F-B Task Board](/G:/Programming/Rust/Aden Lang/docs/FB_TASK_BOARD.md)
   - [F-B Carry-Forward Tracker](/G:/Programming/Rust/Aden Lang/docs/FB_CARRY_FORWARD_TRACKER.md)
   - [Author Library Strategy](/G:/Programming/Rust/Aden Lang/docs/AUTHORLIB_STRATEGY.md)
   - [Aden-Side Test Model](/G:/Programming/Rust/Aden Lang/docs/PULSE_TEST_MODEL.md)
   - [Native Backend Contract](/D:/Programming/codex/Aden Lang/docs/NATIVE_BACKEND_CONTRACT.md)

2. Read the paused-but-still-authoritative F1 continuity docs:
   - [F1 Task Board](/D:/Programming/codex/Aden Lang/docs/F1_TASK_BOARD.md)
   - [F1 Baseline Audit](/D:/Programming/codex/Aden Lang/docs/F1_BASELINE_AUDIT.md)
   - [F1 Gap Matrix](/D:/Programming/codex/Aden Lang/docs/F1_GAP_MATRIX.md)
   - [F1 Support Policy](/D:/Programming/codex/Aden Lang/docs/F1_SUPPORT_POLICY.md)
   - [F1 Dependency Graph](/D:/Programming/codex/Aden Lang/docs/F1_DEPENDENCY_GRAPH.md)

3. Read the language docs structure and workflow docs:
   - [F2 Task Board](/D:/Programming/codex/Aden Lang/docs/F2_TASK_BOARD.md)
   - [Compiler-Backed Language Docs README](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/README.md)
   - [Runtime-Backed Language Docs README](/D:/Programming/codex/Aden Lang/docs/language/runtime-backed/README.md)
   - [Stdlib Language Docs README](/D:/Programming/codex/Aden Lang/docs/language/stdlib/README.md)
   - [Tool Inventory](/D:/Programming/codex/Aden Lang/docs/TOOLS.md)

4. Re-examine the relevant code in detail before continuing:
   - frontend/parser/semantics/IR:
     - [adenc-core lib.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/lib.rs)
     - [adenc-core parser mod.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/parser/mod.rs)
     - [adenc-core semantics mod.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/semantics/mod.rs)
     - [adenc-core intermediate mod.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/intermediate/mod.rs)
     - [adenc-core tests mod.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/tests/mod.rs)
   - CLI/doc locks:
     - [stage_locks.rs](/D:/Programming/codex/Aden Lang/crates/adenc-cli/tests/stage_locks.rs)
   - stdlib source:
     - [stdlib/src/aden](/D:/Programming/codex/Aden Lang/stdlib/src/aden)
   - stdlib docs generator:
     - [generate-stdlib-docs.ps1](/D:/Programming/codex/Aden Lang/scripts/generate-stdlib-docs.ps1)
...
- [LANGUAGE_GUIDE.md](/D:/Programming/codex/Aden Lang/docs/LANGUAGE_GUIDE.md) is older and not the live execution contract
- [LANGUAGE_OVERVIEW.md](/D:/Programming/codex/Aden Lang/docs/LANGUAGE_OVERVIEW.md) is older and not the live execution contract
...
- `F1`, `F2`, `F-A`, and `F-B` are the hard self-sustained push; agents should bias toward aggressive closure of real Rust-to-Aden ownership gaps rather than stopping at thinner milestone slices when a practical continuation is still achievable
...
- if a feature can move into Aden `stdlib` or `adklib` rather than staying compiler/bootstrap-owned, that migration is required before the compiler/runtime rewrite into Aden unless the ownership genuinely belongs in compiler/runtime/backend
- compiler/runtime ownership is acceptable only where that ownership genuinely belongs; otherwise the feature should end in Aden library code before the self-sustained-hosting transition
...
- if a new feature, runtime surface, stdlib helper, `adklib` helper, or semantic rule is discovered during implementation to be required for self-sustained hosting, it must be added to the tracking docs/board and implemented instead of being treated as optional future cleanup
- if the later self-sustained work is real but would be disproportionate or badly sequenced in the current lane, it must be recorded in [FB_CARRY_FORWARD_TRACKER.md](/G:/Programming/Rust/Aden Lang/docs/FB_CARRY_FORWARD_TRACKER.md) rather than being left as tribal knowledge
...
- when valid or intended Java-like Aden source hits an unexpected snag, the default response is to treat it as a potential language/compiler/runtime bug and fix it; do not silently rewrite user code, examples, stdlib code, or tests into a workaround shape unless the user explicitly approves that as a temporary stopgap
- language capability verification rule:
  - before introducing any architectural workaround, first verify whether the requested pattern is already supported by Aden syntax and semantics
  - if the pattern parses and typechecks, it is valid Aden source
...
- self-sustained implementation rule:
  ...
  - if a helper, subsystem, or support API can be expressed in Aden, move it into Aden now and keep it current in both the Rust and Aden implementations
  - if it cannot yet be expressed in Aden, first add the missing language/runtime capability that makes it expressible, then move it
  ...
  - use `adklib` for privileged or sharp Aden-owned authoring surfaces; do not treat Rust as the default home for them just because they are not ordinary public-stdlib APIs
- lift-through-surface rule:
  - before lifting a feature, inspect the existing `aden.*` / `adk.*` surface
...
- backend/runtime truth exposed by Aden-owned surface growth:
...
- compiler recursion truth:
  - oversized contract-style adklib checks are a valid source shape in this project and must compile honestly
  - if a long `&&` / `||` contract expression overflows the Rust stack, treat that as a compiler recursion bug and flatten the compiler visitor/lowering path instead of weakening the Aden test source
...
- what Aden Lang currently is at a high level
...
The docs are currently the primary way to learn Aden Lang. They are not cleanup work.
...
### 2. Stdlib `.aden` files must be AdenDoc'd

Any stdlib `.aden` file added or modified must have multiline AdenDoc blocks.

The current lock suite already enforces:

- file/type-level AdenDoc
- AdenDoc before public members
...
- the generator rewrites the checked-in stdlib reference docs from current AdenDoc
- the generated docs under `docs/language/stdlib/aden/**` are supposed to stay current
...
- [Stdlib README](/D:/Programming/codex/Aden Lang/docs/language/stdlib/README.md)
- [generate-stdlib-docs.ps1](/D:/Programming/codex/Aden Lang/scripts/generate-stdlib-docs.ps1)
- [stage_locks.rs](/D:/Programming/codex/Aden Lang/crates/adenc-cli/tests/stage_locks.rs)

### 4. Prefer focused tests, not full `adenc` test runs

The user explicitly asked not to keep running the full `cargo test -p adenc` suite after every change.

Preferred pattern:

- run targeted `adenc-core` tests when touching parser/semantics/IR
...
- `cargo test -q -p adenc-core`
- `cargo test -q -p adenc --test stage_locks`

### 5. Fix language bugs instead of normalizing workarounds

If expected Aden source behavior fails, the next agent should assume it may be a real bug in parser/semantics/IR/backend/runtime behavior rather than immediately changing the source into a workaround shape.
...
- [F1 Task Board](/D:/Programming/codex/Aden Lang/docs/F1_TASK_BOARD.md): `F1-97`
- immediate next slice:
  ...
  - `F1-97` now owns the explicit `stdlib` vs `adklib` ownership split for compiler/runtime/systems-facing helper surfaces, with no bootstrap exemption for features that can live in Aden
  - the first concrete migration map for this work now lives in [F1_97_BOOTSTRAP_OWNERSHIP_MAP.md](/G:/Programming/Rust/Aden Lang/docs/F1_97_BOOTSTRAP_OWNERSHIP_MAP.md)
  - the first implementation work under `F1-97` should follow that split and start the Aden-owned compiler/resource/systems utility foundation without leaking systems-level APIs into ordinary public `stdlib`
  - `F1-97` is now also the first real self-host/self-sustained transition task:
    - by the end of `F1-97`, the Aden compiler/runtime codebase should exist in Aden and be compilable by the Rust-built Aden compiler
    - by the end of `F1-97`, Rust should own no normal compiler/runtime/toolchain product path at all
    - `F1-97` closes only if Aden owns enough that Rust can be abandoned entirely if desired
...
- `F1-113` is now locked:
  - `aden.time.Clock` is the real interface contract rather than a mode-switch class
  ...
  - `aden.time.Clocks` is retained only as a thin companion holder instead of being the real center of the design
  ...
  - `aden.time.TimeSource` / `TimeSources` provide the separate monotonic/process-relative elapsed-time abstraction on top of `System.nanoTime()`
...
- `F1-112` is now locked honestly: the deterministic non-secure random/UUID lane covers widened seed ingress, corrected bounded/fractional behavior, unsigned and smaller primitive helper lanes, Gaussian helpers, current-array-model bulk helpers, state-control helpers (`copy()` / `skip(int)`), and a fully Aden-owned deterministic non-secure `UUID` type with parse/format, equality/hash, version/variant, nil/default helpers, ordering, and deterministic generation
- the remaining later randomness boundary is explicit rather than hidden: secure randomness, OS/runtime entropy ingress, and secure UUID generation are deferred and already tracked in [FB_CARRY_FORWARD_TRACKER.md](/G:/Programming/Rust/Aden Lang/docs/FB_CARRY_FORWARD_TRACKER.md)
...
- the bottom of the `F1` board now also carries an explicit post-`F1` inward-lift audit task in `Workstream N`: `F1-113A` exists to run the full code audit for moving code inward into Aden for self-sustained lifting once the main F1 implementation lanes are closed
- current reassessment result before starting `F1-97` implementation:
  ...
  - the real missing work is the sharper ownership split and inward move for compiler/runtime/systems helpers, which now belongs to `F1-97` and the later `adklib` foundation rather than to reopening already-locked public stdlib tasks
  - the task is intentionally larger now than a pure ownership inventory: the point is to leave the project able, if desired, to compile the Aden compiler/runtime with the Rust-built Aden compiler rather than only having the ownership split documented
...
- `F1-63`
  - `aden.util` now exists instead of being only planned, and the first foundational utility slice is real through `Objects`
...
- the post-`Scanner` string hardening pass is now underway: the stable current slice added indexed search (`indexOf` / `lastIndexOf` with start offsets and char overloads), immutable text replacement, offset prefix checks, `repeat(...)`, ASCII case conversion (`toLowerCase`, `toUpperCase`), case-insensitive equality (`equalsIgnoreCase`), content-based `hashCode()`, ASCII whitespace helpers (`isBlank`, `strip`, `stripLeading`, `stripTrailing`), char containment, prefix/suffix removal helpers, left/right padding, array-based literal splitting (`split(char)`, `split(String)`), array-based joining (`String.join(String, String[])`), and char-array/region text helpers (`toCharArray()`, `getChars(...)`, `contentEquals(...)`, `regionMatches(...)`, `copyValueOf(...)`, `valueOf(char[])`, `valueOf(char[], int, int)`) on `String`, while `StringBuilder` gained widened signed/unsigned integral `append(...)` and `appendLine(...)` coverage including `appendLine(CharSequence)` and `appendLine(char[])`, widened `append(...)` support for `CharSequence`, bounded `append(CharSequence, int, int)`, `char[]`, and bounded `append(char[], int, int)`, widened `insert(...)` overloads across string/object/scalar lanes including bounded `insert(int, CharSequence, int, int)`, `char[]`, and bounded `insert(int, char[], int, int)`, `isEmpty()`, `deleteCharAt(...)`, range `replace(...)`, `setCharAt(...)`, immutable `substring(...)`, indexed search, and `reverse()` on top of the earlier append/insert/delete/setLength surface
  - the stable Aden-owned conversion slice is broader now: `Long.toString(...)`, `Long.parse(...)`, `Integer.parse(...)`, `Char.parse(...)`, `Boolean.parse(...)`, `UInteger.parse(...)`, `UInteger.toString(...)`, `ULong.parse(...)`, and `ULong.toString(...)` are now implemented in Aden stdlib instead of routing through runtime helpers
  ...
  - the remaining string/runtime boundary is narrower and more explicit: public `Intrinsics.stringLength(...)` / `Intrinsics.stringConcat(...)` are gone, with `String.length()` / `String.concat(...)` now routing through private `String.runtimeLength(...)` / `String.runtimeConcat(...)`; `String.substring(...)` plus `Class.getSimpleName()` / `getPackageName()` are now Aden-owned, while `charAt` and single-char string bridging still remain runtime-backed; the final private string representation seam (`runtimeLength`, `runtimeConcat`, `runtimeCharAt`, `runtimeFromChar`) is now recorded in [FB_CARRY_FORWARD_TRACKER.md](/G:/Programming/Rust/Aden Lang/docs/FB_CARRY_FORWARD_TRACKER.md) rather than left implicit; wrapper-local string bridges are shrinking too, including `Double` now using the public `String.charAt()` surface instead of carrying its own private string runtime helper
  ...
  - user classes now inherit `Object` methods implicitly the Java-like way, `Object.toString()` now defaults to `ClassName@hexHash`, and `Integer.toHexString(...)` is available in Aden stdlib for identity/debug formatting without falling back to runtime-owned text helpers
...
- `RB-20` planning spine alignment
  - [REBASE_PLANNING_SPINE_ALIGNMENT.md](/D:/Programming/codex/Aden Lang/docs/REBASE_PLANNING_SPINE_ALIGNMENT.md) now locks the closed rebase state and the active `F1-51` return point
...
- `RB-21` closure package publication
  - [REBASE_CLOSURE_CHECKLIST.md](/D:/Programming/codex/Aden Lang/docs/REBASE_CLOSURE_CHECKLIST.md) and [REBASE_EVIDENCE_INDEX.md](/D:/Programming/codex/Aden Lang/docs/REBASE_EVIDENCE_INDEX.md) are now published
...
- `RB-19` Windows x64 scope / freeze policy
  - [WINDOWS_X64_SCOPE_FREEZE_POLICY.md](/D:/Programming/codex/Aden Lang/docs/WINDOWS_X64_SCOPE_FREEZE_POLICY.md) now separates retained live Windows x64 adapter scope from frozen Windows-only productization scope
...
- `RB-18` rebase validation layering
  - [REBASE_VALIDATION_LAYERING.md](/D:/Programming/codex/Aden Lang/docs/REBASE_VALIDATION_LAYERING.md) now maps the live suites into target-neutral, cross-cutting contract, Windows x64 adapter, and AdenOS requested-target homes
  - the current mixed `fixture_projects.rs` ownership is now explicit instead of being left implicit by filename
  - Windows x64 executable/parity proof is now documented as Windows adapter evidence only, not as proxy proof for AdenOS or generic Linux
...
- `RB-17.1` compiler packaging removal
  - [COMPILER_PACKAGING_REMOVAL.md](/D:/Programming/codex/Aden Lang/docs/COMPILER_PACKAGING_REMOVAL.md) now records that `adenc` no longer owns package/install/signing workflows
...
- `RB-17` AdenOS executable-loading proof target
  - [PULSEOS_EXECUTABLE_LOADING_PROOF_TARGET.md](/D:/Programming/codex/Aden Lang/docs/PULSEOS_EXECUTABLE_LOADING_PROOF_TARGET.md) now defines the first contract-level AdenOS loading proof target
  - `native.plan.json` now emits `requested_target_executable_loading_proof_target` for `adenos-x64`
  - the requested-target AdenOS lane now has an explicit definition of what must be published and exposed before it can honestly count as executable-loading-capable
  - this remains a proof target only; it does not pretend the AdenOS adapter executes yet

- `RB-16` AdenOS startup/loader/publication contract
  - [PULSEOS_STARTUP_LOADER_PUBLICATION_CONTRACT.md](/D:/Programming/codex/Aden Lang/docs/PULSEOS_STARTUP_LOADER_PUBLICATION_CONTRACT.md) now publishes distinct Windows x64 and AdenOS startup/loader contracts
  - `native.plan.json` now emits `startup_loader_contracts` with separate `active_adapter` and `requested_target` blocks
  - the requested-target AdenOS lane now has an honest first-slice `adenos_start`, `adenos-loader-entry-v1`, and `bin/<entry> + bin/launch.json` publication contract
  - requested AdenOS `shared` mode is explicit but currently collapses to the same first-slice single-image layout until a later task publishes a real separate runtime-companion lane

- `RB-15` target artifact contract split
  - [TARGET_ARTIFACT_CONTRACT_SPLIT.md](/D:/Programming/codex/Aden Lang/docs/TARGET_ARTIFACT_CONTRACT_SPLIT.md) now publishes the retained Windows x64 artifact contract separately from the requested-target AdenOS artifact contract
  - `native.plan.json` now emits `artifact_contracts` with distinct `active_adapter` and `requested_target` blocks
  - the current active adapter artifact contract still names the Windows x64 host/bootstrap publication story, while the AdenOS requested-target contract now stays aligned with the published `RB-16` startup/publication contract instead of leaving publication truth implicit

- `RB-14` runtime ownership rebase
  - [RUNTIME_OWNERSHIP_REBASE.md](/D:/Programming/codex/Aden Lang/docs/RUNTIME_OWNERSHIP_REBASE.md) now defines the three-way ownership split between Aden surface, retained Rust host-bootstrap runtime, and adapter-owned glue
...
- `RB-13` AdenOS runtime-service slice
  - [PULSEOS_RUNTIME_SERVICE_ABI_SLICE.md](/D:/Programming/codex/Aden Lang/docs/PULSEOS_RUNTIME_SERVICE_ABI_SLICE.md) now defines the first requested-target AdenOS service boundary for the meeting milestone surface
  - `native.plan.json` now emits `requested_target_runtime_service_abi` with schema `adenc.adenos.runtime_service.v1` when the requested target is `adenos-x64`
  - the published first slice covers console, panic, exit, and minimum path/file/service seam capability IDs without pretending the full AdenOS ABI, startup entry, or publication layout are done
...
- `RB-12` host bootstrap runtime contract
  - [HOST_BOOTSTRAP_RUNTIME_CONTRACT.md](/D:/Programming/codex/Aden Lang/docs/HOST_BOOTSTRAP_RUNTIME_CONTRACT.md) now defines the minimum retained Rust-host bootstrap runtime procedure families required to keep compiler artifact emission and executable validation moving
  ...
  - this retained bootstrap seam remains separate from the requested-target AdenOS service contract instead of being treated as AdenOS default truth

- `RB-11` runtime/intrinsics partition
  - [RUNTIME_INTRINSICS_PARTITION.md](/D:/Programming/codex/Aden Lang/docs/RUNTIME_INTRINSICS_PARTITION.md) now consumes `WIN-RT-01`, `WIN-RT-02`, `WIN-RT-03`, and `WIN-RT-05` directly from the rebase inventory/classification inputs
  ...
  - [RUNTIME_INTRINSICS_ABI.md](/D:/Programming/codex/Aden Lang/docs/RUNTIME_INTRINSICS_ABI.md) now describes the portable Aden/runtime bridge first and treats Windows x64 ABI/process/allocator notes as adapter reference rather than cross-target truth
  - [E2_RUNTIME_BOUNDARY_GUIDE.md](/D:/Programming/codex/Aden Lang/docs/E2_RUNTIME_BOUNDARY_GUIDE.md) now explicitly frames its startup/import sequence as current Windows x64 shared-mode evidence rather than the portable intrinsic contract

- `RB-10` target-neutral planning boundary
  - [TARGET_NEUTRAL_PLANNING_BOUNDARY.md](/D:/Programming/codex/Aden Lang/docs/TARGET_NEUTRAL_PLANNING_BOUNDARY.md) now names `WIN-PLAN-01` and `WIN-PLAN-04` as consumed rows
  ...
  - `crates/adenc-cli/src/backend/analysis/plan_rendering.rs` no longer hard-codes `kernel32.lib`
...
- `RB-09` Windows x64 adapter scope
  - [WINDOWS_X64_ADAPTER_SCOPE.md](/D:/Programming/codex/Aden Lang/docs/WINDOWS_X64_ADAPTER_SCOPE.md) now defines the retained Windows x64 lane as an immediate target plus the current host/bootstrap adapter
  - [WINDOWS_TOOLCHAIN_MATRIX.md](/D:/Programming/codex/Aden Lang/docs/WINDOWS_TOOLCHAIN_MATRIX.md) is now framed as Windows adapter support evidence, not as the whole project support matrix
  - [CLI_COMMAND_CONTRACT.md](/D:/Programming/codex/Aden Lang/docs/CLI_COMMAND_CONTRACT.md) and [D_CLI_UX_GUIDE.md](/D:/Programming/codex/Aden Lang/docs/D_CLI_UX_GUIDE.md) now explain that `--linker` and `--assembler` are retained Windows adapter overrides rather than target-neutral guarantees
...
- `RB-08` adapter registry seam
  - [BACKEND_ADAPTER_REGISTRY.md](/D:/Programming/codex/Aden Lang/docs/BACKEND_ADAPTER_REGISTRY.md) is now the source of truth for target-adapter selection
  ...
  - the current Windows x64 adapter remains the only implemented adapter, but AdenOS and generic Linux requests now route through one explicit host/bootstrap registry boundary instead of scattered helper imports
...
- `F1-43` wrapper-class depth
  ...
  - focused proof:
    - `cargo test -q -p adenc --test fixture_projects cli_build_executes_wrapper_integral_text_and_constants_flow -- --exact`
    - `cargo test -q -p adenc --test fixture_projects cli_build_executes_wrapper_floating_text_and_compare_flow -- --exact`
- `F1-44` unsigned policy
  - `ubyte` / `ushort` / `uint` / `ulong` plus their wrapper classes are now explicitly locked as retained Aden extensions
...
- `F1-47` foundational `aden.lang` types
...
- `F1-48` collection API inventory and strategy
    - the shipped `aden.collections` surface is now inventoried explicitly
...
- `F1-50` core collection contract expansion
    ...
    - `Map` is now the shared raw-object interface contract and `HashMap` is the current Aden-owned concrete implementation instead of keeping two parallel concrete facades
...
- `F1-51` practical list helpers
  ...
  - `aden.lang.Enum` remains explicitly bootstrap; the fuller enum object model still returns later under `F1-10`
- `F1-53` collection iteration contract
  - `Collection` now extends `aden.lang.Iterable`
  - the current non-fail-fast object-stream baseline is real across `ArrayList`, `LinkedList`, `HashSet`, and `aden.collections.Array` once its int/string lane is established
...
- `F1-54` collection utility families
  - `aden.util.Arrays` now provides the chosen current array utility baseline: `asList(...)`, fixed-lane `indexOf(...)`, and fixed-lane `contains(...)`
  - `aden.collections.Collections` now provides the chosen current collection utility baseline: static `addAll(...)`, `frequency(...)`, `reverse(...)`, `singletonList(...)`, and `singleton(...)`
...
- `F1-109` collection value-kind closure
  ...
  - `aden.collections.Array` is now explicitly locked as a runtime-backed fixed-length helper with `int` / `String` lanes plus object-stream iteration from an established lane; it is not the final generic Java-array replacement
...
- `F1-110` collection-shape closure
  ...
  - `HashMap.entrySet()` now exists as a copy-style `Set<MapEntry>` baseline through the top-level `aden.collections.MapEntry` value type; it is intentionally non-live and not a nested `Map.Entry` contract
...
- shared publication and shared profile builds are green again after:
  - moving the split runtime support object out of the `com/aden/lang/IO.asm` collision path into `obj/runtime/StdlibRuntime.asm`
...
- `F1-29`, `F1-35`, `F1-46`, and `F1-101` are now locked together for the shipped exception slice
  ...
  - runtime Aden-throw propagation works across method boundaries
  - matching caller-side `catch` / `finally` blocks execute for propagated throws
  - uncaught Aden throws print source-aware stack frames in `Class.method(File.aden:line)` form
...
- `F1-34` and `F1-35` are now locked
  ...
  - the runtime now maintains an exception-handler stack for Aden `throw` values
...
- `F1-37` is now locked
  ...
  - exception/trace runtime growth is explicitly documented through `adenc_rt_throw` and `adenc_rt_traceUpdateTop`
- `F1-38` is now locked
...
- `F1-41` is now locked
  ...
  - `substring` is now Aden-owned on top of the remaining private string runtime seam; only storage/index primitives like `charAt`, `length`, `concat`, and single-char bridging still depend on runtime helpers
- `F1-42` is now locked
...
- memory-management ownership was cleaned up in stdlib
  - [Memory.aden](/D:/Programming/codex/Aden Lang/stdlib/src/aden/memory/Memory.aden) now owns the user-facing ARC/cycle/weak surface
  - `aden.rt.Intrinsics` remains the internal backend bridge for those operations
...
- exception-package ownership still needs a dedicated reorganization pass before self-host
  ...
  - the current shipped throwable hierarchy still lives under `aden.lang`
- self-host-ready test authoring/execution is now tracked explicitly too
  - `F1-107` covers the pre-self-host test baseline: `@Test` ownership, assertion helpers, discovery policy, `adenc test`, output contract, and the minimum product-owned support needed for Aden-native compiler/runtime tests before self-host
- `F1-46` is now locked
...
- validated with:
  ...
  - `cargo test -q -p adenc --test fixture_projects cli_build_executes_strict_stress_soak_with_repeated_fat_shared_parity`
  - `cargo test -q -p adenc --test stage_locks lock_m3_16_stress_soak_fixture_check -- --exact --test-threads=1`
...
- `adenc_rt_objectNew` no longer returns a raw slot; it returns `slot | (generation << 32)`
- `adenc_rt_objectClassId` now decodes and validates packed object handles before reading class metadata
...
- focused validation:
  ...
  - `cargo test -q -p adenc --test fixture_projects --manifest-path "D:\Programming\codex\Aden Lang\Cargo.toml"`
  - `cargo test -q -p adenc --test stage_locks --manifest-path "D:\Programming\codex\Aden Lang\Cargo.toml"`

The following tasks were reopened as explicit bootstrap-to-Aden return passes:

- `F1-10` enum baseline
  - return in `F1-47` / `F1-69` to complete the std-facing `aden.lang.Enum` contract
...
- `F1-12` richer annotation syntax/support policy
  - return in `F-A` with `F2` docs updates for Aden-owned annotation processor migration
...
Annotation declarations in Aden are now real:
...
- [Override.aden](/D:/Programming/codex/Aden Lang/stdlib/src/aden/lang/Override.aden)
- [NotNull.aden](/D:/Programming/codex/Aden Lang/stdlib/src/aden/lang/NotNull.aden)
- [Test.aden](/D:/Programming/codex/Aden Lang/stdlib/src/aden/lang/Test.aden)
...
- [enums.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/enums.md)
...
- [generics.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/generics.md)
...
- [adenc-core parser.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/parser.rs)
  - declaration parsing
  - annotation parsing
  - enum parsing
  - generic declaration/type-use parsing

- [adenc-core semantics.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/semantics.rs)
  - class/type index
...
- [adenc-core ir.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/ir.rs)
  - IR lowering
  - enum ordinal lowering
  - erased generic runtime type lowering

- [adenc-core tests.rs](/D:/Programming/codex/Aden Lang/crates/adenc-core/src/tests.rs)
...
- [compiler-backed](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed)
- [runtime-backed](/D:/Programming/codex/Aden Lang/docs/language/runtime-backed)
- [stdlib](/D:/Programming/codex/Aden Lang/docs/language/stdlib)

Compiler-backed docs added/updated during this window:

- [lexer-and-literals.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/lexer-and-literals.md)
- [declarations-and-modifiers.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/declarations-and-modifiers.md)
- [types-and-conversions.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/types-and-conversions.md)
- [packages-and-imports.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/packages-and-imports.md)
- [constructors.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/constructors.md)
- [annotations.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/annotations.md)
- [enums.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/enums.md)
- [generics.md](/D:/Programming/codex/Aden Lang/docs/language/compiler-backed/generics.md)

Runtime-backed docs already created for current non-stdlib runtime behavior:

- [program-entry-and-execution.md](/D:/Programming/codex/Aden Lang/docs/language/runtime-backed/program-entry-and-execution.md)
- [object-lifetime-and-gc.md](/D:/Programming/codex/Aden Lang/docs/language/runtime-backed/object-lifetime-and-gc.md)
- [runtime-type-and-null-behavior.md](/D:/Programming/codex/Aden Lang/docs/language/runtime-backed/runtime-type-and-null-behavior.md)
- [exceptions-and-failure.md](/D:/Programming/codex/Aden Lang/docs/language/runtime-backed/exceptions-and-failure.md)
...
- [Library Artifact Strategy](/D:/Programming/codex/Aden Lang/docs/LIBRARY_ARTIFACT_STRATEGY.md)
- [Standalone Roadmap](/D:/Programming/codex/Aden Lang/docs/STANDALONE_ROADMAP.md)

Current future-direction decisions:

- Aden-authored libraries are intended to compile to native DLLs
- companion binding artifacts will be normal `.aden` files, not a separate custom extension
...
- app/runtime consumption later will use:
  - `libraries/` for DLLs
  - `bindings/` for binding `.aden` files
...
- `F1-100` now covers the real pre-self-host local `var` slice:
  ...
  - `foreach (var item : iterable)` on the current `aden.lang.Iterable` object-stream baseline
  - `foreach (var item : aden.collections.Array)` on the current object-stream baseline once the array lane is established
...
- the author build bridge is now prewarmed explicitly and cached at `target/adk_build_bridge`
- ordinary resolution should reuse that cache, not silently compile a fresh bridge runner inside a child `adenc` process
...
- manifest/publication regressions exposed two real Aden-side truth bugs that are now fixed:
  - absent `out_dir` in adklib manifest parsing must stay `null`, not collapse to project-root
  - `aden.io.Path` must treat normalized Windows drive paths (`C:/...`) as absolute without breaking the existing slash-child combine contract
- later runtime regressions exposed a real backend ownership bug in MASM lowering:
  ...
  - this cleared both the weak-reference runtime lock drift and the `aden.io.Path` runtime dispatch crash without changing those surfaces
- later bridge regressions exposed a second real backend ownership bug in MASM lowering:
  ...
  - this cleared the adklib bridge `resolve_build_invocation` regressions without changing `adk.build.*` or `aden.lang.String`
- toolchain ownership moved one real step inward during this push:
  - Aden now owns toolchain precedence through `adk.toolchain.ToolchainConfigResolver`
  - Aden `BuildInvocation` now carries a real `ToolchainConfig`
...
- the next smaller toolchain step also landed:
  - Aden now owns ordered candidate-planning policy for linker selection and adjacent `ml64.exe` discovery through `adk.toolchain.ToolchainDiscoveryResolver`
  - Aden now also owns the Windows host-tool discovery graph through `ToolchainDiscoveryPlan`
  - Aden now also owns the first real host probe execution slice through `ToolchainDiscoveryResult` and `ToolchainDiscoveryResolver.probeWindowsDiscovery(...)`, using host-backed `aden.io.Files` instead of leaving the directory walk in Rust
...
- the next toolchain step after that also landed:
  - Aden now owns ordered Windows linker command-plan and response-file formatting policy
  ...
  - Aden now also owns the first host response-file materialization path through `ToolchainHostFilesystem.writeResponseFile(...)`
  ...
  - Aden now also owns the first real host tool-process execution path through `aden.process.Processes.execute(...)`, `aden.process.ProcessResult`, and host-backed `Intrinsics.hostRunShellProcess(...)`
...
- bridge bootstrap truths from this follow-up:
  - author build bridge cache locking now records owner metadata and reclaims stale/orphaned `target/adk_build_bridge/lock` directories after crashes
  ...
  - the cached bridge should depend on the smallest relevant `adk.toolchain` surface, not a monolithic resolver that drags unrelated toolchain code into bootstrap
  ...
  - `adenc_rt_consoleReadLine` can no longer use the legacy fixed 4095-byte stack buffer; it now grows dynamically so large bridge/toolchain request lines survive bootstrap intact
  ...
  - `aden.process.Processes.renderWindowsCommand(...)` must pass the already-quoted script text directly to `cmd.exe /d /c`; wrapping the entire script as one more quoted shell argument breaks valid Windows command parsing
  - `adenc_rt_hostRunShellProcess` now carries real bridge/runtime responsibility: it must preserve the incoming working-directory handle until `hostPathAlloc`, allocate a writable raw command-line buffer, invoke `CreateProcessA`, wait, capture the exit code, and free/close all host resources correctly
  ...
  - release backend emission now strips statement/file-line trace payload from `aden.*` / `adk.*` class asm while keeping method-level frames for runtime stack traces; release app/user code still keeps statement/file-line payload so runtime stack traces remain source-truthful there, and debug builds keep the richer payload for asm debugging
  - repeated `instanceof` / checked-cast class-id compare ladders now route through a shared runtime helper (`adenc_rt_classIdInSet`) plus emitted class-id tables instead of duplicating long inline compare chains at every use site
  - the cached author-build bridge lock now treats transient Windows `PermissionDenied` reads/stats on `target/adk_build_bridge/lock` as "lock is owned right now" rather than surfacing random full-workspace failures under concurrent test binaries
  ...
  - adklib outward-audit truth after the first inward lifts:
    - the main missed public seam was process/env execution, not filesystem access
    - public filesystem/path IO already exists in `aden.io.*` (`File`, `Files`, `Path`), and the first public host-filesystem materialization seam is now also real under `aden.io.HostFiles`
    ...
    - `adk.toolchain` now crosses into those public surfaces afterward
    - `ToolchainHostFilesystem` is now a wrapper over public host-filesystem surface, not a private first-home for it
    - the rest of the current `adk.toolchain` surface still looks properly privileged/tooling-owned:
    ...
    - the current `adk.project.*` and `adk.build.*` surface also still looks appropriately privileged at this stage; no equally strong outward-move candidate has shown up there yet
  - build/publication truth after the next publication slice:
  - `adk.build.BuildPublicationWriter` now owns artifact stamp writing, shared launch metadata writing, single-file copy, full-tree copy, and recursive copy filtered by extension
  - `adk.build.BuildPublicationPlan` / `BuildPublicationPlanBridge` / `BuildPublicationWriter.resolvePublicationPlan(...)` now own publication path/layout planning
  - `adk.build.BuildPublicationWriter.ensureLayoutRoots(...)`, `materializeBuildLayout(...)`, and `writeBuildConfigPlan(...)` now also own the normal build-layout staging roots and build-config-plan writing
  - `adk.build.BuildSummaryWriter.renderBuildSummary(...)` now owns the normal build result/status summary text that `adenc build` prints after a successful build
  - `adk.build.BuildPublishedArtifact`, `BuildPublishedArtifactBridge`, and `BuildPublicationWriter.publishArtifacts(...)` now own the normal published-artifact result contract and the actual artifact publication execution path
  - `adk.build.BuildPublicationWriter.finalizeBuildBridgeText(...)` now owns the normal hot-path post-emit authored build finalization sequence:
    - layout materialization
    - publication-plan resolution
    - artifact publication
    - build-config-plan writing
    - artifact-stamp writing
  - `adk.compiler.*` now has its first real live slice through `adk.compiler.CheckSummaryWriter`
  - that check slice now also includes `adk.compiler.CheckResult`, and `CheckSummaryWriter` now renders the structured authored result instead of separate success/failure tuples
  - `adenc check` and workspace-check status rendering now prefer that authored compiler surface through the cached bridge
  - `adk.compiler.*` now also has a second live slice through `adk.compiler.TestSummaryWriter`
  - that test slice now also includes `adk.compiler.TestResult`, and `TestSummaryWriter` / `TestDiagnosticWriter` now render structured authored test result state for summary and aggregate-failure text instead of loose tuples
  - that same test slice now also includes `adk.compiler.TestDiscoveryResult`, and discovery success/failure/no-tests/workspace-member-no-tests text now routes through that authored model instead of loose path/detail tuples
  - `adenc test` and workspace-test discovery/summary lines now prefer that authored compiler surface through the cached bridge
  - `adk.compiler.*` now also has a third live slice through `adk.compiler.TestDiagnosticWriter`
  - `adenc test` now prefers that authored compiler surface for discovery-failure/no-tests/aggregate-failure message text
  - `adk.project.*` now also owns more of the normal `adenc check` / `adenc test` path through:
  ...
  - `adk.compiler.*` now also owns workspace aggregation models through:
    - `adk.compiler.WorkspaceCheckResult`
    - `adk.compiler.WorkspaceTestResult`
  - `adk.compiler.*` now also owns workspace member/test execution outcome models through:
    - `adk.compiler.WorkspaceCheckMemberResult`
    - `adk.compiler.TestExecutionResult`
    - `adk.compiler.TestExecutionWriter`
  ...
  - `adk.compiler.*` now also has the first real compiler execution slice through:
    - `adk.compiler.CheckExecutionResult`
    - `adk.compiler.CheckExecutionBridge`
    - `adk.compiler.CheckExecutionProvider`
  - single-project `adenc check` now prefers that authored execution path:
    - the cached bridge executes a Aden-owned compiler provider contract
    - that Aden contract uses the public `aden.process.*` surface to invoke the current `adenc` binary as a host compiler provider
  ...
  - `adk.compiler.*` now also has the first provider-backed build-core execution slice through:
    - `adk.compiler.BuildCoreExecutionResult`
    - `adk.compiler.BuildCoreExecutionBridge`
    - `adk.compiler.BuildCoreExecutionProvider`
  - `cli/build/mod.rs` now prefers that authored build-core execution path before falling back to the older inline Rust compiler loop:
    - the cached bridge executes a Aden-owned build-core provider contract
    - that Aden contract uses the public `aden.process.*` surface to invoke the current `adenc` binary as a host compiler provider
  ...
  - `adk.compiler.*` now also has the first provider-backed test-file execution slice through:
    - `adk.compiler.TestFileExecutionResult`
    - `adk.compiler.TestFileExecutionBridge`
    - `adk.compiler.TestFileExecutionProvider`
  - single-project `adenc test` now prefers that authored test-file execution path before falling back to the older direct Rust check loop:
    - the cached bridge executes a Aden-owned test-file provider contract
    - that Aden contract uses the public `aden.process.*` surface to invoke the current `adenc` binary as a host compiler provider
  ...
  - the remaining workspace/builder hot paths now also prefer authored provider-backed execution through the cached bridge:
    - `adk.compiler.WorkspaceCheckExecutionResult`
    - `adk.compiler.WorkspaceCheckExecutionBridge`
    - `adk.compiler.WorkspaceCheckExecutionProvider`
    - `adk.compiler.WorkspaceTestExecutionResult`
    - `adk.compiler.WorkspaceTestExecutionBridge`
    - `adk.compiler.WorkspaceTestExecutionProvider`
    - `adk.build.WorkspaceBuildExecutionResult`
    - `adk.build.WorkspaceBuildExecutionBridge`
    - `adk.build.WorkspaceBuildExecutionProvider`
    - `adk.build.BuildPipelineExecutionResult`
    - `adk.build.BuildPipelineExecutionBridge`
    - `adk.build.BuildPipelineExecutionProvider`
...
- operational correction after that aggressive lift:
  ...
  - this removes the nested `adenc` provider-process tax from the general suite while keeping the compiler/runtime ownership seam alive without hard-coding that seam to a staging directory name
- the first real Aden-side compiler/runtime project roots now live directly at the repository top level:
  - [compiler](/G:/Programming/Rust/Aden Lang/compiler)
  - [runtime](/G:/Programming/Rust/Aden Lang/runtime)
- those members are intentionally still small, but they are real Aden projects with manifests, entrypoints, and smoke tests that the Rust-built `adenc` can already compile and exercise
- compiler/runtime port structure is now explicitly governed by [SELFHOST_PORTING_RULES.md](/G:/Programming/Rust/Aden Lang/docs/SELFHOST_PORTING_RULES.md):
  - keep packages nested and intentional
  - prefer small focused classes
  - no mega-files or Aden equivalents of giant Rust blobs
...
- the first explicit compiler/runtime package map now also exists in [SELFHOST_PACKAGE_MAP.md](/G:/Programming/Rust/Aden Lang/docs/SELFHOST_PACKAGE_MAP.md):
  - `compiler.*` and `runtime.*` are the real roots
  - `*.entry` is only for entrypoints and top-level stage identity
  - real subsystem slices should land in domain packages such as frontend/semantics/lowering/backend and startup/memory/handles/strings/interop/io as they come online
  ...
  - the widened adklib feature lock exposed one more compiler truth:
  ...
  - Rust `cli/build/mod.rs` now uses bridge-first author-build copy paths for:
    ...
    - Rust `cli/build/mod.rs` now also consumes the Aden-owned publication plan for:
    ...
    - Rust `cli/build/mod.rs` now also uses bridge-first authored layout materialization for build staging roots, emitted object/asm staging, resource copies, and sanity `src_main_aden` copies
  ...
  - the first live `adk.compiler` bridge slice exposed another real ML64 truth:
  ...
  - the later `adk.compiler` slices exposed another bootstrap truth:
    - cold bridge prewarm could still fail too eagerly even after the cached bridge executable became usable
    - `prewarm_adk_build_bridge_runner()` now retries the ensure path and accepts a now-available cached exe after transient output-lock failures
  - this is a worthwhile Rust-side cleanup to keep bootstrap output honest and inspectable now, but the deeper emitter cleanup still belongs to the future Aden compiler/runtime backend rewrite:
...
4. Keep moving any feature surface that can live in Aden stdlib out of Rust bootstrap ownership.
...
- `F1-53` collection iteration contract
  - `Collection` now extends `aden.lang.Iterable`
  - the current non-fail-fast object-stream baseline is real across `ArrayList`, `LinkedList`, `HashSet`, and `aden.collections.Array` once its lane is established
  - `Map` now has an explicit non-fail-fast `keyIterator()` baseline instead of leaving map iteration implicit
  - manual `iterator()` use plus `foreach` over collection objects and `Array` values with `Object` / `var` loop variables are now executable and fixture-validated
...
- public raw interop lives in `aden.interop.*`
- the first executable slice covers:
  ...
  - structured mixed argument marshalling through `aden.interop.NativeArgument`
  ...
  - registered callback handles with native-to-Aden trampoline re-entry
...
- absorbed foreign-backed features should wrap that raw surface behind
  Aden-native contracts instead of making Rust a privileged reachback path
