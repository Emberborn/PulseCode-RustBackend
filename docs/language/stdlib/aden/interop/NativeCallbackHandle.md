...
Source: ``stdlib/src/aden/interop/NativeCallbackHandle.aden``

## Summary

ARC-managed registration handle for one Aden callback exposed as one native function pointer.
Use this when foreign code must call back into Aden through one raw function pointer without leaking callback-slot management into normal user code.
...
Use this when callback handles participate in hash-based Aden caches.
