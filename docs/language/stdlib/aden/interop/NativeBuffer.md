...
Source: ``stdlib/src/aden/interop/NativeBuffer.aden``
...
Wraps one foreign allocation and transfers release responsibility into Aden.
Use this when foreign code returned one allocation that should now be ARC-managed by the Aden wrapper.

### ``public static NativeBuffer borrow(NativePointer pointer, int byteLength)``

Wraps one foreign allocation as borrowed memory.
Use this when Aden may inspect or mutate one native buffer but must not release it.
...
Use this when advanced interop code wants a structured Aden wrapper without automatic release behavior.
...
Use this for direct mutable-buffer inspection under Aden-owned lifetime rules.
