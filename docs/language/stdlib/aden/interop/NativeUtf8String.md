...
Source: ``stdlib/src/aden/interop/NativeUtf8String.aden``

## Summary

Owned temporary UTF-8 backing storage for native interop.
Use this when Aden text must cross the foreign boundary as one native NUL-terminated byte buffer.
...
Encodes one Aden string into owned native UTF-8 storage with a trailing NUL terminator.
...
Decodes one explicit-length UTF-8 payload into a Aden string.
Returns `null` when the supplied pointer is null.

### ``public static String decodeNullTerminated(NativePointer pointer)``

Decodes one NUL-terminated UTF-8 payload into a Aden string.
Returns `null` when the supplied pointer is null.
