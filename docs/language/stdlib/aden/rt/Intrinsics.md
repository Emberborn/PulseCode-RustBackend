...
Source: ``stdlib/src/aden/rt/Intrinsics.aden``
...
Returns the UTF-8 byte length of one Aden string payload.
...
Allocates one native UTF-8 buffer with a trailing NUL terminator for the supplied Aden string.
Returns `0` when allocation fails.

### ``public static String hostStringFromUtf8(long address, int byteLength)``

Decodes one explicit-length UTF-8 payload from a native address into a Aden string.
Returns `null` when the native address is null.

### ``public static String hostStringFromUtf8Z(long address)``

Decodes one NUL-terminated UTF-8 payload from a native address into a Aden string.
Returns `null` when the native address is null.
...
Registers one zero-argument Aden callback and returns one runtime callback slot id.
Use this only behind `aden.interop.NativeCallbackHandle`.
...
Registers one one-argument Aden callback and returns one runtime callback slot id.
Use this only behind `aden.interop.NativeCallbackHandle`.
...
Registers one two-argument Aden callback and returns one runtime callback slot id.
Use this only behind `aden.interop.NativeCallbackHandle`.
...
Registers one three-argument Aden callback and returns one runtime callback slot id.
Use this only behind `aden.interop.NativeCallbackHandle`.
...
Registers one four-argument Aden callback and returns one runtime callback slot id.
Use this only behind `aden.interop.NativeCallbackHandle`.
