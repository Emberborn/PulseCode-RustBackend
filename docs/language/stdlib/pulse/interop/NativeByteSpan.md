...
Source: ``stdlib/src/pulse/interop/NativeByteSpan.pulse``

## Summary

Non-owning byte view over one native memory range.
Use this when Pulse code needs to inspect or slice native memory without taking ownership of the backing allocation itself.
...
Use this when foreign code returned owned or borrowed byte data that should be raised into Pulse text.
