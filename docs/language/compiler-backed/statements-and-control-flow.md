...
- failed assertions lower through the stdlib-owned `com.aden.lang.AssertionError` bridge
...
- `for (Object item : iterableExpr) { ... }` works when the source type is `com.aden.lang.Iterable`
- `for (var item : iterableExpr) { ... }` works on that same object-stream `Iterable` baseline
- `for (int value : arrayExpr) { ... }` works for the current stdlib `com.aden.collections.Array` int lane
- `for (String value : arrayExpr) { ... }` works for the current stdlib `com.aden.collections.Array` string lane
- `for (Object value : arrayExpr) { ... }` works for `com.aden.collections.Array` once writes have established its int or string lane
- `for (var value : arrayExpr) { ... }` works for `com.aden.collections.Array` through that same object-stream baseline once the lane is established
...
- `com.aden.collections.Array` object-stream iteration is also an object-stream baseline and boxes int-lane reads to `Integer`
