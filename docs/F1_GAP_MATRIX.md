...
| concurrency library baseline | Partial | `pulse.concurrent` now has a locked shipped scope: monitor, mutex, event, semaphore, latch, wait/notify coordination, boolean/int/long/reference atomics, selected concurrent collections (`ConcurrentHashMap`, `ConcurrentHashSet`, `CopyOnWriteArrayList`, `ConcurrentLinkedQueue`, `ConcurrentLinkedDeque`, `LinkedBlockingQueue`, `LinkedBlockingDeque`), thread-per-task plus fixed-worker-pool plus scheduled-worker-pool plus work-stealing fork-join executor/future baselines, delayed plus fixed-rate/fixed-delay periodic scheduling, bulk callable submission/first-success coordination, and a broader but still bounded `CompletableFuture` baseline including both/either/all-of/any-of composition are in; `Thread` stays in `pulse.lang`; skip-list / transfer / fairness-specialized concurrent-collection families, richer scheduled-executor families, and broader completion-stage families are still later work |
...
| concurrent | Partial | Low-level concurrency floor plus selected concurrent collections, thread-per-task plus fixed-worker-pool plus scheduled-worker-pool plus work-stealing fork-join executor/future baselines, delayed plus fixed-rate/fixed-delay periodic scheduling, `invokeAll` / `invokeAny`, and broader-but-bounded `CompletableFuture` support including both/either/all-of/any-of composition are shipped; skip-list / transfer / fairness-specialized concurrent-collection families, richer scheduled-executor families, and broader completion-stage families remain later work |
