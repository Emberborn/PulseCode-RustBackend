...
- PulseOS requested-target validation proves PulseOS contract and plan truth without pretending the adapter executes yet
...
- the target-neutral executable corpus inside `crates/pulsec-cli/tests/fixture_projects.rs`
...
- deterministic front-to-back Pulse execution semantics
...
- `crates/pulsec-cli/src/backend/tests/mod.rs`
- `crates/pulsec-cli/tests/stage_locks_c2.rs`
- `crates/pulsec-cli/tests/stage_locks_c3.rs`
- `crates/pulsec-cli/tests/e3_parity.rs`
- the Windows adapter parity/failure/publication subfamily inside `crates/pulsec-cli/tests/fixture_projects.rs`
...
### PulseOS Requested-Target Contract / Proof Validation

This layer owns validation for what the requested `pulseos-x64` lane means today without claiming executable support:
...
This should now be read as PulseOS platform-contract proof, not as proof that the adapter executes yet.

Primary homes:

- `crates/pulsec-cli/src/backend/tests/mod.rs` for `RB-13` through `RB-17` plan/contract emission checks
- `crates/pulsec-cli/tests/stage_locks.rs` for `RB-13` through `RB-18` doc/board/handoff locks
- `crates/pulsec-cli/tests/stage_locks_d.rs` for requested-target CLI/manifest/build-plan snapshots that keep `pulseos-x64` visible and honest

Passing this layer does not mean PulseOS executes yet. It means the requested-target contract is published and locked honestly.

### Linux Expansion Slot

`linux-x64` is already part of the canonical public target taxonomy, but it does not yet have adapter execution or a published first-slice contract equivalent to PulseOS.
...
- A passing Windows x64 executable/parity test is never proof of PulseOS or Linux execution.
- A passing PulseOS requested-target plan/contract test is never proof of PulseOS execution.
...
- New adapter execution suites for PulseOS or Linux should get their own file homes instead of extending `e3_parity.rs` as if all adapters shared one parity lane.
...
|---|---|---|
| `crates/pulsec-cli/tests/stage_locks_d.rs` | target-neutral | CLI, manifest, scaffold, canonical target, and build-plan surface locks |
| `crates/pulsec-cli/tests/phase_d_cli.rs` | target-neutral | project lifecycle and publication behavior that stays above adapter-specific runtime claims |
| `crates/pulsec-cli/tests/fixture_projects.rs` | mixed | portable executable corpus plus retained Windows x64 parity/failure overlay by test family |
| `crates/pulsec-cli/tests/stage_locks.rs` | cross-cutting | board, handoff, roadmap, and contract locks for rebase/F1 continuity |
| `crates/pulsec-cli/src/backend/tests/mod.rs` | Windows x64 adapter + requested-target contract | active adapter contract/codegen/runtime plan checks plus requested-target PulseOS contract emissions |
| `crates/pulsec-cli/tests/stage_locks_c2.rs` | Windows x64 adapter | runtime/ARC execution and contract locks |
| `crates/pulsec-cli/tests/stage_locks_c3.rs` | Windows x64 adapter | object-model, dispatch, and runtime execution locks |
| `crates/pulsec-cli/tests/e3_parity.rs` | Windows x64 adapter | fat/shared parity evidence on the retained Windows x64 executable lane |
