...
fn cli_build_executes_runtime_mix_fixture_project_in_shared_mode() {
    ...
    assert!(
        report.contains("pulsec_std_com_pulse_lang_IO_println"),
        "shared build must include stdlib callable exports in inventory:\n{}",
        report
    );

    let version = env!("CARGO_PKG_VERSION");
    let runtime_library = root
        .join("build")
        .join(format!("pulsecore-{version}.dll"));
    let runtime_import_library = root
        .join("build")
    ...
    let exe = root.join("build").join("main.exe");
    assert!(exe.exists(), "missing shared-mode executable '{}'", exe.display());
...
fn cli_build_executes_strict_stress_soak_fixture_project_in_shared_mode() {
    ...
    let startup_asm = fs::read_to_string(root.join("build").join("obj").join("runtime").join("Startup.asm"))
    ...
    assert!(
        startup_asm.find("call pulsec_rt_init").unwrap() < startup_asm.find("call pulsec_rt_shutdown").unwrap(),
        "shared startup asm must init before shutdown\n{}",
        startup_asm
    );

    let exe = root.join("build").join("main.exe");
    let run = Command::new(&exe).output().expect("run strict_stress_soak shared exe");
    assert!(
        run.status.success(),
...
fn cli_build_executes_strict_stress_soak_repeatedly_in_shared_mode() {
    ...
    for _ in 0..iterations {
        ...
        assert!(exe.exists(), "missing shared soak executable '{}'", exe.display());
        assert!(
            runtime_library.exists(),
...
fn cli_build_emits_ir_summary_for_fixture_project() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Build IR ready:"), "unexpected stdout:\n{}", stdout);
    assert!(stdout.contains("IR artifact:"), "unexpected stdout:\n{}", stdout);
    assert!(stdout.contains("Native plan:"), "unexpected stdout:\n{}", stdout);
    assert!(stdout.contains("Object scaffold:"), "unexpected stdout:\n{}", stdout);
    assert!(stdout.contains("Link report:"), "unexpected stdout:\n{}", stdout);
    assert!(stdout.contains("Entry codegen:"), "unexpected stdout:\n{}", stdout);
    ...
    let object_path = if root.join("build").join("obj").join("runtime").join("Startup.obj").exists() {
        root.join("build").join("obj").join("runtime").join("Startup.obj")
    } else {
...
fn cli_build_accepts_runtime_string_intrinsics_baseline() {
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                String text = Intrinsics.stringConcat("a", "b");
                int len = Intrinsics.stringLength(text);
    ...
    let plan = fs::read_to_string(&plan_path).expect("read native plan");
    assert!(
        plan.contains("com.pulse.rt.Intrinsics.stringConcat"),
        "missing stringConcat runtime symbol in plan:\n{}",
        plan
    );
    assert!(
        plan.contains("com.pulse.rt.Intrinsics.stringLength"),
        "missing stringLength runtime symbol in plan:\n{}",
        plan
    );
    assert!(
        plan.contains("com.pulse.rt.Intrinsics.intToString"),
        "missing intToString runtime symbol in plan:\n{}",
        plan
    );
    assert!(
        plan.contains("com.pulse.rt.Intrinsics.booleanToString"),
        "missing booleanToString runtime symbol in plan:\n{}",
        plan
    );
...
fn cli_build_shared_runtime_abi_mismatch_fails_deterministically_at_launch() {
    ...
    let exe = root.join("build").join("main.exe");
    let run = Command::new(&exe).output().expect("run shared mismatch exe");
    assert!(
        !run.status.success(),
...
fn cli_build_shared_hard_fails_when_linker_is_unavailable_and_no_fake_artifacts_publish() {
    ...
    assert!(report.contains("status=not-linked"), "expected not-linked report:\n{}", report);
    assert!(report.contains("mode=shared"), "expected shared-mode report:\n{}", report);
    assert!(
    ...
    assert!(
        !root.join("build").join(format!("pulsecore-{version}.dll")).exists(),
        "shared link failure must not publish runtime library"
    );
    assert!(
        !root.join("build").join(format!("pulsecore-{version}.lib")).exists(),
...
fn cli_build_shared_launch_metadata_detects_missing_runtime_library_payload() {
    ...
    let new_output = Command::new(env!("CARGO_BIN_EXE_pulsec"))
        .arg("new")
    ...
    assert!(new_output.status.success(), "shared app scaffold should succeed");
    ...
    let manifest = fs::read_to_string(&manifest_path)
        ...
        .replace("entry = \"app/main/Main.pulse\"", "entry = \"app/runtime/Main.pulse\"")
    ...
    let runtime_library = release_root
        .join("bin")
        .join(format!("pulsecore-{version}.dll"));
    assert!(runtime_library.exists(), "expected shared runtime library before removal");
...
fn cli_build_executes_runtime_panic_intrinsic_baseline() {
    ...
    let plan = fs::read_to_string(&plan_path).expect("read native plan");
    assert!(
        plan.contains("com.pulse.rt.Intrinsics.panic"),
...
fn cli_build_executes_cross_method_uncaught_exception_stack_trace_flow() {
    ...
    let out = String::from_utf8_lossy(&run.stdout).replace('\r', "");
    assert!(out.starts_with("Exception: boom\n"), "unexpected stdout:\n{}", out);
...
fn cli_build_executes_assert_baseline() {
    ...
    let exe_path = root.join("build").join("main.exe");
    let run = Command::new(&exe_path).output().expect("run built executable");
    assert!(run.status.success(), "assert success baseline should exit zero");
    let out = String::from_utf8_lossy(&run.stdout).replace('\r', "");
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                ...
                IO.println(String.valueOf(new Main()));
            }
        }
        "#,
    );
...
fn cli_build_executes_stringbuilder_insert_delete_setlength_and_object_append_flow() {
    ...
    let out = String::from_utf8_lossy(&run.stdout).replace('\r', "");
    assert_eq!(out, "abcdapp.core.Main\nabcd\nab\n4\ntrue\ntrue\n");
...
fn cli_build_executes_wrapper_invalid_boolean_parse_panics() {
    ...
    let out = String::from_utf8_lossy(&run.stdout).replace('\r', "");
    assert!(
        out.starts_with("Invalid boolean literal\n"),
        "unexpected stdout:\n{}",
        out
    );
    assert!(out.contains("Stack trace:\n"), "missing stack trace:\n{}", out);
...
fn cli_build_executes_runtime_heavy_app_with_side_by_side_fat_shared_parity() {
    ...
    assert_eq!(fat_out, "hero\n6\n9\n11\n6\ntrue\n");
    assert_eq!(shared_out, "hero\n6\n9\n11\n6\ntrue\n");
    assert_eq!(fat_out, shared_out, "runtime-heavy fat/shared outputs diverged");
}
...
fn cli_build_executes_failure_paths_with_side_by_side_fat_shared_parity() {
    ...
    let Some(shared_null) =
        build_and_run_source_expect_failure_in_mode(null_dispatch, "shared")
    else {
        return;
    };
    for out in [&fat_null, &shared_null] {
        assert!(out.contains("before\n"), "missing pre-panic marker\n{out}");
        assert!(out.contains("Null dispatch receiver"), "missing null-dispatch diagnostic\n{out}");
        assert!(out.contains("Stack trace:\n"), "missing stack trace\n{out}");
    }
    ...
    let Some(shared_cast) =
        build_and_run_source_expect_failure_in_mode(invalid_cast, "shared")
    else {
        return;
    };
    for out in [&fat_cast, &shared_cast] {
        assert!(out.contains("Invalid cast"), "missing invalid-cast diagnostic\n{out}");
        assert!(out.contains("Stack trace:\n"), "missing stack trace\n{out}");
    }
}
...
fn cli_build_shared_boundary_failure_diagnostics_remain_explicit_and_deterministic() {
    ...
    let runtime_abi_output = Command::new(env!("CARGO_BIN_EXE_pulsec"))
        .env("PULSEC_FORCE_RUNTIME_ABI_VERSION", "1")
        .arg("build")
        .arg(entry.to_str().expect("entry utf8"))
        .arg("--source-root")
    ...
    assert!(!runtime_run.status.success(), "expected shared runtime ABI mismatch failure");
    let runtime_out = String::from_utf8_lossy(&runtime_run.stdout).replace('\r', "");
    assert!(runtime_out.contains("Runtime ABI mismatch"), "unexpected runtime ABI mismatch output:\n{runtime_out}");

    let object_model_output = Command::new(env!("CARGO_BIN_EXE_pulsec"))
        .env("PULSEC_FORCE_OBJECT_MODEL_ABI_VERSION", "2")
        .arg("build")
        .arg(entry.to_str().expect("entry utf8"))
        .arg("--source-root")
    ...
    let launch_build = Command::new(env!("CARGO_BIN_EXE_pulsec"))
        .arg("build")
        .arg("--project-root")
        .arg(project.to_str().expect("project utf8"))
        .output()
...
fn cli_build_preserves_package_object_layout_for_multifile_project() {
    ...
    let startup_obj = root.join("build").join("obj").join("runtime").join("Startup.obj");
    let main_obj = root.join("build").join("obj").join("app").join("core").join("Main.obj");
    let helper_obj = root.join("build").join("obj").join("app").join("core").join("Helper.obj");
    let io_obj = root.join("build").join("obj").join("com").join("pulse").join("lang").join("IO.obj");
    assert!(startup_obj.exists(), "missing startup object '{}'", startup_obj.display());
    assert!(main_obj.exists(), "missing main object '{}'", main_obj.display());
    assert!(helper_obj.exists(), "missing helper object '{}'", helper_obj.display());
    assert!(io_obj.exists(), "missing io object '{}'", io_obj.display());

    let exe_path = root.join("build").join("main.exe");
    assert!(exe_path.exists(), "missing executable '{}'", exe_path.display());
    let run = Command::new(&exe_path).output().expect("run built executable");
    assert!(
        run.status.success(),
        "expected executable success\nstdout:\n{}\nstderr:\n{}",
...
fn cli_build_executes_fully_qualified_io_call_without_import() {
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                com.pulse.lang.IO.println("fq_ok");
            }
        }
        "#,
    );
...
fn cli_build_link_report_includes_codegen_and_status() {
    ...
    let report = fs::read_to_string(root.join("build").join("native.link.txt")).expect("read report");
    assert!(report.contains("status="), "missing status in report:\n{}", report);
    assert!(report.contains("entry_codegen="), "missing entry codegen in report:\n{}", report);
}
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                ...
                d.addFirst(5);
                d.addLast(9);
                IO.println(d.removeFirst());
                IO.println(d.removeLast());
...
fn cli_build_executes_static_import_math_path() {
    ...
    write_file(
        ...
        package app.core;
        import com.pulse.lang.IO;
        import static com.pulse.math.Math.max;
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                ...
                IO.println(a.toEpochMillis());
                IO.println(b.toEpochMillis());

                Duration d1 = Duration.ofMillis(9);
                Duration d2 = new Duration(4);
                IO.println(d1.plus(d2).toMillis());
                IO.println(d1.minus(d2).toMillis());
            }
        }
        "#,
    );
