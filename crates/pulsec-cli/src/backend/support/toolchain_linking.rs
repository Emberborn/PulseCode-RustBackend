...
use crate::cli::config::{
    ...
    run_author_build_bridge_request,
};
...
fn parse_toolchain_process_bridge_output(
    ...
    let exit_code = required_bridge_value(&values, "exitCode")?
        .parse::<i32>()
        .map_err(|e| format!("invalid author process bridge exit code: {e}"))?;
...
fn author_toolchain_discovery_plan() -> Result<AuthorToolchainDiscoveryPlan, String> {
    let output = run_author_build_bridge_request(&emit_toolchain_discovery_bridge_request())?;
    parse_toolchain_discovery_bridge_output(&output)
}

fn author_toolchain_probe_result() -> Result<AuthorToolchainDiscoveryResult, String> {
    let output = run_author_build_bridge_request(&emit_toolchain_probe_bridge_request())?;
    parse_toolchain_probe_bridge_output(&output)
}

#[cfg(test)]
fn author_toolchain_probe_result_for_plan(
    visual_studio_roots: &[PathBuf],
    visual_studio_tool_root_relative_path: &Path,
    visual_studio_linker_relative_path: &Path,
    windows_sdk_lib_roots: &[PathBuf],
    ...
    let output = run_author_build_bridge_request(&emit_toolchain_probe_custom_bridge_request(
        visual_studio_roots,
...
fn author_toolchain_candidate_plan(
    linker_override: Option<&Path>,
    env_linker: Option<&str>,
    discovered_visual_studio_linkers: &[PathBuf],
) -> Result<(Vec<PathBuf>, Vec<PathBuf>), String> {
    let output = run_author_build_bridge_request(&emit_toolchain_candidate_bridge_request(
        linker_override,
        env_linker,
...
fn author_toolchain_write_response_file(
    response_file_path: &Path,
    args: &[String],
...
fn author_toolchain_execute_process(plan: &ToolchainProcessPlan) -> Result<Output, String> {
    ...
    let result = run_author_build_bridge_request(&request)
...
pub(crate) fn discover_kernel32_lib() -> Option<PathBuf> {
    if let Ok(result) = author_toolchain_probe_result() {
        if let Some(candidate) = result.kernel32_lib_path {
            return Some(candidate);
        }
    }
    if let Ok(plan) = author_toolchain_discovery_plan() {
...
fn write_linker_response_file(
    ...
    if let Ok(written_path) = author_toolchain_write_response_file(&rsp_path, args) {
...
pub(crate) fn execute_toolchain_process(
    ...
    if let Ok(output) = author_toolchain_execute_process(plan) {
        return Ok(output);
    }
...
fn plan_windows_shared_app_link_args(
    ...
    for object in app_owned_objects {
        let normalized = object
            ...
            .replace('\\', "/")
            .to_ascii_lowercase();
        if normalized.contains("/obj/pulse/") || normalized.starts_with("obj/pulse/") {
            continue;
        }
        args.push(windows_cmd_path(object));
    }
...
mod tests {
    use super::*;
    use crate::cli::config::prewarm_author_build_bridge_runner;
    ...
    fn toolchain_probe_bridge_executes_windows_probe_graph() {
        ...
        let root = std::env::temp_dir().join(format!(
            "pulsec_toolchain_probe_bridge_{}_{}",
            std::process::id(),
        ...
        let result = author_toolchain_probe_result_for_plan(
            &[vs_root.clone()],
    ...
    fn toolchain_discovery_helpers_lock_windows_probe_walks() {
        let root = std::env::temp_dir().join(format!(
            "pulsec_toolchain_probe_lock_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("time")
    ...
    fn windows_shared_link_args_lock_runtime_and_app_shapes() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/NODEFAULTLIB",
                "/LARGEADDRESSAWARE:NO",
                "/OUT:build/pulsecore.dll",
                "/IMPLIB:build/pulsecore.lib",
                "/EXPORT:pulsec_rt_init",
                "/EXPORT:pulsec_rt_shutdown",
                "obj/runtime/StdlibRuntime.obj",
                "obj/pulse/lang/IO.obj",
                "kernel32.lib",
            ]
            .into_iter()
            .map(String::from)
    ...
    fn toolchain_response_file_bridge_writes_host_file() {
        ...
        let temp_root = std::env::temp_dir().join(format!(
            "pulsec_toolchain_rsp_bridge_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system time")
        ...
        for index in 0..40 {
            args.push(format!(
                "C:/Users/EMBERB~1/AppData/Local/Temp/pulsec_toolchain_rsp_bridge/obj{:02}.obj",
                index
            ));
        }
