...
fn lock_c2_01_arc_header_contract_is_documented_and_emitted_in_native_plan() {
    ...
    assert!(doc.contains("adenc.arc.header.v1"));
    ...
    assert!(plan.contains("\"header_schema\": \"adenc.arc.header.v1\""));
...
fn lock_c2_02_arc_fast_path_null_noop_behavior_executes() {
    ...
    let plan = fs::read_to_string(root.join("build").join("native.plan.json"))
        .expect("read native.plan.json");
    assert!(plan.contains("aden.memory.Memory.retain"));
    assert!(plan.contains("aden.memory.Memory.release"));
...
fn lock_c2_04_arc_insertion_boundaries_emit_retain_release_sequences() {
    ...
    let retain_count = asm.matches("call adenc_rt_arcRetain").count();
    let release_count = asm.matches("call adenc_rt_arcRelease").count();
    assert!(
        retain_count >= 4,
...
fn lock_c2_05_cycle_detector_cadence_and_symbols_are_locked() {
    ...
    assert!(plan.contains("\"schema\": \"adenc.arc.cycle.v1\""));
    ...
    assert!(plan.contains("aden.memory.Memory.cycleYoungPass"));
    assert!(plan.contains("aden.memory.Memory.cycleFullPass"));
    assert!(plan.contains("aden.memory.Memory.cycleTick"));
...
fn lock_c2_user_object_storage_tables_are_dynamic_pointer_backed() {
    ...
    assert!(
        io_asm.contains("adenc_rt_obj_class_ids dq adenc_rt_obj_class_ids_init"),
        "expected object class-id table pointer root in runtime asm"
    );
    assert!(
        io_asm.contains("adenc_rt_obj_class_ids_heap_owned dd 0"),
        "expected object class-id heap-owned flag in runtime asm"
    );
    assert!(
        io_asm.contains("mov r10, qword ptr [adenc_rt_obj_class_ids]"),
        "expected object class-id table loads to dereference pointer root"
    );
    assert!(
        !io_asm.contains("adenc_rt_obj_class_ids dd "),
    ...
    assert!(
        node_asm.contains("adenc_fld_app_core_Node_next dq adenc_fld_app_core_Node_next_tbl"),
        "expected non-static object field pointer root in class asm"
    );
    assert!(
        node_asm.contains("adenc_fld_app_core_Node_next_heap_owned dd 0"),
...
fn lock_c2_06_weak_reference_runtime_support_is_locked() {
    ...
    let plan = fs::read_to_string(root.join("build").join("native.plan.json"))
        .expect("read native.plan.json");
    assert!(plan.contains("aden.memory.Memory.weakNew"));
    assert!(plan.contains("aden.memory.Memory.weakGet"));
    assert!(plan.contains("aden.memory.Memory.weakClear"));
...
fn lock_c2_weak_allocator_grows_beyond_legacy_limit_without_early_exhaustion() {
    ...
    assert!(
        io_asm.contains("call adenc_rt_ensureSlotCapacity"),
...
fn lock_c2_08_array_heap_allocation_contract_is_locked() {
    ...
    assert!(
        doc.contains("adenc.alloc.policy.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.alloc.policy.v1\""));
    ...
    assert!(
        io_asm.contains("adenc_rt_ensureSlotCapacity proc"),
...
fn lock_c2_10_container_shrink_hysteresis_executes() {
    ...
    let io_asm = read_runtime_asm(&root);
    assert!(
        io_asm.contains("adenc_rt_listClear proc"),
        "expected list clear runtime proc in runtime asm"
    );
    assert!(
        io_asm.contains("adenc_rt_mapClear proc"),
...
fn lock_c2_11_list_string_ownership_retain_and_clear_release() {
    ...
    let io_asm = read_runtime_asm(&root);
    assert!(
        io_asm.contains("adenc_rt_listAddString proc")
            && io_asm.contains("call adenc_rt_arcRetain"),
        "expected list add string ownership retain in runtime asm"
    );
    assert!(
        io_asm.contains("adenc_rt_listClear proc") && io_asm.contains("call adenc_rt_arcRelease"),
...
fn lock_c2_11_map_string_ownership_replace_and_clear_release() {
    ...
    let io_asm = read_runtime_asm(&root);
    assert!(
        io_asm.contains("adenc_rt_mapPut proc")
            && io_asm.contains("call adenc_rt_arcRetain")
            && io_asm.contains("call adenc_rt_arcRelease"),
        "expected map put ownership retain/release behavior in runtime asm"
    );
    assert!(
        io_asm.contains("adenc_rt_mapClear proc") && io_asm.contains("call adenc_rt_arcRelease"),
...
fn lock_c2_20_same_package_gameloop_frame_is_bounded_and_executable() {
    ...
    write_file(
        &src_root.join("app/runtime/GameLoop.aden"),
        r#"
        package app.runtime;
...
fn lock_c2_23_threading_contract_is_documented_and_emitted() {
    ...
    assert!(
        doc.contains("adenc.runtime.threading.v1"),
...
fn lock_c2_24_runtime_abi_mismatch_fails_deterministically() {
    ...
    assert!(
        doc.contains("adenc.runtime.abi.v1"),
