...
fn lock_d1_01_help_contract_lists_commands_and_exit_policy() {
    let output = run_adenc(&["help"]);
    assert!(output.status.success(), "help should succeed");
    let stdout = norm(&output.stdout);
    assert!(stdout.contains("Aden Lang CLI"));
...
fn lock_d1_01_version_contract_is_stable() {
    let output = run_adenc(&["version"]);
    assert!(output.status.success(), "version should succeed");
    let stdout = norm(&output.stdout);
    assert_eq!(stdout, "adenc 0.1.0\n");
}
...
fn lock_d1_01_missing_command_is_usage_error() {
    ...
    assert!(stderr.contains("hint: run 'adenc help' to see available commands"));
    assert!(stderr.contains("Usage:"));
    assert!(stderr.contains("adenc <command> [arguments] [flags]"));
    assert_no_ansi(&stderr);
}

#[test]
fn lock_d1_01_unknown_command_is_usage_error() {
    let output = run_adenc(&["compile"]);
    assert_eq!(output.status.code(), Some(2));
    let stderr = norm(&output.stderr);
    assert!(stderr.contains("error[PULSEC:E_USAGE]: Unknown command 'compile'"));
    assert!(stderr.contains("diag.code=PULSEC:E_USAGE"));
    assert!(stderr.contains("Unknown command 'compile'"));
    assert!(stderr.contains("Use 'adenc <command> --help'"));
    assert_no_ansi(&stderr);
}

#[test]
fn lock_d1_01_command_specific_help_contract_is_stable() {
    let check = run_adenc(&["check", "--help"]);
    assert!(check.status.success());
    assert_eq!(
        norm(&check.stdout),
        "Usage: adenc check [<entry.aden>] [--project-root <dir>] [--source-root <dir>] [--strict-package|--friendly]\n"
    );

    let build = run_adenc(&["build", "--help"]);
    assert!(build.status.success());
    assert_eq!(
        norm(&build.stdout),
        "Usage: adenc build [<entry.aden>] [--project-root <dir>] [--source-root <dir>] [--profile <debug|release>] [--target <windows-x64|adenos-x64|linux-x64|macos-arm64>] [--output-mode <fat|shared>] [--runtime-debug-allocator <on|off>] [--runtime-cycle-collector <on|off>] [--out-dir <dir>] [--linker <path>] [--assembler <path>]\n"
    );

    let test = run_adenc(&["test", "--help"]);
    assert!(test.status.success());
    assert_eq!(
        norm(&test.stdout),
        "Usage: adenc test [--project-root <dir>] [--source-root <dir>] [--strict-package|--friendly]\n"
    );

    let package = run_adenc(&["package", "--help"]);
...
fn lock_d1_02_new_app_scaffold_is_deterministic_and_buildable() {
    ...
    let manifest = project.join("adenc.toml");
    let entry = project
        .join("src")
        .join("main")
        .join("aden")
    ...
    assert!(manifest_text.contains("main_aden = \"src/main/aden\""));
    assert!(manifest_text.contains("entry = \"app/main/Main.aden\""));
...
fn lock_d1_02_new_lib_scaffold_is_deterministic_and_checkable() {
    ...
    let manifest_text = read(&manifest).replace('\r', "");
    assert!(manifest_text.contains("[package]\nname = \"core_lib\""));
    assert!(manifest_text.contains("entry = \"lib/core/Main.aden\""));
    assert!(manifest_text.contains("target = \"windows-x64\""));
    assert_eq!(
        read(&entry).replace('\r', ""),
...
fn lock_d1_03_check_manifest_project_mode_resolves_entry_and_source_root() {
    ...
    let check = run_adenc(&["check", "--project-root", project.to_str().expect("utf8")]);
    assert!(
        check.status.success(),
...
fn lock_d1_03_check_strict_and_friendly_modes_are_deterministic() {
    ...
    fs::write(
        project.join("adenc.toml"),
        "[package]\nname = \"mode_demo\"\nversion = \"0.1.0\"\n\n[sources]\nroot = \"src\"\nentry = \"app/bad/Main.aden\"\n",
    )
    .expect("write manifest");
    fs::write(
        src.join("app").join("bad").join("Main.aden"),
        "package app.main;\nclass Main {\n    public static void main() {\n    }\n}\n",
    )
    .expect("write source");

    let friendly = run_adenc(&[
        "check",
        "--project-root",
...
fn lock_d1_03_check_failure_summary_includes_context() {
    ...
    .expect("write manifest");
    fs::write(
        src.join("app").join("main").join("Main.aden"),
        "package app.main;\nimport app.missing.Util;\nclass Main {\n    public static void main() {\n    }\n}\n",
    )
    .expect("write source");

    let output = run_adenc(&["check", "--project-root", project.to_str().expect("utf8")]);
    assert_eq!(output.status.code(), Some(1));
    let stderr = norm(&output.stderr);
...
fn lock_d1_04_build_manifest_project_mode_defaults_to_release_profile() {
    ...
    let build = run_adenc(&["build", "--project-root", project.to_str().expect("utf8")]);
    assert!(
        build.status.success(),
...
fn lock_d1_04_build_manifest_profile_and_cli_out_dir_are_honored() {
    ...
    assert!(new_output.status.success(), "scaffold should succeed");
    let project = root.join(project_name);
    let manifest_path = project.join("adenc.toml");
    ...
    let manifest_release =
        run_adenc(&["build", "--project-root", project.to_str().expect("utf8")]);
    assert!(
...
fn lock_d1_04_build_direct_mode_default_profile_and_status_summary_are_stable() {
    ...
    fs::create_dir_all(src.join("app").join("main")).expect("create dirs");
    let entry = src.join("app").join("main").join("Main.aden");
    fs::write(
        &entry,
...
fn lock_d1_05_test_discovers_and_reports_passing_project_tests() {
    ...
    fs::write(
        test_dir.join("SmokeTest.aden"),
        "package app.tests;\nimport aden.lang.IO;\nclass SmokeTest {\n    public static void main() {\n        IO.println(\"ok\");\n    }\n}\n",
    )
    .expect("write test source");

    let output = run_adenc(&["test", "--project-root", project.to_str().expect("utf8")]);
    assert!(
        output.status.success(),
    ...
    assert!(stdout.contains("[PASS] app\\tests\\SmokeTest.aden"));
...
fn lock_d1_05_test_reports_failures_and_exits_non_zero() {
    ...
    assert!(stderr.contains("[FAIL] app\\tests\\BrokenTest.aden :: Import 'app.missing.Util'"));
}
...
fn lock_d1_05_test_no_files_is_deterministic_failure() {
    ...
    assert!(stderr.contains("hint: add .aden files under tests/"));
    assert!(stderr.contains("Test discovery failed: no .aden tests found under"));
    assert_no_ansi(&stderr);
}
...
fn lock_d3_05_build_emits_stamped_artifact_bundle() {
    ...
    let output = run_adenc(&["build", "--project-root", project.to_str().expect("utf8")]);
    assert!(
        output.status.success(),
    ...
    assert!(!artifacts
        .join("artifact_stamp_app-0.1.0-adenc.ir.txt")
        .exists());
    assert!(!artifacts
...
fn lock_d1_07_diagnostic_envelope_is_machine_readable_and_actionable() {
    let output = run_adenc(&["check"]);
    assert_eq!(output.status.code(), Some(2));
    let stderr = norm(&output.stderr);
    assert!(stderr.contains("error[PULSEC:E_USAGE]:"));
    assert!(stderr.contains("diag.code=PULSEC:E_USAGE"));
    assert!(stderr.contains("hint: provide <entry.aden> or set [sources].entry in adenc.toml"));
    assert_no_ansi(&stderr);
}
...
#[test]
fn lock_d2_01_adklib_opt_in_enables_adk_imports() {
    let root = unique_temp_root();
    let project = root.join("adklib_enabled_ok");
    fs::create_dir_all(
        project
            .join("src")
            .join("main")
...
fn lock_d2_02_project_model_defaults_follow_gradle_layout() {
    ...
    for path in [
        project.join("src").join("main").join("aden"),
        project.join("src").join("main").join("resources"),
        project.join("src").join("test").join("aden"),
        project.join("src").join("test").join("resources"),
        project.join("src").join("api").join("aden"),
    ...
    .expect("write test");
    let test_output = run_adenc(&["test", "--project-root", project.to_str().expect("utf8")]);
    assert!(
        test_output.status.success(),
        "test command should detect src/test/aden\nstdout:\n{}\nstderr:\n{}",
        norm(&test_output.stdout),
        norm(&test_output.stderr)
    );
    assert!(norm_slashes(&norm(&test_output.stdout)).contains("src/test/aden"));

    let build_output = run_adenc(&["build", "--project-root", project.to_str().expect("utf8")]);
    assert!(
...
fn lock_d2_03_layout_overrides_are_respected() {
    ...
    assert!(
        check.status.success(),
        "check should respect main_aden override"
    );

    let test = run_adenc(&["test", "--project-root", project.to_str().expect("utf8")]);
    assert!(
        test.status.success(),
        "test should respect test_aden override"
    );
...
fn lock_d2_03_empty_layout_override_is_rejected() {
    ...
    let stderr = norm(&output.stderr);
    assert!(stderr.contains("Manifest validation error"));
    assert!(stderr.contains("[sources].main_aden cannot be empty"));
}
...
fn lock_d2_04_build_config_plan_surfaces_resolved_target_profile_and_runtime_toggles() {
    ...
    let stdout = norm(&build.stdout);
    assert!(
        stdout.contains("Build mode: profile=release target=adenos-x64")
    ...
    assert!(plan_text.contains("\"schema\": \"adenc.build.config.v1\""));
    assert!(plan_text.contains("\"profile\": \"release\""));
    assert!(plan_text.contains("\"target\": \"adenos-x64\""));
    assert!(plan_text.contains("\"target_adapter\": {"));
    assert!(plan_text.contains("\"requested\": {"));
    assert!(plan_text.contains("\"lane_name\": \"adenos-x64-first-slice\""));
    ...
    assert!(plan_text.contains("\"id\": \"adenc-runtime-abi-v2-shared-windows-bootstrap\""));
    ...
    assert!(native_plan_text.contains("\"lane_name\": \"adenos-x64-first-slice\""));
    ...
    assert!(native_plan_text.contains("\"id\": \"adenc-runtime-abi-v2-shared-windows-bootstrap\""));
    assert!(native_plan_text.contains("\"outputs\": {"));
    assert!(native_plan_text.contains("\"shared_boundary\": {"));
    assert!(native_plan_text.contains("\"schema\": \"adenc.shared_boundary.v1\""));
    ...
    assert!(native_plan_text.contains("\"schema\": \"adenc.shared_runtime.lookup.v1\""));
    ...
    assert!(native_plan_text.contains("\"runtime_library\": \"adencore-"));
    assert!(native_plan_text.contains("\"runtime_import_library\": \"adencore-"));
    assert!(native_plan_text.contains("\"launch_descriptor\": \"staging/bin/launch.txt\""));
    assert!(native_plan_text.contains("\"runtime_library\": \"adencore-"));
    assert!(native_plan_text.contains("\"runtime_import_library\": \"adencore-"));
    assert!(native_plan_text.contains("\"base_name\": \"adencore\""));
    assert!(native_plan_text.contains(&format!(
        "\"file_name\": \"adencore-{}.dll\"",
        env!("CARGO_PKG_VERSION")
    )));
    assert!(native_plan_text.contains(&format!(
        "\"file_name\": \"adencore-{}.lib\"",
        env!("CARGO_PKG_VERSION")
    )));
    assert!(native_plan_text.contains("\"version_source\": \"adenc_toolchain_semver\""));
    ...
    assert!(native_plan_text.contains("\"schema\": \"adenc.shared_runtime.abi.v1\""));
    assert!(native_plan_text.contains("\"mode\": \"shared\""));
    assert!(native_plan_text.contains("\"compatibility\": {"));
    assert!(native_plan_text.contains("\"runtime_abi_schema\": \"adenc.runtime.abi.v1\""));
    assert!(native_plan_text.contains("\"runtime_abi_version\": 2"));
    assert!(
        native_plan_text.contains("\"object_model_abi_schema\": \"adenc.object_model.abi.v1\"")
    );
    ...
    assert!(native_plan_text.contains("\"forbidden_private_export_patterns\": [\"rt_*\", \"adenc_rt_obj_*\", \"adenc_rt_class_*\"]"));
    assert!(native_plan_text.contains("\"runtime_state_layout\": {"));
    assert!(native_plan_text.contains("\"schema\": \"adenc.shared_runtime.state_layout.v1\""));
    ...
    assert!(native_plan_text.contains("\"object_metadata_symbols\": [\"adenc_rt_obj_counter\", \"adenc_rt_obj_class_ids\", \"adenc_rt_obj_class_ids_init\", \"adenc_rt_obj_class_ids_heap_owned\"]"));
    ...
    assert!(native_plan_text.contains("\"adenc_rt_init\""));
    assert!(native_plan_text.contains("\"adenc_rt_objectNew\""));
    assert!(native_plan_text.contains("\"adenc_rt_arcRetain\""));
    assert!(native_plan_text.contains("\"adenc_rt_dispatchNullReceiverPanic\""));
    assert!(native_plan_text.contains("\"user_field_storage_imports\": \"removed_in_e2_06\""));
    assert!(native_plan_text.contains("\"init_sequence\": [\"startup_entry\", \"adenc_rt_init\", \"app_entry\", \"adenc_rt_shutdown\", \"ExitProcess\"]"));
...
fn lock_d2_04_cli_overrides_take_precedence_over_manifest() {
    ...
    assert!(plan_text.contains("\"runtime_abi_family\": {"));
    assert!(plan_text.contains("\"id\": \"adenc-runtime-abi-v2-embedded-windows-bootstrap\""));
    assert!(plan_text.contains("\"debug_allocator\": \"on\""));
    assert!(plan_text.contains("\"cycle_collector\": \"off\""));
    ...
    assert!(native_plan_text.contains("\"runtime_abi_family\": {"));
    assert!(
        native_plan_text.contains("\"id\": \"adenc-runtime-abi-v2-embedded-windows-bootstrap\"")
    );
    assert!(native_plan_text.contains("\"outputs\": {"));
...
fn lock_e1_16_shared_artifacts_and_publication_layout_are_deterministic() {
    ...
    let release_build = run_adenc(&["build", "--project-root", project.to_str().expect("utf8")]);
    ...
    assert!(release_bin
        .join(format!("adencore-{version}.dll"))
        .exists());
    assert!(release_bin
        .join(format!("adencore-{version}.lib"))
        .exists());
    ...
    assert!(release_launch.contains("schema=adenc.shared.launch.v1"));
    assert!(release_launch.contains("runtime_abi_schema=adenc.runtime.abi.v1"));
    assert!(release_launch.contains("runtime_abi_version=2"));
    assert!(release_launch.contains("object_model_abi_schema=adenc.object_model.abi.v1"));
    ...
    assert!(release_launch.contains("required_runtime_imports=adenc_rt_init"));
    assert!(release_launch.contains("runtime_library=adencore-"));
    assert!(release_launch.contains("runtime_import_library=adencore-"));

    let debug_build = run_adenc(&[
        "build",
        "--project-root",
        project.to_str().expect("utf8"),
        "--profile",
    ...
    assert!(debug_bin.join(format!("adencore-{version}.dll")).exists());
    assert!(debug_bin.join(format!("adencore-{version}.lib")).exists());
    for file in [
        ...
        "shared_artifact_lock-0.1.0-adenc.ir.txt",
        "stamp.txt",
    ] {
        assert!(
...
fn lock_e3_08_fat_and_shared_artifact_publication_parity_rules_are_explicit() {
    ...
    for file in [
        ...
        "fat_parity_artifacts-0.1.0-adenc.ir.txt",
        "stamp.txt",
    ] {
        assert!(
    ...
    let shared_manifest_path = shared_project.join("adenc.toml");
    let shared_manifest =
    ...
    for file in [
        ...
        "shared_parity_artifacts-0.1.0-adenc.ir.txt",
        "stamp.txt",
    ] {
        assert!(
...
fn lock_d2_04_invalid_target_or_runtime_toggle_is_rejected() {
    ...
    assert!(
        stderr.contains(
            "[build].target must be one of: windows-x64, adenos-x64, linux-x64, macos-arm64"
...
fn lock_e3_13_supported_fat_shared_differences_and_shared_only_diagnostics_are_snapshot_locked() {
    ...
    assert!(launch.contains("schema=adenc.shared.launch.v1"));
    ...
    assert!(launch.contains("runtime_library=adencore-"));

    let missing_payload_project_name = "shared_only_diag_snapshot";
    let diag_new = run_adenc(&[
        "new",
    ...
    let diag_manifest_path = diag_project.join("adenc.toml");
    let diag_manifest =
    ...
    assert!(launch_text.contains("required_runtime_imports=adenc_rt_init"));
}
...
fn create_workspace_fixture() -> (PathBuf, PathBuf, PathBuf, PathBuf) {
    ...
    fs::write(workspace_root.join("adenc.toml"), workspace_manifest)
