...
#[test]
fn lock_aden_feature_01_implicit_prelude_contract_resolves_without_imports() {
    let src = r#"
        package app.core;
...
#[test]
fn lock_aden_feature_02_adklib_never_enters_implicit_prelude() {
    let src = r#"
        package app.core;
    ...
    let err = check(src).expect_err("adklib should never resolve through the implicit prelude");
    assert!(err.to_string().contains("Unknown type 'ProjectLayout'"));
}

#[test]
fn lock_aden_feature_03_adklib_opt_in_resolves_project_models() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
        &root.join("adenc.toml"),
        r#"
[package]
name = "author-models"
version = "0.1.0"
...
"#,
    ...
    write_file(
        ...
        import adk.project.ManifestPackage;
        import adk.project.AdklibConfig;
        import adk.project.CheckInvocation;
        ...
        import adk.project.ProjectLayoutResolver;
        import adk.project.ProjectManifest;
        import adk.project.ProjectManifestParser;
        import adk.project.ProjectSources;
        ...
        import adk.toolchain.ToolchainResolver;
        ...
        class Main {
            public static void main() {
                ...
                AdklibConfig adklib = new AdklibConfig(true);
                ProjectManifest manifest = new ProjectManifest(pkg, sources, build, workspace, adklib);
                ...
                String layoutAdenRoot = Path.resolve(publicationRoot, "main-aden");
                String layoutAdenFile = Path.resolve(layoutAdenRoot, "app/core/Main.aden");
                ...
                HostFiles.writeAllText(layoutAdenFile, "aden");
                ...
                BuildPublicationPlan publicationPlan = BuildPublicationWriter.resolvePublicationPlan(
                    ...
                    "workspace/demo/build/tmp/backend/debug/adencore.dll",
                    "workspace/demo/build/tmp/backend/debug/adencore.lib",
                    true,
                    true
                );
                ...
                String checkOkText = CheckSummaryWriter.renderCheckResult(
                    CheckResult.success(
                        ...
                        "friendly",
                        "manifest",
                        "workspace/demo/src/main/aden/app/core/Main.aden",
                        "workspace/demo/src/main/aden"
                    )
                );
                ...
                String workspaceManifestPath = Path.resolve(workspaceRoot, "adenc.toml");
                String workspaceDemoManifestPath = Path.resolve(
                    workspaceRoot,
                    "apps/demo/adenc.toml"
                );
                String workspaceSharedManifestPath = Path.resolve(
                    workspaceRoot,
                    "libs/shared/adenc.toml"
                );
                ArrayList<String> processArgs = new ArrayList<String>();
                processArgs.add("/d");
                processArgs.add("/c");
                processArgs.add("echo aden_toolchain_process_ok");
                ...
                CheckInvocation checkInvocation = ProjectInvocationResolver.resolveCheckInvocation(
                    "workspace/demo/src/main/aden/app/core",
                    null,
                    null
                );
                ...
                String discoveredTestsText = ProjectDiscoveryBridge.toBridgeText(
                    ProjectDiscovery.discoverTestFiles("workspace/demo/src/test/aden")
                );
                ...
                boolean ok = pkg.name().equals("demo")
                    ...
                    && layout.mainAdenRoot().equals("workspace/demo/src/main/aden")
                    && sources.entry().equals("app/main/Main.aden")
                    ...
                    && manifest.adklib().enabled()
                    && manifest.adklibEnabled()
                    ...
                    && parsedManifest.sources().entry().equals("app/core/Main.aden")
                    ...
                    && publicationPlan.sharedProfileLayout()
                    && publicationPlan.publishedIrPath()
                        .equals("workspace/demo/build/distro/debug/metadata/demo-1.2.3-adenc.ir.txt")
                    && publicationPlan.publishedNativePlanPath()
                    ...
                    && publicationPlan.publishedRuntimeLibraryPath()
                        .equals("workspace/demo/build/distro/debug/bin/adencore.dll")
                    && publicationPlan.publishedRuntimeImportLibraryPath()
                        .equals("workspace/demo/build/distro/debug/bin/adencore.lib")
                    && publicationPlan.sharedLaunchMetadataPath()
                    ...
                    && HostFiles.readAllText(Path.resolve(layoutAssetsRoot, "data.txt"))
                        .equals("resource")
                    && HostFiles.readAllText(Path.resolve(layoutSanityRoot, "src_main_aden/app/core/Main.aden"))
                        .equals("aden")
                    ...
                    && checkFailText.equals(
                        "Check FAILED: mode=strict entry=workspace/demo/src/main/aden/app/core/Main.aden source_root=workspace/demo/src/main/aden"
                    )
                    ...
                    && testDiscoveryText.equals(
                        "Test discovery: project_mode=manifest project_root=workspace/demo tests_root=workspace/demo/tests source_root=workspace/demo/src/main/aden count=2"
                    )
                    ...
                    && workspaceMemberDiscoveryText.equals(
                        "Member test discovery: member=workspace/demo tests_root=workspace/demo/tests source_root=workspace/demo/src/main/aden count=1"
                    )
                    ...
                    && testNoFilesText.equals(
                        "Test discovery failed: no .aden tests found under 'workspace/demo/tests'"
                    )
                    && workspaceMemberNoTestsText.equals(
                        "[FAIL] workspace/demo :: no .aden tests found under 'workspace/demo/tests'"
                    )
                    ...
                    && BuildPublishedArtifactBridge.toBridgeText(publishedArtifact)
                        .contains("runtimeImportLibraryPath=1:workspace/demo/build/distro/debug/bin/adencore.lib")
                    ...
                    && finalizationText.contains(
                        "runtimeImportLibraryPath=1:workspace/demo/build/distro/debug/bin/adencore.lib"
                    )
                    ...
                    && processResult.stdoutText().contains("aden_toolchain_process_ok")
                    ...
                    && !checkInvocation.adklibEnabled()
                    && checkInvocation.entryPath().equals("workspace/demo/src/main/aden/app/core/Main.aden")
                    && checkInvocationText.contains("entryPath=1:workspace/demo/src/main/aden/app/core/Main.aden")
                    && checkInvocationText.contains("usedManifest=1:true")
                    && testInvocation.projectRoot().equals("workspace/demo")
                    && testInvocation.testsRoot().equals("workspace/demo/src/test/aden")
                    && testInvocationText.contains("projectRoot=1:workspace/demo")
                    && testInvocationText.contains("testsRoot=1:workspace/demo/src/test/aden")
                    ...
                    && resolver.mainAdenRoot(manifest).equals("workspace/demo/src/main/aden")
                    && resolver.testAdenRoot(manifest).equals("workspace/demo/src/test/aden")
                    && resolver.mainResourcesRoot(manifest).equals("workspace/demo/src/main/resources")
                    && resolver.entryPath(manifest).equals("workspace/demo/src/main/aden/app/main/Main.aden")
    ...
    assert!(
        ...
        "expected adklib-enabled check success\nstdout:\n{}\nstderr:\n{}",
...
#[test]
fn lock_aden_feature_04_adklib_project_models_require_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
    ...
    assert!(
        !output.status.success(),
        "expected adklib-gated check failure"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("Import 'adk.*' requires [adklib].enabled = true in adenc.toml"),
        "expected adklib gating error\nstderr:\n{}",
        stderr
    );
}

#[test]
fn lock_aden_feature_05_public_process_api_resolves_without_adklib_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
    ...
    assert!(
        output.status.success(),
        "expected public process api check success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
#[test]
fn lock_aden_feature_06_public_host_files_copy_executes_without_adklib_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
        ...
        class Main {
            public static void main() {
                ...
                if (copiedSingle && copiedTree
                    ...
                    aden.lang.IO.println("host_copy_ok");
                    return;
                }

                aden.lang.IO.println("host_copy_broken");
            }
        }
    "#,
    );
    ...
    assert!(
        ...
        "expected public host files build success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
...
#[test]
fn lock_aden_feature_07_public_interop_api_executes_without_adklib_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
    ...
    write_file(
        ...
        import aden.interop.NativeCalls;
        import aden.interop.NativeLibrary;
        import aden.interop.NativeSymbol;
        ...
        class Main {
            public static void main() {
                ...
                if (pid > 0 && ticks > 0L && closed) {
                    aden.lang.IO.println("interop_ok");
                    return;
                }

                aden.lang.IO.println("interop_broken");
            }
        }
    "#,
    );
    ...
    assert!(
        build.status.success(),
        "expected public interop build success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
...
#[test]
fn lock_aden_feature_08_public_interop_memory_and_utf8_surface_executes_without_adklib_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
    ...
    write_file(
        ...
        import aden.interop.NativeBuffer;
        import aden.interop.NativeByteSpan;
        import aden.interop.NativeCalls;
        import aden.interop.NativeLibrary;
        import aden.interop.NativePointer;
        import aden.interop.NativeSymbol;
        import aden.interop.NativeUtf8String;
        ...
        class Main {
            public static void main() {
                ...
                if (encodedLength == text.byteLength()
                    ...
                    && marker == 123456789L
                    && closed) {
                    aden.lang.IO.println("interop_memory_ok");
                    return;
                }

                aden.lang.IO.println("interop_memory_broken");
            }
        }
    "#,
    );
    ...
    assert!(
        build.status.success(),
        "expected public interop memory build success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
...
#[test]
fn lock_aden_feature_09_public_interop_ownership_modes_execute_without_adklib_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
    ...
    write_file(
        ...
        import aden.interop.NativeBuffer;
        import aden.interop.NativeLibrary;
        import aden.interop.NativeOwnership;
        import aden.interop.NativeUtf8String;
        import aden.io.ResourceScope;
        ...
        class Main {
            public static void main() {
                ...
                if (ok && closed) {
                    aden.lang.IO.println("interop_ownership_ok");
                    return;
                }

                aden.lang.IO.println("interop_ownership_broken");
            }
        }
    "#,
    );
    ...
    assert!(
        build.status.success(),
        "expected public interop ownership build success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
...
#[test]
fn lock_aden_feature_10_public_interop_argument_and_loader_helpers_execute_without_adklib_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
    ...
    write_file(
        ...
        import aden.interop.NativeArgument;
        import aden.interop.NativeBuffer;
        import aden.interop.NativeCalls;
        import aden.interop.NativeLibrary;
        import aden.interop.NativeOwnership;
        ...
        class Main {
            public static void main() {
                ...
                if (ok) {
                    aden.lang.IO.println("interop_args_ok");
                    return;
                }

                aden.lang.IO.println("interop_args_broken");
            }
        }
    "#,
    );
    ...
    assert!(
        build.status.success(),
        "expected public interop argument/loader helper build success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
...
#[test]
fn lock_aden_feature_11_public_interop_function_pointer_surface_executes_without_adklib_opt_in(
) {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
...
"#,
    ...
    write_file(
        ...
        import aden.interop.NativeBuffer;
        import aden.interop.NativeCalls;
        import aden.interop.NativeFunction;
        import aden.interop.NativeLibrary;
        import aden.interop.NativeOwnership;
        import aden.interop.NativePointer;
        import aden.interop.NativeUtf8String;
        ...
        class Main {
            public static void main() {
                ...
                if (ok) {
                    aden.lang.IO.println("interop_function_pointer_ok");
                    return;
                }

                aden.lang.IO.println("interop_function_pointer_broken");
            }
        }
    "#,
    );
    ...
    assert!(
        build.status.success(),
        "expected public interop function-pointer build success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
...
#[test]
fn lock_aden_feature_12_public_interop_callbacks_execute_without_adklib_opt_in() {
    let root = unique_temp_root();
    let src_root = root.join("src").join("main").join("aden");
    let entry = src_root.join("app/core/Main.aden");
    write_file(
...
"#,
    ...
    write_file(
        ...
        import aden.interop.NativeArgument;
        import aden.interop.NativeBuffer;
        import aden.interop.NativeCallback2;
        import aden.interop.NativeCallbackHandle;
        import aden.interop.NativeCalls;
        import aden.interop.NativeLibrary;
        import aden.interop.NativePointer;
        ...
        class Main {
            public static void main() {
                ...
                if (first == 1L
                    ...
                    && closedComparator
                    && closedLibrary) {
                    aden.lang.IO.println("interop_callback_ok");
                    return;
                }

                aden.lang.IO.println("interop_callback_broken");
            }
        }
    "#,
    );
    ...
    assert!(
        build.status.success(),
        "expected public interop callback build success without adklib opt-in\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
