...
fn cli_check_accepts_real_multifile_project() {
    ...
    let player = src_root.join("app/game/Player.aden");
    let main = src_root.join("app/game/Main.aden");
...
fn cli_check_importing_one_class_does_not_eager_load_broken_sibling_files() {
    let root = tempdir().expect("tempdir");
    let src_root = root.path().join("src");
    let good = src_root.join("app/lib/Good.aden");
    let broken = src_root.join("app/lib/Broken.aden");
    let main = src_root.join("app/core/Main.aden");
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                aden.lang.IO.println(Good.value());
            }
        }
        "#,
    );
...
fn cli_check_same_package_resolution_is_demand_driven_not_whole_package() {
    ...
    let src_root = root.path().join("src");
    let player = src_root.join("app/game/Player.aden");
    let broken = src_root.join("app/game/Broken.aden");
    let main = src_root.join("app/game/Main.aden");
