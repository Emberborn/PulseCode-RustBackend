...
fn cli_check_accepts_battle_sim_fixture_project() {
    ...
    let entry = src_root.join("app/runtime/Main.aden");

    let output = common::adenc_command()
        .arg("check")
...
fn cli_check_accepts_strict_stress_soak_fixture_project() {
    ...
    let src_root = root.join("src");
    let entry = src_root.join("strict_stress_soak/Main.aden");
...
fn cli_build_executes_runtime_mix_fixture_project_in_shared_mode() {
    ...
    assert!(
        report.contains("startup_bootstrap=mainCRTStartup->adenc_rt_init->app_entry->adenc_rt_shutdown->ExitProcess"),
    ...
    assert!(
        report.contains("adenc_aden_lang_IO_println__String"),
        "shared build must include exported IO stdlib methods in inventory:\n{}",
        report
    );

    let version = env!("CARGO_PKG_VERSION");
    let runtime_library = root.join("build").join(format!("adencore-{version}.dll"));
    let runtime_import_library = root.join("build").join(format!("adencore-{version}.lib"));
    assert!(
...
fn cli_build_executes_strict_stress_soak_fixture_project_in_shared_mode() {
    ...
    let output = common::adenc_command()
        ...
        .arg("shared")
        .output()
        .expect("run adenc build shared strict_stress_soak");
    ...
    assert!(
        startup_asm.find("call adenc_rt_init").unwrap()
            < startup_asm.find("call adenc_rt_shutdown").unwrap(),
...
fn cli_build_executes_stress_soak_repeatedly() {
    ...
    copy_dir_recursive(&fixture.join("src"), &root.join("src"));
    let src_root = root.join("src");
    let entry = src_root.join("app/stress/Main.aden");
    let exe = root.join("build").join("main.exe");
    let iterations = env::var("PULSEC_SOAK_ITERS")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
...
fn cli_build_emits_ir_summary_for_fixture_project() {
    ...
    let artifact_path = root.join("build").join("adenc.ir.txt");
    assert!(
    ...
    assert!(artifact.contains("# Aden Lang IR Dump"));
...
fn cli_build_accepts_runtime_string_intrinsics_baseline() {
    ...
    let plan = fs::read_to_string(&plan_path).expect("read native plan");
    assert!(
        plan.contains("aden.lang.String.runtimeConcat"),
        "missing String.runtimeConcat runtime symbol in plan:\n{}",
        plan
    );
    assert!(
        plan.contains("aden.lang.String.runtimeLength"),
...
fn cli_build_executes_object_interface_mix_fixture_project_in_shared_mode() {
    ...
    assert!(
        !app_asm.contains("extrn adenc_rt_obj_class_ids:qword"),
...
fn cli_build_shared_runtime_abi_mismatch_fails_deterministically_at_launch() {
    ...
    let output = common::adenc_command()
        ...
        .arg("shared")
        .output()
        .expect("run adenc build shared with runtime ABI mismatch");
...
fn cli_build_shared_object_model_abi_mismatch_fails_deterministically_at_launch() {
    ...
    let output = common::adenc_command()
        ...
        .arg("shared")
        .output()
        .expect("run adenc build shared with object-model ABI mismatch");
...
fn cli_build_shared_launch_metadata_detects_missing_runtime_library_payload() {
    ...
    let new_output = common::adenc_command()
        .arg("new")
        ...
        .output()
        .expect("run adenc new");
    assert!(
        new_output.status.success(),
        "shared app scaffold should succeed"
    );

    let main_root = project.join("src").join("main").join("aden");
    ...
    let output = common::adenc_command()
        ...
        .output()
        .expect("run adenc build shared project");
    ...
    assert!(launch.contains("required_runtime_imports=adenc_rt_init"));
    ...
    assert!(
        launch.contains(&format!("runtime_library=adencore-{version}.dll")),
...
fn cli_build_executes_runtime_panic_intrinsic_baseline() {
    ...
    let plan = fs::read_to_string(&plan_path).expect("read native plan");
    assert!(
        plan.contains("aden.rt.Intrinsics.panic"),
...
fn cli_build_executes_cross_method_uncaught_exception_stack_trace_flow() {
    ...
    assert!(
        out.contains("at app.core.Worker.explode(Main.aden:7)"),
        "missing throw site in stack trace:\n{}",
        out
    );
    assert!(
        out.contains("at app.core.Main.relay(Main.aden:13)"),
        "missing relay frame in stack trace:\n{}",
        out
    );
    assert!(
        out.contains("at app.core.Main.main(Main.aden:17)"),
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                ...
                String[] names = new String[] { "aden", "code" };
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                ...
                map.put("name", "aden");
                map.putInt("hp", 7);
                IO.println(map.containsValue("aden"));
...
fn cli_build_executes_hashmap_hashset_default_and_bulk_query_flow() {
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                Map map = new HashMap();
                IO.println(map.putIfAbsent("name", "aden") == null);
          ...
          package app.core;
            ...
            class Main {
                public static void main() {
                  ...
                  String repeated = "aden aden code";
                  String replaced = repeated.replace("aden", "beam");
                  ...
                  IO.println(repeated.indexOf("aden", 1));
                  IO.println(text.lastIndexOf("e"));
                  IO.println(repeated.lastIndexOf('p', 5));
                  IO.println(repeated.lastIndexOf("aden", 10));
                  IO.println(replaced);
                  ...
                  IO.println(trimmedPrefix);
                    ...
                    IO.println(textParts[0].equals(""));
                      ...
                      IO.println("aden".contentEquals((CharSequence) "aden"));
                      IO.println("Aden Lang".regionMatches(5, "coder", 0, 3));
                      IO.println("Aden Lang".regionMatches(true, 0, "aden", 0, 5));
                      IO.println(" \t\n".isBlank());
                    IO.println("  aden  ".strip());
                    IO.println("  aden".stripLeading());
                  IO.println("aden  ".stripTrailing());
                    IO.println("aden".hashCode());
          ...
          class Main {
              public static void main() {
                  ...
                  sb.append((ubyte) 200);
                    ...
                    IO.println(new StringBuilder("aden").contentEquals((CharSequence) "aden"));
...
fn cli_build_executes_cross_package_protected_subclass_flow() {
    let root = unique_temp_root();
    let src_root = root.join("src");
    let entry = src_root.join("app/main/Main.aden");

    write_file(
        &src_root.join("app/base/Base.aden"),
        r#"
        package app.base;
    ...
    write_file(
        &src_root.join("app/derived/Derived.aden"),
        r#"
        package app.derived;
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                ...
                String[][] words = new String[][] { { "aden" }, null };
...
fn unique_temp_root() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    std::env::temp_dir().join(format!("adenc_cli_fixture_{nanos}"))
}
...
fn cli_build_executes_strict_stress_soak_with_repeated_fat_shared_parity() {
    ...
    let entry_rel = "strict_stress_soak/Main.aden";
    let iterations = env::var("PULSEC_SOAK_ITERS")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(2);
...
fn cli_build_shared_boundary_failure_diagnostics_remain_explicit_and_deterministic() {
    ...
    let runtime_abi_output = common::adenc_command()
        .env("PULSEC_FORCE_RUNTIME_ABI_VERSION", "1")
        .arg("build")
        .arg(entry.to_str().expect("entry utf8"))
        .arg("--source-root")
    ...
    let object_model_output = common::adenc_command()
        .env("PULSEC_FORCE_OBJECT_MODEL_ABI_VERSION", "2")
        .arg("build")
        .arg(entry.to_str().expect("entry utf8"))
        .arg("--source-root")
    ...
    let new_output = common::adenc_command()
        ...
        .output()
        .expect("run adenc new for shared missing payload project");
    assert!(
        new_output.status.success(),
        "shared app scaffold should succeed"
    );
    ...
    let launch_build = common::adenc_command()
        .arg("build")
        .arg("--project-root")
        .arg(project.to_str().expect("project utf8"))
        .output()
...
fn cli_build_preserves_package_object_layout_for_multifile_project() {
    ...
    let src_root = root.join("src");
    let entry = src_root.join("app/core/Main.aden");
    let helper = src_root.join("app/core/Helper.aden");
...
fn cli_build_executes_cross_file_static_call_with_args_and_return() {
    ...
    let src_root = root.join("src");
    let entry = src_root.join("app/core/Main.aden");
    let helper = src_root.join("app/core/MathOps.aden");
...
fn cli_build_executes_cross_file_instance_call_baseline() {
    ...
    let src_root = root.join("src");
    let entry = src_root.join("app/core/Main.aden");
    let helper = src_root.join("app/core/Greeter.aden");
...
fn cli_build_executes_constructor_and_instance_field_baseline() {
    ...
    let src_root = root.join("src");
    let entry = src_root.join("app/core/Main.aden");
    let counter = src_root.join("app/core/Counter.aden");
...
fn cli_build_executes_constructor_and_instance_args_baseline() {
    ...
    let src_root = root.join("src");
    let entry = src_root.join("app/core/Main.aden");
    let worker = src_root.join("app/core/Worker.aden");
...
fn cli_build_executes_foundational_lang_interface_flow() {
    let source = r#"
        ...
        import aden.lang.Char;
        import aden.lang.IO;
        import aden.lang.Runnable;
        import aden.lang.Appendable;
        import aden.lang.CharSequence;
    ...
    let source = r#"
        ...
        class Main {
            public static void main() {
                ...
                words.add("code");
                words.add("aden");
                IO.println(words.lastIndexOf("aden"));
    ...
    write_file(
        ...
        class Main {
            ...
            public static void main() {
                ...
                IO.println(aden.lang.Boolean.FALSE);
...
fn cli_build_executes_static_import_math_path() {
    ...
    write_file(
        ...
        package app.core;
        import aden.lang.IO;
        import static aden.math.Math.max;
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                ...
                IO.println(Boolean.toString(value.equals(Optional.of("aden"))));
...
#[test]
fn cli_build_executes_adk_project_discovery_baseline() {
    let root = unique_temp_root();
...
"#,
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                File.writeAllText(
                    "workspace/adenc.toml",
                    "[workspace]\n"
                    + "members = \"apps/demo, libs/shared\"\n"
                );
                File.writeAllText(
                    "workspace/apps/demo/adenc.toml",
                    "[package]\n"
                    + "name = \"demo\"\n"
                    + "version = \"1.2.3\"\n"
                    + "\n"
                ...
                File.writeAllText(
                    "workspace/libs/shared/adenc.toml",
                    "[package]\n"
                    + "name = \"shared\"\n"
                    + "version = \"0.1.0\"\n"
                );
                File.writeAllText(
                    "workspace/apps/demo/src/main/aden/app/main/Main.aden",
                    "package app.main;\nclass Main { public static void main() {} }\n"
                );
                File.writeAllText(
                    "workspace/apps/demo/src/test/aden/app/tests/ZetaTest.aden",
                    "package app.tests;\nclass ZetaTest {}\n"
                );
                File.writeAllText(
                    "workspace/apps/demo/src/test/aden/app/tests/AlphaTest.aden",
                    "package app.tests;\nclass AlphaTest {}\n"
                );

                String manifestPath = ProjectDiscovery.findManifestPath(
                    "workspace/apps/demo/src/main/aden/app/main"
                );
                String projectRoot = ProjectDiscovery.findProjectRoot(
                    "workspace/apps/demo/src/main/aden/app/main"
                );
                ProjectManifest demo = ProjectDiscovery.loadManifest("workspace/apps/demo/adenc.toml");
                ProjectManifest found = ProjectDiscovery.findAndLoadManifest(
                    "workspace/apps/demo/src/main/aden/app/main"
                );
                ProjectManifest workspace = ProjectDiscovery.loadManifest("workspace/adenc.toml");
                ...
                CheckInvocation checkFromDirect = ProjectInvocationResolver.resolveCheckInvocation(
                    "workspace/apps/demo",
                    "workspace/apps/demo/src/main/aden",
                    null
                );
                ...
                BuildInvocation buildFromDirect = BuildInvocationResolver.resolveBuildInvocation(
                    "direct-project",
                    null,
                    "direct-project/src/main/aden",
                    "direct-project/src/main/aden/app/main/Main.aden",
                    "debug",
                    null,
                    null,
                    null,
                ...
                String entryPath = ProjectDiscovery.discoverEntryPath(demo.sources().mainAdenRoot());
                List<String> tests = ProjectDiscovery.discoverTestFiles(demo.sources().testAdenRoot());
                ...
                IO.println(Boolean.toString(checkFromManifest.adklibEnabled()));
...
fn cli_build_inputstream_use_after_close_panics_deterministically() {
    ...
    write_file(
        ...
        class Main {
            public static void main() {
                File.writeAllText("closed.txt", "aden");
