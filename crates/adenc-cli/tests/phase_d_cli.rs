...
fn install_runtime_mix_fixture(project: &Path) {
    ...
    let test_root = project.join("src").join("test").join("aden");
    ...
    replace_manifest_entry(project, "app/runtime/Main.aden");
    let smoke = test_root
        .join("app")
        .join("runtime")
        .join("LifecycleSmokeTest.aden");
...
fn install_object_interface_mix_fixture(project: &Path) {
    ...
    replace_manifest_entry(project, "app/mixed/Main.aden");
    let smoke = test_root
        .join("app")
        .join("mixed")
        .join("LifecycleSmokeTest.aden");
...
fn d5_02_cli_project_lifecycle_runs_end_to_end_on_real_fixture_project() {
    ...
    assert!(
        !asm_dir.join("adenc.ir.txt").exists(),
    ...
    assert!(
        !release_dir
            .join("runtime_lifecycle-0.1.0-adenc.ir.txt")
            .exists(),
...
fn e1_11_shared_profile_build_publishes_bin_lib_and_metadata_layout() {
    ...
    assert!(release_launch_text.contains("schema=adenc.shared.launch.v1"));
    ...
    assert!(release_launch_text.contains("runtime_abi_schema=adenc.runtime.abi.v1"));
    assert!(release_launch_text.contains("runtime_abi_version=2"));
    assert!(release_launch_text.contains("object_model_abi_schema=adenc.object_model.abi.v1"));
    ...
    assert!(release_launch_text.contains("required_runtime_imports=adenc_rt_init"));
    assert!(release_launch_text.contains("runtime_library=adencore-"));
    assert!(release_launch_text.contains(".dll"));
    assert!(release_launch_text.contains("runtime_import_library=adencore-"));
    ...
    assert!(
        debug_bin.join(format!("adencore-{version}.dll")).exists(),
        "missing shared debug runtime library"
    );
    assert!(
        debug_bin.join(format!("adencore-{version}.lib")).exists(),
    ...
    for file in [
        ...
        "shared_layout-0.1.0-adenc.ir.txt",
        "stamp.txt",
    ] {
        assert!(
    ...
    let debug_launch_text = norm(&fs::read(&debug_launch).expect("read shared debug launch"));
    assert!(debug_launch_text.contains("schema=adenc.shared.launch.v1"));
    assert!(debug_launch_text.contains("runtime_library=adencore-"));
    assert!(debug_launch_text.contains("runtime_import_library=adencore-"));
...
fn e1_13_shared_profile_debug_release_publication_rules_are_locked() {
    ...
    for file in [
        ...
        "shared_profile_rules-0.1.0-adenc.ir.txt",
        "stamp.txt",
    ] {
        assert!(
...
fn e3_07_debug_release_behavior_and_developer_visible_outputs_are_locked_for_fat_and_shared() {
    ...
    assert!(!fat_release_dir
        ...
        .exists());
    assert!(!fat_release_dir
        .join("fat_profile_parity-0.1.0-adenc.ir.txt")
        .exists());
    ...
    for file in [
        ...
        "fat_profile_parity-0.1.0-adenc.ir.txt",
        "stamp.txt",
    ] {
        assert!(
    ...
    for file in [
        ...
        "shared_profile_parity-0.1.0-adenc.ir.txt",
        "stamp.txt",
    ] {
        assert!(
...
fn d5_02_cli_workspace_lifecycle_runs_on_real_fixture_members() {
    ...
    fs::write(
        workspace_root.join("adenc.toml"),
