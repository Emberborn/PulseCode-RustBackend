...
fn lock_m3_01_exception_hierarchy_available() {
    ...
    write_file(
        ...
        r#"
        package app.core;
        import aden.lang.Throwable;
        import aden.lang.Exception;
        import aden.lang.RuntimeException;
        class Main {
            public static void main() {
...
fn lock_m3_02_runtime_exception_set_available() {
    ...
    write_file(
        ...
        import aden.lang.IllegalStateException;
        import aden.lang.NullPointerException;
        import aden.lang.IndexOutOfBoundsException;
        import aden.lang.UnsupportedOperationException;
        import aden.lang.NumberFormatException;
        class Main {
            public static void main() {
...
fn lock_m3_14_abi_doc_and_lock_tests_exist() {
    ...
    assert!(abi_doc.contains("adenc_rt_mapGetInt"));
}
...
fn lock_m3_16_stress_soak_fixture_check() {
    ...
    let strict_entry = strict_src.join("strict_stress_soak/Main.aden");
...
fn lock_f1_board_explicitly_tracks_scanner_util_concurrency_network_and_defers_desktop_ui() {
    ...
    assert!(board.contains("aden.util"));
    ...
    assert!(board.contains("runtime cross-method Aden-throw unwind now works"));
    ...
    assert!(roadmap.contains("aden.util"));
...
fn lock_f2_language_docs_tree_is_explicit_and_present() {
    ...
    assert!(stdlib.contains("each doc file corresponds to exactly one `.aden` class"));
...
#[test]
fn lock_self_sustained_hosting_direction_and_adklib_inventory_are_documented() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("..");
    let roadmap = fs::read_to_string(root.join("docs").join("STANDALONE_ROADMAP.md"))
    ...
    let adklib = fs::read_to_string(root.join("docs").join("AUTHORLIB_STRATEGY.md"))
        .expect("read adklib strategy");
    ...
    assert!(roadmap.contains("`adklib` becomes the privileged Aden-owned extension library"));
    assert!(handoff
        .contains("self-sustained hosting is the transition mechanism, not the end-state goal"));
    assert!(handoff.contains("`adklib` is the privileged Aden-owned extension library"));
    ...
    assert!(fb.contains("compiler/runtime-facing `adklib`"));
    assert!(fb.contains("compiler/runtime builds can always access it"));
    assert!(adklib.contains("`stdlib`"));
    assert!(adklib.contains("`adklib`"));
    assert!(adklib.contains("compiler must still be able to resolve `adk.*` at all times"));
}

#[test]
fn lock_current_shipped_stdlib_aden_files_have_multiline_adendoc_blocks() {
    fn has_adendoc_before(lines: &[&str], index: usize) -> bool {
    ...
    fn collect_aden_files(root: &Path, out: &mut Vec<PathBuf>) {
        for entry in fs::read_dir(root).expect("read stdlib dir") {
            let entry = entry.expect("dir entry");
            let path = entry.path();
            if path.is_dir() {
                collect_aden_files(&path, out);
    ...
    collect_aden_files(&root, &mut files);
    for file in files {
        let text = fs::read_to_string(&file).expect("read stdlib aden file");
        let lines: Vec<&str> = text.lines().collect();
        assert!(
            text.contains("/**") && text.contains("*/"),
            "expected AdenDoc block in {}",
            file.display()
        );
        for (index, line) in lines.iter().enumerate() {
            ...
            if is_type_declaration(trimmed) {
                assert!(
                    has_adendoc_before(&lines, index),
                    "expected AdenDoc before type declaration in {}: {}",
                    file.display(),
                    trimmed
                );
            }
            if is_public_method_or_constructor(trimmed) || is_public_field(trimmed) {
                assert!(
                    has_adendoc_before(&lines, index),
                    "expected AdenDoc before public member in {}: {}",
                    file.display(),
                    trimmed
                );
            }
...
fn lock_generated_stdlib_reference_docs_cover_shipped_stdlib_surface() {
    ...
    let stdlib_root = root.join("stdlib").join("src").join("aden");
    let docs_root = root
        .join("docs")
        .join("language")
        .join("stdlib")
    ...
    let mut aden_files = Vec::new();
    let mut doc_files = Vec::new();
    collect_files(&stdlib_root, "aden", &mut aden_files);
    collect_files(&docs_root, "md", &mut doc_files);

    assert!(script.contains("docs\\language\\stdlib"));
    assert!(script.contains("Remove-Item -Recurse -Force $docsAdenRoot"));
    assert!(script.contains("## Members"));
    assert!(stdlib_readme.contains("overwrites the generated `docs/language/stdlib/aden/**` tree"));
    assert_eq!(
        aden_files.len(),
        doc_files.len(),
        "expected generated stdlib docs to cover each shipped stdlib aden file"
    );
    assert!(sample.contains("# aden.lang.String"));
    assert!(sample.contains("Source: ``stdlib/src/aden/lang/String.aden``"));
...
fn lock_f1_01_to_f1_03_gap_policy_and_dependency_docs_are_present_and_truthful() {
    ...
    assert!(policy.contains("Aden-Specific Intentional Differences"));
...
fn lock_f1_10_enum_baseline_is_documented_and_backend_backed() {
    ...
    assert!(board.contains("aden.lang.Enum"));
}
...
fn lock_f1_10_cli_build_executes_enum_baseline_honestly() {
    let root = unique_temp_root();
    let src_root = root.join("src");
    let entry = src_root.join("app/types/Main.aden");
    write_file(
        &entry,
        r#"
        package app.types;
...
fn lock_f1_31_cli_build_executes_instantiated_generic_members_with_runtime_erasure() {
    let root = unique_temp_root();
    let src_root = root.join("src");
    let entry = src_root.join("app/generics/Main.aden");
    write_file(
        &entry,
        r#"
...
fn lock_rb_03_target_taxonomy_and_handoff_resume_point_are_documented() {
    ...
    assert!(taxonomy.contains("`adenos-x64`"));
    ...
    assert!(taxonomy.contains("`adenos-x64-first-slice`"));
    ...
    assert!(contract.contains("`adenos-x64`"));
...
fn lock_rb_04_cli_target_surface_is_documented() {
    ...
    assert!(cli_contract.contains("adenos-x64"));
    ...
    assert!(cli_contract.contains("`--target <windows-x64|adenos-x64|linux-x64|macos-arm64>`"));
    assert!(cli_guide.contains("Rebase note (`RB-05`)"));
    assert!(cli_guide.contains("windows-x64"));
    assert!(cli_guide.contains("adenos-x64"));
    ...
    assert!(cli_guide.contains("`adenc build [<entry.aden>] [--project-root <dir>] [--source-root <dir>] [--profile <debug|release>] [--target <windows-x64|adenos-x64|linux-x64|macos-arm64>]"));
}
...
fn lock_rb_05_manifest_target_surface_and_handoff_resume_point_are_documented() {
    ...
    assert!(manifest_examples.contains("target = \"adenos-x64\""));
...
fn lock_rb_07_backend_layer_split_and_handoff_resume_point_are_documented() {
    ...
    assert!(contract.contains("`crates/adenc-cli/src/backend/adapters/windows_x64/mod.rs`"));
    assert!(roadmap.contains("the rebase board is now closed through `RB-21`"));
    ...
    assert!(architecture.contains("`crates/adenc-cli/src/backend/target_neutral/mod.rs`"));
    assert!(architecture.contains("`crates/adenc-cli/src/backend/host_bootstrap/mod.rs`"));
    assert!(architecture.contains("`crates/adenc-cli/src/backend/adapters/windows_x64/mod.rs`"));
...
fn lock_rb_08_adapter_registry_and_handoff_resume_point_are_documented() {
    ...
    assert!(registry.contains("`crates/adenc-cli/src/backend/host_bootstrap/mod.rs`"));
...
fn lock_rb_17_1_compiler_packaging_removal_is_documented_and_handoffed() {
    ...
    assert!(cli.contains("`adenc package` is not part of the public CLI anymore"));
    ...
    assert!(removal.contains("`adenc package`"));
...
fn lock_rb_12_host_bootstrap_runtime_contract_and_handoff_resume_point_are_documented() {
    ...
    assert!(cli.contains("schema `adenc.host_bootstrap.runtime.v1`"));
    assert!(architecture.contains("minimum retained bootstrap runtime contract"));
    assert!(bootstrap.contains("Schema id:"));
    assert!(bootstrap.contains("`adenc.host_bootstrap.runtime.v1`"));
    ...
    assert!(runtime_contract.contains("adenc.host_bootstrap.runtime.v1"));
...
#[test]
fn lock_rb_13_adenos_runtime_service_contract_and_handoff_resume_point_are_documented() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("..");
    let board = fs::read_to_string(root.join("docs").join("REBASE_TAKS_BOARD.md"))
    ...
    let adenos = fs::read_to_string(
        root.join("docs")
            .join("PULSEOS_RUNTIME_SERVICE_ABI_SLICE.md"),
    )
    .expect("read adenos runtime service abi slice");
    let adapters_mod = fs::read_to_string(
        root.join("crates")
            .join("adenc-cli")
            .join("src")
    ...
    .expect("read adapters mod");
    let adenos_mod = fs::read_to_string(
        root.join("crates")
            .join("adenc-cli")
            .join("src")
            .join("backend")
    ...
    .expect("read adenos adapter contract mod");
    let plan_rendering = fs::read_to_string(
        root.join("crates")
            .join("adenc-cli")
            .join("src")
    ...
    assert!(board.contains("| RB-G4 | Runtime/Intrinsics ownership and AdenOS first-slice service contract are explicit | Done (Locked) |"));
    assert!(handoff.contains("`RB-18`"));
    assert!(handoff.contains("`RB-13` AdenOS runtime-service slice"));
    assert!(contract.contains("AdenOS Runtime Service ABI Slice (`RB-13`)"));
    assert!(roadmap.contains("the rebase board is now closed through `RB-21`"));
    assert!(roadmap.contains("[AdenOS Runtime Service ABI Slice]"));
    assert!(cli.contains("requested_target_runtime_service_abi"));
    assert!(cli.contains("adenc.adenos.runtime_service.v1"));
    assert!(partition.contains("PULSEOS_RUNTIME_SERVICE_ABI_SLICE.md"));
    assert!(adenos.contains("Schema id:"));
    assert!(adenos.contains("`adenc.adenos.runtime_service.v1`"));
    assert!(adenos.contains("## Meeting Milestone Surface"));
    assert!(adenos.contains("## ABI Ground Rules"));
    assert!(adenos.contains("## Plan Surface"));
    assert!(adapters_mod.contains("pub(crate) mod adenos_x64;"));
    assert!(adapters_mod.contains("resolve_requested_target_runtime_service_contract("));
    assert!(adenos_mod.contains("AdenOsRuntimeServiceContract"));
    assert!(adenos_mod.contains("adenc.adenos.runtime_service.v1"));
    assert!(adenos_mod.contains("contract-defined-not-yet-implemented"));
...
fn lock_rb_14_runtime_ownership_rebase_and_handoff_resume_point_are_documented() {
    ...
    .expect("read adenos runtime service slice");
    ...
    assert!(cli.contains("adenc.runtime.ownership.rebase.v1"));
    assert!(architecture.contains("RUNTIME_OWNERSHIP_REBASE.md"));
    assert!(bootstrap.contains("RUNTIME_OWNERSHIP_REBASE.md"));
    assert!(adenos.contains("RUNTIME_OWNERSHIP_REBASE.md"));
    assert!(ownership.contains("Schema id:"));
    assert!(ownership.contains("`adenc.runtime.ownership.rebase.v1`"));
    ...
    assert!(ownership.contains("`aden.lang.*`"));
    ...
    assert!(ownership_model.contains("adenc.runtime.ownership.rebase.v1"));
...
fn lock_rb_15_target_artifact_contract_split_and_handoff_resume_point_are_documented() {
    ...
    .expect("read adenos artifact mod");
    let plan_rendering = fs::read_to_string(
        root.join("crates")
            .join("adenc-cli")
            .join("src")
    ...
    assert!(cli.contains("adenc.windows_x64.artifact_contract.v1"));
    assert!(cli.contains("adenc.adenos.artifact_contract.v1"));
    ...
    assert!(artifact.contains("`adenc.windows_x64.artifact_contract.v1`"));
    assert!(artifact.contains("`adenc.adenos.artifact_contract.v1`"));
    assert!(artifact.contains("## Windows x64 Adapter Artifact Contract"));
    assert!(artifact.contains("## AdenOS Requested-Target Artifact Contract"));
    ...
    assert!(windows_mod.contains("adenc.windows_x64.artifact_contract.v1"));
    assert!(adenos_mod.contains("adenos_x64_artifact_contract("));
    assert!(adenos_mod.contains("adenc.adenos.artifact_contract.v1"));
    assert!(adenos_mod.contains("first-slice-loader-layout-published"));
    assert!(adenos_mod.contains("shared-request-collapses-to-first-slice-single-image-layout"));
...
#[test]
fn lock_rb_16_adenos_startup_loader_publication_contract_and_handoff_resume_point_are_documented()
{
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("..");
    ...
    .expect("read adenos startup loader publication contract");
    let adapters_mod = fs::read_to_string(
        root.join("crates")
            .join("adenc-cli")
            .join("src")
    ...
    .expect("read adenos adapter mod");
    let plan_rendering = fs::read_to_string(
        root.join("crates")
            .join("adenc-cli")
            .join("src")
    ...
    assert!(board.contains("Artifact/startup/loader contracts are explicit for the Windows x64 lane and the AdenOS lane"));
    assert!(board.contains("Done (Locked)"));
    assert!(handoff.contains("`RB-18`"));
    assert!(handoff.contains("`RB-16` AdenOS startup/loader/publication contract"));
    assert!(contract.contains("AdenOS Startup, Loader, and Publication Contract (`RB-16`)"));
    assert!(roadmap.contains("the rebase board is now closed through `RB-21`"));
    assert!(roadmap.contains("[AdenOS Startup, Loader, and Publication Contract]"));
    assert!(cli.contains("startup_loader_contracts"));
    assert!(cli.contains("adenc.windows_x64.startup_loader_contract.v1"));
    assert!(cli.contains("adenc.adenos.startup_loader_contract.v1"));
    assert!(runtime_service.contains("PULSEOS_STARTUP_LOADER_PUBLICATION_CONTRACT.md"));
    assert!(artifact.contains("PULSEOS_STARTUP_LOADER_PUBLICATION_CONTRACT.md"));
    assert!(startup.contains("Schema ids:"));
    assert!(startup.contains("`adenc.windows_x64.startup_loader_contract.v1`"));
    assert!(startup.contains("`adenc.adenos.startup_loader_contract.v1`"));
    assert!(startup.contains("## Windows x64 Active Adapter Startup/Loader Contract"));
    assert!(startup.contains("## AdenOS Requested-Target Startup/Loader Contract"));
    ...
    assert!(startup.contains("`adenos_start`"));
    ...
    assert!(windows_mod.contains("adenc.windows_x64.startup_loader_contract.v1"));
    assert!(adenos_mod.contains("adenos_x64_startup_loader_contract("));
    assert!(adenos_mod.contains("adenc.adenos.startup_loader_contract.v1"));
    assert!(adenos_mod.contains("adenos_start"));
    assert!(adenos_mod.contains("bin/launch.json"));
...
#[test]
fn lock_rb_17_adenos_loading_proof_target_and_handoff_resume_point_are_documented() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("..");
    let board = fs::read_to_string(root.join("docs").join("REBASE_TAKS_BOARD.md"))
    ...
    .expect("read adenos startup contract");
    let proof = fs::read_to_string(
        root.join("docs")
            .join("PULSEOS_EXECUTABLE_LOADING_PROOF_TARGET.md"),
    )
    .expect("read adenos loading proof target");
    let adapters_mod = fs::read_to_string(
        root.join("crates")
            .join("adenc-cli")
            .join("src")
    ...
    assert!(handoff.contains("`RB-18`"));
    assert!(handoff.contains("`RB-17` AdenOS executable-loading proof target"));
    assert!(contract.contains("AdenOS Executable-Loading Proof Target (`RB-17`)"));
    assert!(roadmap.contains("the rebase board is now closed through `RB-21`"));
    assert!(roadmap.contains("[AdenOS Executable-Loading Proof Target]"));
    assert!(cli.contains("requested_target_executable_loading_proof_target"));
    assert!(cli.contains("adenc.adenos.loading_proof_target.v1"));
    assert!(startup.contains("PULSEOS_EXECUTABLE_LOADING_PROOF_TARGET.md"));
    assert!(proof.contains("Schema id:"));
    assert!(proof.contains("`adenc.adenos.loading_proof_target.v1`"));
    ...
    assert!(proof.contains("`adenos_start`"));
    ...
    assert!(adenos_mod.contains("adenos_x64_executable_loading_proof_target("));
    assert!(adenos_mod.contains("adenc.adenos.loading_proof_target.v1"));
    assert!(adenos_mod.contains("proof-contract-defined-execution-deferred"));
...
fn lock_rb_18_validation_layering_is_documented_and_test_homes_are_labeled() {
    ...
    assert!(parity.contains("It does not prove AdenOS or Linux parity by proxy"));
    ...
    assert!(layering.contains("### AdenOS Requested-Target Contract / Proof Validation"));
    assert!(layering.contains("### Linux Expansion Slot"));
    assert!(layering.contains("fixture_projects.rs"));
    assert!(layering.contains("A passing Windows x64 executable/parity test is never proof of AdenOS or Linux execution."));
    assert!(layering.contains("A passing AdenOS requested-target plan/contract test is never proof of AdenOS execution."));
...
fn lock_rb_19_windows_scope_freeze_policy_and_handoff_resume_point_are_documented() {
    ...
    assert!(
        guide.contains("license to add installer/signing/productization flags back into `adenc`")
    );
    ...
    assert!(policy.contains("restoring `adenc package`"));
...
fn lock_f1_43_wrapper_surface_is_documented_and_board_locked() {
    ...
    assert!(!abi.contains("adenc_rt_fpToString"));
...
fn lock_f1_44_unsigned_policy_is_documented_and_board_locked() {
    ...
    assert!(policy.contains("unsigned primitive and wrapper support"));
    assert!(policy.contains("retained Aden extensions, not part of the Java-close claim"));
    assert!(policy.contains("must not imply that unsigned types came from Java"));
    assert!(compiler_doc.contains("Aden extension unsigned primitives"));
    assert!(compiler_doc.contains("intentional Aden extensions"));
    assert!(compiler_doc.contains("not part of the Java-close compatibility claim"));
    assert!(handoff.contains("`F1-44` unsigned policy"));
    assert!(handoff.contains("retained Aden extensions"));
    ...
    assert!(board.contains("Done (Locked)"));
    assert!(board.contains("retained Aden extensions"));
...
fn lock_f1_45_system_process_surface_is_documented_and_board_locked() {
    ...
    assert!(abi.contains("adenc_rt_consoleErrorWrite"));
    assert!(abi.contains("adenc_rt_currentTimeMillis"));
    assert!(abi.contains("adenc_rt_nanoTime"));
    assert!(abi.contains("adenc_rt_systemExit"));
...
fn lock_f1_09_f1_13_and_f1_14_cli_baseline_is_char_varargs_and_explicit_nested_type_fence() {
    ...
    let nested_entry = src_root.join("app/core/NestedFail.aden");
    write_file(
        &nested_entry,
        r#"
        package app.core;
...
fn lock_f1_12_and_f1_30_annotation_baseline_is_documented_and_compiler_owned() {
    ...
    assert!(board.contains("In Progress"));
    assert!(board.contains("processor/runtime-backed Aden ownership"));
}
...
fn lock_f1_12_and_f1_30_cli_check_enforces_override_notnull_and_test_baseline() {
    let root = unique_temp_root();
    let src_root = root.join("src");
    let ok = src_root.join("app/core/Main.aden");
    write_file(
        &ok,
        r#"
        package app.core;
    ...
    let not_null_fail = src_root.join("app/core/NotNullFail.aden");
    write_file(
        &not_null_fail,
        r#"
        package app.core;
    ...
    let test_fail = src_root.join("app/core/TestFail.aden");
    write_file(
        &test_fail,
        r#"
        package app.core;
...
fn lock_f1_15_throw_baseline_is_documented_and_board_locked() {
    ...
    assert!(compiler_doc.contains("must typecheck as `aden.lang.Throwable`"));
...
fn lock_f1_17_try_with_resources_policy_is_documented_and_fenced() {
    ...
    assert!(compiler_doc.contains("aden.lang.AutoCloseable"));
...
fn lock_f1_18_foreach_baseline_is_documented_and_board_staged() {
    ...
    assert!(doc.contains("aden.lang.Iterable"));
    assert!(doc.contains("aden.collections.Array"));
...
fn lock_f1_19_assert_baseline_is_documented_and_board_locked() {
    ...
    assert!(doc.contains("aden.lang.AssertionError"));
...
fn lock_f1_37_runtime_abi_growth_is_documented_and_board_locked() {
    ...
    assert!(abi_doc.contains("adenc_rt_throw"));
    assert!(abi_doc.contains("adenc_rt_traceUpdateTop"));
    assert!(abi_doc.contains("exception-handler stack"));
    assert!(abi_doc.contains("Class.method(File.aden:line)"));
    assert!(board.contains("| F1-37 |"));
    assert!(board.contains("Done (Locked)"));
    assert!(board.contains("adenc_rt_throw"));
    assert!(board.contains("adenc_rt_traceUpdateTop"));
}
...
fn lock_f1_26_f1_28_numeric_promotion_slice_is_documented_and_board_staged() {
    ...
    assert!(compiler_doc
        .contains("mixed signed/unsigned widening now follows the current executable Aden slice"));
...
fn lock_f1_29_checked_exception_model_is_documented_and_board_locked() {
    ...
    assert!(runtime_doc
        .contains("checked exceptions from calls/constructors are enforced by semantic analysis"));
    assert!(runtime_doc.contains("cross-method Aden-throw propagation"));
    assert!(runtime_doc
...
fn lock_f1_101_exception_diagnostics_are_documented_and_board_locked() {
    ...
    assert!(compiler_doc.contains("`Class.method(File.aden:line)`"));
...
fn lock_m3_18_closure_docs_exist() {
    ...
    assert!(idx.contains("aden.collections"));
...
fn gate_m3_g1_fixture_project_coverage() {
    assert_fixture_build_and_run("runtime_mix", "app/runtime/Main.aden", "runtime_mix_ok\n");
    assert_fixture_build_and_run(
...
fn gate_m3_g3_io_stream_behavior_coverage() {
    ...
    let src_root = root.join("src");
    copy_dir_recursive(&fixture.join("src"), &src_root);
    let entry = src_root.join("app/mixed/Main.aden");
    let output = run_adenc(&[
        "build",
        entry.to_str().expect("entry utf8"),
        "--source-root",
    ...
    assert!(plan.contains("aden.io.File.readAllText"));
}
...
fn lock_f1_58_io_text_and_encoding_policy_is_documented_and_board_locked() {
    ...
    assert!(runtime_doc.contains("Aden Lang currently ships a text-first IO model"));
