...
fn lock_c3_01_object_layout_contract_is_documented_and_emitted() {
    ...
    assert!(
        doc.contains("adenc.object.layout.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.object.layout.v1\""));
...
fn lock_c3_02_static_field_storage_contract_and_runtime_flow_are_locked() {
    ...
    assert!(
        doc.contains("adenc.static.storage.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.static.storage.v1\""));
...
fn lock_c3_03_constructor_invocation_contract_is_documented_and_emitted() {
    ...
    assert!(
        doc.contains("adenc.constructor.model.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.constructor.model.v1\""));
...
fn lock_c3_04_class_allocation_contract_emits_variable_class_sizes() {
    ...
    assert!(
        doc.contains("adenc.class.alloc.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.class.alloc.v1\""));
...
fn lock_c3_06_dispatch_schema_is_documented_and_emitted() {
    ...
    assert!(
        doc.contains("adenc.dispatch.schema.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.dispatch.schema.v1\""));
...
fn lock_c3_12_type_id_schema_is_documented_and_emitted() {
    ...
    assert!(
        doc.contains("adenc.typeid.schema.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.typeid.schema.v1\""));
...
fn lock_c3_14_object_model_abi_contract_is_documented_and_emitted() {
    ...
    assert!(
        doc.contains("adenc.object_model.abi.v1"),
    ...
    assert!(plan.contains("\"schema\": \"adenc.object_model.abi.v1\""));
...
fn lock_c3_15_semantic_constructor_and_layout_edge_matrix_is_locked() {
    let root = unique_temp_root();
    let src_root = root.join("src");
    let ok_entry = src_root.join("app/core/OkMain.aden");
    write_file(
        &ok_entry,
        r#"
        package app.core;
    ...
    let ok = common::adenc_command()
        ...
        .arg("--strict-package")
        .output()
        .expect("run adenc build for legal matrix fixture");
    ...
    let bad_ctor_entry = bad_ctor_src_root.join("app/core/BadCtor.aden");
    write_file(
        &bad_ctor_entry,
        r#"
        package app.core;
    ...
    let bad_ctor = common::adenc_command()
        .arg("build")
        ...
        .arg("--strict-package")
        .output()
        .expect("run adenc build for constructor arity failure fixture");
    assert!(
    ...
    let bad_layout_entry = bad_layout_src_root.join("app/core/BadLayout.aden");
    write_file(
        &bad_layout_entry,
        r#"
        package app.core;
    ...
    let bad_layout = common::adenc_command()
        .arg("build")
        ...
        .arg("--strict-package")
        .output()
        .expect("run adenc build for class-layout failure fixture");
    assert!(
...
fn lock_e2_06_runtime_no_longer_imports_user_field_storage_and_field_growth_stays_live() {
    ...
    assert!(
        !runtime_asm.contains("adenc_fld_app_core_Box_value"),
        "runtime asm should not import app field storage anymore\n{}",
        runtime_asm
    );
    assert!(
        !runtime_asm.contains("adenc_fld_app_core_Box_value_heap_owned"),
        "runtime asm should not import app field ownership flags anymore\n{}",
        runtime_asm
    );
    assert!(
        !runtime_asm.contains("adenc_fld_app_core_Box_next"),
        "runtime asm should not import app ARC field storage anymore\n{}",
        runtime_asm
    );
    assert!(
        !runtime_asm.contains("adenc_fld_app_core_Box_next_heap_owned"),
