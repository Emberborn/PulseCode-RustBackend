...
pub(super) fn resolve_workspace_context(
    ...
    if let Ok(workspace) = resolve_workspace_context_via_adklib(entry_arg, flags) {
        return Ok(workspace);
    }
    resolve_workspace_context_fallback(entry_arg, flags)
}

fn resolve_workspace_context_via_adklib(
    entry_arg: Option<&str>,
    flags: &CliFlags,
) -> Result<Option<WorkspaceContext>, String> {
...
fn parse_workspace_context_bridge_output(text: &str) -> Result<Option<WorkspaceContext>, String> {
    ...
    let count = manifest_required_bridge_value(&values, "count")?
        .parse::<usize>()
        .map_err(|e| format!("adk bridge workspace count parse failed: {e}"))?;
...
pub(super) fn resolve_workspace_member_roots(
    ...
    for member in members {
        ...
        let manifest = normalized.join("aden.toml");
        if !manifest.exists() {
            return Err(format!(
    ...
    if out.is_empty() {
        return Err(format!(
            ...
            workspace_root.join("aden.toml").display()
        ));
    }
    Ok(out)
}
...
pub(super) fn find_manifest_path(start_dir: &Path) -> Result<Option<PathBuf>, String> {
    ...
    loop {
        let candidate = dir.join("aden.toml");
        if candidate.exists() {
...
pub(super) fn load_manifest_config(manifest_path: &Path) -> Result<ManifestConfig, String> {
    ...
    let mut main_aden: Option<String> = None;
    let mut main_resources: Option<String> = None;
    let mut test_aden: Option<String> = None;
    let mut test_resources: Option<String> = None;
    let mut api_aden: Option<String> = None;
    ...
    let mut adklib_enabled: Option<bool> = None;
    ...
    for (line_no, raw_line) in content.lines().enumerate() {
        ...
        if line.starts_with('[') && line.ends_with(']') {
            ...
            match section_name {
                "package" | "sources" | "build" | "toolchain" | "workspace" | "adklib" => {
        ...
        match active_section {
            ...
            "sources" => match key {
                ...
                "main_aden" => {
                    main_aden = Some(require_manifest_string(
                        manifest_path,
                        key,
                        v.trim(),
                        line_no + 1,
                ...
                "test_aden" => {
                    test_aden = Some(require_manifest_string(
                        manifest_path,
                        key,
                        v.trim(),
                        line_no + 1,
                ...
                "api_aden" => {
                    api_aden = Some(require_manifest_string(
                        manifest_path,
                        key,
                        v.trim(),
                        line_no + 1,
            ...
            "adklib" => match key {
                "enabled" => {
                    adklib_enabled = Some(require_manifest_bool(
                        manifest_path,
                        key,
                        v.trim(),
                        line_no + 1,
                ...
                _ => {
                    return Err(format!(
                        "Manifest parse error in '{}': unknown key '{}' in [adklib] at line {}",
                        manifest_path.display(),
                        key,
                        line_no + 1
                    ))
    ...
    let sources_root = root.unwrap_or_else(|| "src/main/aden".to_string());
    ...
    for (key, value) in [
        ("[sources].main_aden", main_aden.as_deref()),
        ("[sources].main_resources", main_resources.as_deref()),
        ("[sources].test_aden", test_aden.as_deref()),
        ("[sources].test_resources", test_resources.as_deref()),
        ("[sources].api_aden", api_aden.as_deref()),
    ...
    Ok(ManifestConfig {
        package,
        workspace,
        adklib: adklib_enabled.map(|enabled| ManifestAdklib { enabled }),
