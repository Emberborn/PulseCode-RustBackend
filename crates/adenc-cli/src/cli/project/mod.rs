...
use super::*;
use adenc_core::{ArrayInitExpr, ClassMember, Expr, Stmt};
...
pub(super) fn check_project(
    ...
    check_project_with_adklib(entry_path, source_root, strict_package, false)
}

pub(super) fn check_project_with_adklib(
    entry_path: &str,
    source_root: Option<&str>,
    strict_package: bool,
    adklib_enabled: bool,
...
pub(super) fn load_unit(
    ...
    let source_name = canonical
        ...
        .unwrap_or("<memory>");
    let program =
        adenc_core::parse_with_source_name(&source, source_name).map_err(|e| e.to_string())?;
    ...
    for import in &program.imports {
        validate_aden_import(import, adklib_enabled)?;
        if is_builtin_import(&import.path) {
            if let Some(stdlib_targets) = resolve_builtin_import_targets(import, adklib_enabled)?
            {
...
fn try_load_same_package_candidate(
    ...
    let snapshot: Vec<(PathBuf, Option<String>)> = units
        ...
        .collect();
    let file_name = format!("{simple_name}.aden");
...
pub(super) fn resolve_builtin_import_targets(
    ...
    if import.is_static {
        ...
        if class_path.starts_with("adk.") && !adklib_enabled {
            return Err(
                "Import 'adk.*' requires [adklib].enabled = true in aden.toml".to_string(),
            );
        }
    ...
    if import.is_wildcard {
        ...
        if import.path.starts_with("adk.") && !adklib_enabled {
            return Err(
                "Import 'adk.*' requires [adklib].enabled = true in aden.toml".to_string(),
            );
        }
...
pub(super) fn discover_entry_from_source_root(
    ...
    let candidate = source_root.join("app").join("main").join("Main.aden");
    if candidate.exists() {
        return Ok(Some(candidate));
    }
    let mut aden_files = Vec::new();
    collect_aden_files(source_root, &mut aden_files)?;
    aden_files.sort();
    if aden_files.len() == 1 {
        Ok(aden_files.into_iter().next())
    } else {
        Ok(None)
    }
}

pub(super) fn collect_aden_files(dir: &Path, out: &mut Vec<PathBuf>) -> Result<(), String> {
    let entries = fs::read_dir(dir)
    ...
    for entry in entries {
        ...
        } else if path.extension().and_then(|e| e.to_str()) == Some("aden") {
            out.push(path);
        }
    }
    Ok(())
...
pub(super) fn scaffold_project(
    ...
    let source_root = project_root.join("src").join("main").join("aden");
    for path in [
        project_root.join("src").join("main").join("aden"),
        project_root.join("src").join("main").join("resources"),
        project_root.join("src").join("test").join("aden"),
        project_root.join("src").join("test").join("resources"),
        project_root.join("src").join("api").join("aden"),
    ...
    let (entry_rel, entry_content, extra_files): (PathBuf, String, Vec<(PathBuf, String)>) =
        ...
        NewTemplate::App => (
            PathBuf::from("app/main/Main.aden"),
            format!(
                "package app.main;\nimport aden.lang.IO;\n\nclass Main {{\n    public static void main() {{\n        IO.println(\"{} online\");\n    }}\n}}\n",
                escape_aden_string(trimmed)
            ),
            Vec::new(),
        ),
        NewTemplate::Lib => (
            PathBuf::from("lib/core/Main.aden"),
            "package lib.core;\nimport aden.lang.IO;\n\nclass Main {\n    public static void main() {\n        IO.println(Library.versionCode());\n    }\n}\n".to_string(),
            vec![(
                PathBuf::from("lib/core/Library.aden"),
    ...
    let manifest_path = project_root.join("aden.toml");
    ...
    fs::create_dir_all(project_root.join("src").join("test").join("aden")).map_err(|e| {
        format!(
    ...
    fs::write(
        project_root
            ...
            .join("aden")
            .join(".gitkeep"),
        "# Reserved for aden test fixtures\n",
    )
...
pub(super) fn escape_aden_string(value: &str) -> String {
...
pub(super) fn is_builtin_import(path: &str) -> bool {
    path == "aden" || path.starts_with("aden.") || path == "adk" || path.starts_with("adk.")
}

pub(super) fn validate_aden_import(
    import: &ImportDecl,
    adklib_enabled: bool,
) -> Result<(), String> {
    let is_aden_import = import.path == "aden" || import.path.starts_with("aden.");
    let is_adk_import = import.path == "adk" || import.path.starts_with("adk.");
    if !is_aden_import && !is_adk_import {
        return Ok(());
    }

    if import.path == "aden" {
        return Err(format!(
            "Import '{}' is too broad. Import a package under 'aden.*' or a specific class",
            import.path
        ));
    }
    if import.path == "adk" {
        return Err(
            "Import 'adk' is too broad. Import a package under 'adk.*' or a specific class"
                .to_string(),
        );
    }
    if is_adk_import && !adklib_enabled {
        return Err(
            "Import 'adk.*' requires [adklib].enabled = true in aden.toml".to_string(),
        );
    }
        ...
        if import.is_wildcard {
            ...
            if !owner_path.exists() {
                return Err(format!(
                    ...
                    if is_adk_import { "adk" } else { "aden" },
                    import.path,
                ));
            }
            return Ok(());
...
pub(super) fn resolve_import_targets(
    ...
    if import.is_wildcard {
        ...
        if out.is_empty() {
            return Err(format!(
                "Wildcard import '{}' from '{}' resolved to '{}' but found no .aden files",
                import.path,
                current_file.display(),
                dir.display()
            ));
