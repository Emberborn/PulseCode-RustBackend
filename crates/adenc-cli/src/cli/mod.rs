...
pub(crate) fn run() {
    ...
    let Some(command_arg) = args.get(1) else {
        ...
        emit_hint("run 'aden help' to see available commands");
        print_help(true);
    ...
    match command {
        CliCommand::PrewarmAuthorBuildBridge => match prewarm_adk_build_bridge_runner() {
            Ok(path) => {
        ...
        CliCommand::ProviderCheck => {
            ...
            match check_project_with_adklib(entry_path, source_root, strict, adklib_enabled) {
                Ok(result) => {
                    print!(
                        "{}",
        ...
        CliCommand::New => {
            ...
            let flags = match parse_new_flags(&command_args[1..]) {
                ...
                Err(msg) => {
                    ...
                    emit_hint("run 'aden new --help' to see accepted flags");
                    print_new_usage(true);
                    process::exit(EXIT_USAGE_ERROR);
                }
            };
        ...
        CliCommand::Check => {
            ...
            let cli_flags = match parse_flags(CliCommand::Check, flag_args) {
                ...
                Err(msg) => {
                    emit_error(DIAG_USAGE, &msg);
                    emit_hint("run 'aden check --help' to see accepted flags");
            ...
            if let Some(workspace) = workspace {
                ...
                for member_root in &workspace.member_roots {
                    ...
                    match check_project_with_adklib(
                        &resolved.entry_path,
                        resolved.source_root.as_deref(),
                        strict,
                        resolved.adklib_enabled,
                    ) {
                        Ok(result) => {
                            passed += 1;
                            println!(
            ...
            let resolved = match resolve_check_invocation(entry_arg, &cli_flags) {
                ...
                Err(msg) => {
                    emit_error(DIAG_USAGE, &msg);
                    emit_hint("provide <entry.aden> or set [sources].entry in aden.toml");
                    print_check_usage(true);
                    process::exit(EXIT_USAGE_ERROR);
                }
            };
        ...
        CliCommand::Build => {
            ...
            let cli_flags = match parse_flags(CliCommand::Build, flag_args) {
                ...
                Err(msg) => {
                    emit_error(DIAG_USAGE, &msg);
                    emit_hint("run 'aden build --help' to see accepted flags");
            ...
            match execute_build_pipeline_owned(&resolved, &cli_flags) {
                ...
                Ok(result) => {
                    ...
                    emit_hint("run 'aden check' first to isolate compile-time issues");
                    process::exit(EXIT_COMMAND_FAILURE);
                }
                Err(err) => {
        ...
        CliCommand::Test => {
            ...
            let cli_flags = match parse_flags(CliCommand::Test, command_args) {
                ...
                Err(msg) => {
                    emit_error(DIAG_USAGE, &msg);
                    emit_hint("run 'aden test --help' to see accepted flags");
            ...
            if tests.is_empty() {
                ...
                emit_hint("add .aden files under tests/ with a main entrypoint");
                process::exit(EXIT_COMMAND_FAILURE);
            }
            println!(
                "{}",
...
fn execute_build_pipeline_inline_from_args(args: &[String]) -> Result<BuildRun, String> {
    ...
    let resolved = BuildInvocation {
        ...
        main_aden_root: PathBuf::from(required_provider_arg(args, 4, "main_aden_root")?),
        ...
        adklib_enabled: provider_bool_arg(args.get(23)),
    };
    let flags = CliFlags {
...
fn render_workspace_check_start(root: &Path, member_count: usize, mode: &str) -> String {
    ...
    run_adk_build_bridge_request(&request).unwrap_or_else(|_| {
        format!(
...
fn render_workspace_build_summary(failed: usize, total: usize) -> String {
    ...
    run_adk_build_bridge_request(&request)
...
fn parse_workspace_check_execution_output(text: &str) -> Result<WorkspaceCheckExecution, String> {
    ...
    let success = match values.first().and_then(|value| value.as_deref()) {
        ...
        other => {
            return Err(format!(
                "adk compiler workspace-check bridge returned invalid success flag {:?}",
                other
            ))
        }
    };
...
fn parse_workspace_test_execution_output(text: &str) -> Result<WorkspaceTestExecution, String> {
    ...
    let success = match values.first().and_then(|value| value.as_deref()) {
        ...
        other => {
            return Err(format!(
                "adk compiler workspace-test bridge returned invalid success flag {:?}",
                other
            ))
        }
    };
...
fn parse_workspace_build_execution_output(text: &str) -> Result<WorkspaceBuildExecution, String> {
    ...
    let success = match values.first().and_then(|value| value.as_deref()) {
        ...
        other => {
            return Err(format!(
                "adk build workspace bridge returned invalid success flag {:?}",
                other
            ))
        }
    };
...
fn execute_compiler_check(
    ...
    if selfhost_provider {
        if let Ok(result) = execute_compiler_check_via_adklib_bridge(
            entry_path,
            source_root,
            strict_package,
            adklib_enabled,
    ...
    match check_project_with_adklib(entry_path, source_root, strict_package, adklib_enabled) {
...
fn execute_compiler_check_via_adklib_bridge(
    entry_path: &str,
    source_root: Option<&str>,
    strict_package: bool,
    adklib_enabled: bool,
    ...
    let adklib_value = if adklib_enabled { "true" } else { "false" };
    let request = emit_compiler_bridge_summary_request(
...
fn parse_compiler_check_execution_output(text: &str) -> Result<CompilerCheckExecution, String> {
    ...
    if values.len() < 6 {
        return Err(format!(
            "adk compiler bridge returned {} values, expected at least 6",
            values.len()
        ));
    }
    let success = match values[0].as_deref() {
        ...
        other => {
            return Err(format!(
                "adk compiler bridge returned invalid success flag {:?}",
                other
            ));
        }
    };
...
fn parse_bridge_raw_values(text: &str) -> Result<Vec<Option<String>>, String> {
    ...
    for line in text.lines() {
        let separator = line
            .find(':')
            .ok_or_else(|| format!("adk compiler bridge line missing ':' separator: {line}"))?;
        ...
        let value = match present {
            ...
            other => {
                return Err(format!(
                    "adk compiler bridge line has invalid presence flag '{other}'"
                ));
            }
        };
        out.push(value);
...
fn parse_bridge_usize(value: Option<&str>, key: &str) -> Result<usize, String> {
    match value {
        Some(raw) if !raw.is_empty() => raw
            .parse::<usize>()
            .map_err(|e| format!("adk compiler bridge key '{key}' parse failed: {e}")),
        _ => Ok(0),
    }
}
...
fn unescape_bridge_raw_value(value: &str) -> Result<String, String> {
    ...
    while let Some(ch) = chars.next() {
        ...
        let escaped = chars
            .next()
            .ok_or_else(|| "adk compiler bridge value ended in dangling escape".to_string())?;
        match escaped {
            'n' => out.push('\n'),
            'r' => out.push('\r'),
            '\\' => out.push('\\'),
...
fn execute_build_compiler_core_inline(
    ...
    let result = check_project_with_adklib(entry_path, source_root, true, adklib_enabled)?;
    let ir =
...
fn render_test_no_files(tests_root: &Path) -> String {
    ...
    run_adk_build_bridge_request(&request).unwrap_or_else(|_| {
        format!(
            "Test discovery failed: no .aden tests found under '{}'",
            tests_root.display()
        )
    })
}
...
fn render_workspace_member_no_tests(member_root: &Path, tests_root: &Path) -> String {
    ...
    run_adk_build_bridge_request(&request).unwrap_or_else(|_| {
        format!(
            "[FAIL] {} :: no .aden tests found under '{}'",
            member_root.display(),
            tests_root.display()
        )
    })
...
mod tests {
    ...
    fn unique_temp_root() -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        env::temp_dir().join(format!("adenc_cli_tests_{nanos}"))
    }
    ...
    fn parse_flags_build_accepts_other_canonical_targets() {
        let adenos = parse_flags(
            CliCommand::Build,
            &["--target".to_string(), "adenos-x64".to_string()],
        )
        .expect("adenos target should parse");
        assert_eq!(adenos.target.as_deref(), Some("adenos-x64"));
    ...
    fn target_taxonomy_defines_immediate_and_later_target_ids() {
        ...
        let adenos = resolve_target_descriptor(PULSEOS_X64_TARGET_ID).expect("adenos target");
        assert_eq!(adenos.os_family, "adenos");
        assert_eq!(adenos.arch, "x64");
        assert_eq!(adenos.support_level, TargetSupportLevel::Immediate);
        assert_eq!(adenos.lane_name, "adenos-x64-first-slice");
        assert_eq!(adenos.lane_kind, TargetLaneKind::FirstSliceContract);
    ...
    fn manifest_v1_accepts_full_schema_sections() {
        ...
        let manifest = root.join("aden.toml");
        write_file(
            &manifest,
            r#"
            [package]
        ...
        assert_eq!(parsed.sources.root, "src/main/aden");
        assert_eq!(parsed.sources.main_aden.as_deref(), Some("src/main/aden"));
        assert_eq!(parsed.sources.test_aden.as_deref(), Some("src/test/aden"));
        assert_eq!(parsed.sources.entry.as_deref(), Some("app/main/Main.aden"));
        assert_eq!(parsed.build.profile.as_deref(), Some("release"));
        assert_eq!(parsed.build.target.as_deref(), Some("adenos-x64"));
    ...
    fn check_project_rejects_import_cycle() {
        let root = unique_temp_root();
        let src_root = root.join("src");
        let a = src_root.join("app/core/A.aden");
        let b = src_root.join("app/core/B.aden");
    ...
    fn check_project_enforces_cross_file_visibility_and_package_rules() {
        let root = unique_temp_root();
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        let util_same_pkg = src_root.join("app/core/Util.aden");
        let util_other_pkg = src_root.join("app/other/OtherUtil.aden");
    ...
    fn check_project_accepts_wildcard_package_import() {
        ...
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        let util = src_root.join("app/shared/Util.aden");
    ...
    fn check_project_accepts_static_import_member_and_wildcard() {
        ...
        let src_root = root.join("src");
        let util = src_root.join("app/shared/Util.aden");
        let main_member = src_root.join("app/core/MainMember.aden");
        let main_wild = src_root.join("app/alt/MainWild.aden");
    ...
    fn check_project_rejects_ambiguous_class_imports() {
        ...
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        let util_a = src_root.join("app/shared/Util.aden");
        let util_b = src_root.join("app/tools/Util.aden");
    ...
    fn check_project_allows_same_package_class_without_import() {
        ...
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        let util = src_root.join("app/core/Util.aden");
    ...
    #[test]
    fn check_project_accepts_supported_aden_imports() {
        let root = unique_temp_root();
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        ...
        write_file(
            ...
            import aden.lang.ConsoleWriter;
    ...
    #[test]
    fn check_project_loads_stdlib_sources_for_aden_imports() {
        let root = unique_temp_root();
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
    ...
    #[test]
    fn check_project_rejects_unknown_aden_import() {
        let root = unique_temp_root();
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("expected unknown aden import error")
            .contains("Unknown aden import 'aden.time.TimeMachine'"));

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn check_project_rejects_too_broad_or_unknown_aden_imports() {
        let root = unique_temp_root();
        let src_root = root.join("src");
        let main_broad = src_root.join("app/core/MainBroad.aden");
        let main_wild = src_root.join("app/core/MainWild.aden");
        ...
        assert!(wild_result
            ...
            .contains("Unknown aden wildcard import 'aden.ghost'"));
    ...
    #[test]
    fn check_project_rejects_adklib_import_without_manifest_opt_in() {
        let root = unique_temp_root();
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("expected adklib gate error")
            .contains("requires [adklib].enabled = true"));
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn check_project_accepts_adklib_import_with_manifest_opt_in() {
        let root = unique_temp_root();
        let src_root = root.join("src/main/aden");
        let main = src_root.join("app/core/Main.aden");
        let manifest = root.join("aden.toml");
        ...
        assert!(
            result.is_ok(),
            "expected adklib import success: {result:?}"
        );
    ...
    fn check_project_prefers_same_package_class_over_imported_same_name() {
        ...
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        let local_util = src_root.join("app/core/Util.aden");
        let shared_util = src_root.join("app/shared/Util.aden");
    ...
    fn check_project_prefers_explicit_import_over_wildcard_same_name() {
        ...
        let src_root = root.join("src");
        let main = src_root.join("app/core/Main.aden");
        let tools_util = src_root.join("app/tools/Util.aden");
        let shared_util = src_root.join("app/shared/Util.aden");
