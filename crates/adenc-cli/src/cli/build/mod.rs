...
use crate::cli::config::{
    append_bridge_request_raw_value, append_bridge_request_value, run_adk_build_bridge_request,
};
...
fn execute_build_pipeline_via_authorlib_bridge(
    ...
    let request = {
        ...
        append_bridge_request_raw_value(&mut out, resolved.main_aden_root.to_str());
        ...
        append_bridge_request_raw_value(
            &mut out,
            Some(if resolved.adklib_enabled { "true" } else { "false" }),
        );
        out
    };
...
fn parse_build_pipeline_execution_output(text: &str) -> Result<BuildPipelineProviderExecution, String> {
    ...
    let success = match values.first().and_then(|value| value.as_deref()) {
        ...
        other => {
            return Err(format!(
                "adk build pipeline bridge returned invalid success flag {:?}",
                other
            ))
        }
    };
...
fn parse_build_compiler_core_bridge_output(text: &str) -> Result<(usize, BackendArtifact), String> {
    ...
    let success = match values.first().and_then(|value| value.as_deref()) {
        ...
        other => {
            return Err(format!(
                "adk compiler build bridge returned invalid success flag {:?}",
                other
            ));
        }
    };
...
fn parse_build_compiler_core_bridge_values(text: &str) -> Result<Vec<Option<String>>, String> {
    ...
    for line in text.lines() {
        let separator = line.find(':').ok_or_else(|| {
            format!("adk compiler build bridge line missing ':' separator: {line}")
        })?;
        ...
        let value = match present {
            ...
            other => {
                return Err(format!(
                    "adk compiler build bridge line has invalid presence flag '{other}'"
                ));
            }
        };
        out.push(value);
...
fn required_build_core_value(
    ...
    values
        .get(index)
        .and_then(|value| value.clone())
        .ok_or_else(|| format!("adk compiler build bridge missing '{key}'"))
}
...
fn parse_build_core_usize(value: Option<&str>, key: &str) -> Result<usize, String> {
    match value {
        Some(raw) if !raw.is_empty() => raw
            .parse::<usize>()
            .map_err(|e| format!("adk compiler build bridge key '{key}' parse failed: {e}")),
        _ => Ok(0),
    }
}
...
fn unescape_build_core_value(value: &str) -> Result<String, String> {
    ...
    while let Some(ch) = chars.next() {
        ...
        let escaped = chars.next().ok_or_else(|| {
            "adk compiler build bridge value ended in dangling escape".to_string()
        })?;
        match escaped {
            'n' => out.push('\n'),
            'r' => out.push('\r'),
...
pub(super) fn publish_build_artifacts(
    ...
    if let Ok(output) = run_adk_build_bridge_request(
...
fn resolve_build_publication_plan(
    ...
    if let Ok(output) =
        run_adk_build_bridge_request(&emit_build_publication_plan_bridge_request(
            resolved,
            artifact,
            backend_out_dir,
            publish_profile_layout,
    ...
    let published_ir_path = if publish_debug_diagnostics {
        debug_target_root.join(format!("{artifact_base_name}-adenc.ir.txt"))
    } else {
...
fn emit_build_materialize_layout_bridge_request(
    ...
    append_bridge_request_value(&mut out, "mainAdenRoot", resolved.main_aden_root.to_str());
    out
}
...
fn emit_build_finalize_artifacts_bridge_request(
    ...
    append_bridge_request_value(
        &mut out,
        "mainAdenRoot",
        Some(resolved.main_aden_root.to_string_lossy().as_ref()),
    );
    append_bridge_request_raw_value(&mut out, Some(&resolved.profile));
    append_bridge_request_raw_value(&mut out, Some(&resolved.target));
    append_bridge_request_raw_value(&mut out, Some(&resolved.output_mode));
...
fn author_build_finalize_outputs(
    ...
    let output = run_adk_build_bridge_request(&request)?;
...
fn author_build_write_shared_runtime_lookup_metadata(
    ...
    let output =
    run_adk_build_bridge_request(&emit_build_write_launch_metadata_bridge_request(
            descriptor_path,
            profile,
            artifact_stamp,
            exe,
    ...
    if written.is_empty() {
        return Err("adk build launch metadata writer returned no path".to_string());
    }
    Ok(())
}
...
fn author_build_write_artifact_stamp(
    ...
    let output = run_adk_build_bridge_request(&emit_build_write_artifact_stamp_bridge_request(
        stamp_path,
        package_name,
        package_version,
        profile,
    ...
    let written = output.trim();
    if written.is_empty() {
        return Err("adk build artifact stamp writer returned no path".to_string());
    }
    Ok(())
}

fn author_build_copy_file(source_path: &Path, destination_path: &Path) -> Result<(), String> {
    let output = run_adk_build_bridge_request(&emit_build_copy_file_bridge_request(
        source_path,
        destination_path,
    ))?;
    let written = output.trim();
    if written.is_empty() {
        return Err("adk build copy file returned no path".to_string());
    }
    Ok(())
}

fn author_build_copy_recursive(source_path: &Path, destination_path: &Path) -> Result<(), String> {
    let output = run_adk_build_bridge_request(&emit_build_copy_recursive_bridge_request(
        source_path,
        destination_path,
    ))?;
    let written = output.trim();
    if written.is_empty() {
        return Err("adk build copy recursive returned no path".to_string());
    }
    Ok(())
}
...
fn author_build_copy_recursive_extension(
    ...
    let output =
    run_adk_build_bridge_request(&emit_build_copy_recursive_extension_bridge_request(
            source_path,
            destination_path,
            extension,
        ))?;
    let written = output.trim();
    if written.is_empty() {
        return Err("adk build copy recursive extension returned no path".to_string());
    }
    Ok(())
}
...
fn author_build_materialize_layout(
    ...
    let output = run_adk_build_bridge_request(&emit_build_materialize_layout_bridge_request(
        resolved,
        artifact,
        backend_out_dir,
        published_object_path,
    ...
    let written = output.trim();
    if written.is_empty() {
        return Err("adk build materialize layout returned no path".to_string());
    }
    Ok(())
}
...
fn author_build_write_config_plan(
    ...
    let output = run_adk_build_bridge_request(&emit_build_write_config_plan_bridge_request(
        plan_path,
        resolved,
        artifact,
        target_adapter_metadata,
    ...
    let written = output.trim();
    if written.is_empty() {
        return Err("adk build config plan writer returned no path".to_string());
    }
    Ok(())
}

fn author_build_render_summary(build: &BuildRun) -> Result<String, String> {
    run_adk_build_bridge_request(&emit_build_render_summary_bridge_request(build))
}
...
pub(super) fn write_shared_runtime_lookup_metadata(
    ...
    let runtime_library_name = runtime_library
        .file_name()
        .unwrap_or_else(|| std::ffi::OsStr::new("adenrt.dll"))
        .to_string_lossy();
    let runtime_import_library_name = runtime_import_library
        .file_name()
        .unwrap_or_else(|| std::ffi::OsStr::new("adenrt.lib"))
        .to_string_lossy();
    ...
    launch_text.push_str("schema=adenc.shared.launch.v1\n");
    ...
    launch_text.push_str("runtime_abi_schema=adenc.runtime.abi.v1\n");
    launch_text.push_str("runtime_abi_version=2\n");
    launch_text.push_str("object_model_abi_schema=adenc.object_model.abi.v1\n");
...
pub(super) fn materialize_stamped_build_artifacts(
    ...
    stamp_text.push_str("schema=adenc.artifact.stamp.v1\n");
...
pub(super) fn emit_resolved_build_config_plan(
    ...
    let mut out = String::new();
    out.push_str("{\n");
    out.push_str("  \"schema\": \"adenc.build.config.v1\",\n");
...
pub(super) fn materialize_build_layout(
    ...
    if resolved.main_aden_root.exists() {
        let sanity_src_dir = resolved.build_sanity_dir.join("src_main_aden");
        copy_dir_recursive(&resolved.main_aden_root, &sanity_src_dir)?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn emit_resolved_build_config_plan_uses_resolved_toolchain_not_cli_flags() {
        ...
        let resolved = BuildInvocation {
            entry_path: root
                .join("src/main/aden/app/core/Main.aden")
                .display()
                .to_string(),
            source_root: Some(root.join("src/main/aden").display().to_string()),
            build_root: root.join("build"),
            output_root: root.join("build"),
            main_aden_root: root.join("src/main/aden"),
