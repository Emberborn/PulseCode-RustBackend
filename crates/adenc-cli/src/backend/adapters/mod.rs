...
use adenc_core::{IrMethod, IrProgram};
use std::path::{Path, PathBuf};

pub(crate) mod adenos_x64;
...
pub(crate) fn resolve_requested_target_runtime_service_contract(
    requested_target_id: &str,
) -> Result<Option<&'static adenos_x64::AdenOsRuntimeServiceContract>, String> {
    let requested_target = resolve_target_descriptor(requested_target_id).ok_or_else(|| {
        format!(
    ...
    match requested_target.target_id {
        PULSEOS_X64_TARGET_ID => Ok(Some(adenos_x64::adenos_x64_runtime_service_contract())),
        _ => Ok(None),
    }
}
...
pub(crate) fn resolve_requested_target_artifact_contract(
    ...
    match requested_target.target_id {
        ...
        PULSEOS_X64_TARGET_ID => Ok(Some(adenos_x64::adenos_x64_artifact_contract(
            output_mode,
        ))),
        _ => Ok(None),
    }
...
pub(crate) fn resolve_requested_target_startup_loader_contract(
    ...
    match requested_target.target_id {
        ...
        PULSEOS_X64_TARGET_ID => Ok(Some(adenos_x64::adenos_x64_startup_loader_contract(
            output_mode,
        ))),
        _ => Ok(None),
    }
...
pub(crate) fn resolve_requested_target_executable_loading_proof_target(
    ...
    match requested_target.target_id {
        PULSEOS_X64_TARGET_ID => Ok(Some(
            adenos_x64::adenos_x64_executable_loading_proof_target(output_mode),
        )),
        _ => Ok(None),
    }
}
