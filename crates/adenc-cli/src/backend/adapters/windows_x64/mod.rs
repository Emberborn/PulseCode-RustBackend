...
static WINDOWS_X64_FAT_LOADER_EXPECTATIONS: &[&str] = &[
    ...
    "call-adenc_rt_init-before-app_entry",
    "call-adenc_rt_shutdown-before-exitprocess",
...
pub(crate) fn windows_x64_artifact_contract(output_mode: &str) -> TargetArtifactContract {
    match output_mode {
        "shared" => TargetArtifactContract {
            ...
            naming_policy: "<entry>.exe + adencore-<adenc-semver>.dll/.lib + launch.txt",
...
pub(crate) fn windows_x64_startup_loader_contract(
    ...
    match output_mode {
        "shared" => TargetStartupLoaderContract {
            ...
            loader_manifest_schema: Some("adenc.shared.launch.v1"),
...
impl TargetAdapter for WindowsX64TargetAdapter {
    ...
    fn runtime_abi_family(&self, output_mode: &str) -> &'static str {
        match output_mode {
            "shared" => "adenc-runtime-abi-v2-shared-windows-bootstrap",
            _ => "adenc-runtime-abi-v2-embedded-windows-bootstrap",
        }
    }
