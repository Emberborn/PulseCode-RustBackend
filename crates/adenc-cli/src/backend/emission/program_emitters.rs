...
fn should_emit_trace_frame(package_name: &str, class_name: &str, method_name: &str) -> bool {
    !(package_name == "aden.lang" && class_name == "Throwable" && method_name == "panic")
}
...
fn should_emit_statement_trace_metadata(
    ...
    !(package_name.starts_with("aden.") || package_name.starts_with("adk."))
}
...
fn callback_interface_name(arity: usize) -> String {
    format!("aden.interop.NativeCallback{}", arity)
}
...
fn emit_object_class_name_proc(
    ...
    out.push_str(&format!("{} proc\n", symbol));
    out.push_str("    sub rsp, 56\n");
    out.push_str("    call adenc_rt_objectClassId\n");
...
pub(crate) fn emit_masm_split_program_objects(
    ...
    for class in &ir.classes {
        ...
        source.push_str("extrn adenc_rt_stringFromBytes:proc\n");
        source.push_str("extrn adenc_rt_consoleWrite:proc\n");
        source.push_str("extrn adenc_rt_consoleWriteLine:proc\n");
        source.push_str("extrn adenc_rt_panic:proc\n");
        source.push_str("extrn adenc_rt_fpToInt:proc\n");
        source.push_str("extrn adenc_rt_fpToLong:proc\n");
        let object_to_string_symbol = mangle_method_symbol("aden.lang", "Object", "toString", &[]);
        let throwable_to_string_symbol =
            mangle_method_symbol("aden.lang", "Throwable", "toString", &[]);
        if !(class.package_name == "aden.lang" && class.name == "Object") {
            source.push_str(&format!("extrn {}:proc\n", object_to_string_symbol));
        }
        if !(class.package_name == "aden.lang" && class.name == "Throwable") {
        ...
        source.push_str("extrn adenc_rt_arcRetain:proc\n");
        source.push_str("extrn adenc_rt_arcRelease:proc\n");
    ...
    emit_runtime_init_proc(&mut std_src, "adenc_rt_init", WRITE_RAW_SYMBOL);
    std_src.push('\n');
    emit_runtime_shutdown_proc(&mut std_src, "adenc_rt_shutdown");
    std_src.push('\n');
    ...
    std_src.push('\n');
    emit_string_from_bytes_proc(&mut std_src, "adenc_rt_stringFromBytes");
    std_src.push('\n');
    emit_string_equals_proc(&mut std_src, "adenc_rt_stringEquals");
    std_src.push('\n');
    emit_write_raw_proc(&mut std_src, WRITE_RAW_SYMBOL);
    std_src.push('\n');
    emit_host_path_alloc_proc(&mut std_src, "adenc_rt_hostPathAlloc");
    std_src.push('\n');
    emit_fp_to_int_proc(&mut std_src, "adenc_rt_fpToInt");
    std_src.push('\n');
    emit_fp_to_long_proc(&mut std_src, "adenc_rt_fpToLong");
    std_src.push('\n');
    ...
    std_src.push('\n');
    emit_ensure_slot_capacity_proc(&mut std_src, "adenc_rt_ensureSlotCapacity");
    std_src.push('\n');
    emit_arc_teardown_proc(&mut std_src, "adenc_rt_arcTeardown", &object_arc_fields);
    std_src.push('\n');
    ...
    for sym in std_symbols {
        match sym.as_str() {
            ...
            "adenc_rt_consoleReadLine" => emit_console_read_line_proc(&mut std_src, &sym),
            "adenc_rt_consoleWrite" => {
            ...
            "adenc_rt_panic" => emit_panic_proc(&mut std_src, &sym),
            DISPATCH_NULL_PANIC_SYMBOL => {
            ...
            "adenc_rt_stringConcat" => emit_string_concat_proc(&mut std_src, &sym),
            "adenc_rt_stringLength" => emit_string_length_proc(&mut std_src, &sym),
            "adenc_rt_parseInt" => emit_parse_int_proc(&mut std_src, &sym),
            "adenc_rt_parseBoolean" => emit_parse_boolean_proc(&mut std_src, &sym),
            OBJECT_CLASS_NAME_SYMBOL => {}
            OBJECT_HASH_CODE_SYMBOL => {}
            ...
            "adenc_rt_arrayNew" => emit_array_new_proc(&mut std_src, &sym),
            "adenc_rt_arrayNewMulti" => emit_array_new_multi_proc(&mut std_src, &sym),
            "adenc_rt_arrayLength" => emit_array_length_proc(&mut std_src, &sym),
            "adenc_rt_arrayGetInt" => emit_array_get_proc(&mut std_src, &sym, "rt_arr_i_ptr"),
            "adenc_rt_arraySetInt" => emit_array_set_proc(&mut std_src, &sym, "rt_arr_i_ptr"),
            ...
            "adenc_rt_arrayGetString" => emit_array_get_proc(&mut std_src, &sym, "rt_arr_s_ptr"),
            "adenc_rt_arraySetString" => emit_array_set_proc(&mut std_src, &sym, "rt_arr_s_ptr"),
            "adenc_rt_listNew" => emit_list_new_proc(&mut std_src, &sym),
            "adenc_rt_listSize" => emit_list_size_proc(&mut std_src, &sym),
            "adenc_rt_listKind" => emit_list_kind_proc(&mut std_src, &sym),
            "adenc_rt_listClear" => emit_list_clear_proc(&mut std_src, &sym),
            "adenc_rt_listAddInt" => emit_list_add_proc(&mut std_src, &sym, "rt_list_i_ptr"),
            "adenc_rt_listAddString" => emit_list_add_proc(&mut std_src, &sym, "rt_list_s_ptr"),
            "adenc_rt_listGetInt" => emit_list_get_proc(&mut std_src, &sym, "rt_list_i_ptr"),
            "adenc_rt_listGetString" => emit_list_get_proc(&mut std_src, &sym, "rt_list_s_ptr"),
            "adenc_rt_mapNew" => emit_map_new_proc(&mut std_src, &sym),
            "adenc_rt_mapSize" => emit_map_size_proc(&mut std_src, &sym),
            "adenc_rt_mapClear" => emit_map_clear_proc(&mut std_src, &sym),
            "adenc_rt_mapContainsKey" => emit_map_contains_proc(&mut std_src, &sym),
            "adenc_rt_mapPut" => emit_map_put_proc(&mut std_src, &sym, false),
            "adenc_rt_mapPutInt" => emit_map_put_proc(&mut std_src, &sym, true),
            "adenc_rt_mapGet" => emit_map_get_proc(&mut std_src, &sym, false),
            "adenc_rt_mapGetInt" => emit_map_get_proc(&mut std_src, &sym, true),
            "adenc_rt_hostExists" => emit_host_exists_proc(&mut std_src, &sym),
            "adenc_rt_hostIsFile" => emit_host_is_file_proc(&mut std_src, &sym),
            "adenc_rt_hostIsDirectory" => emit_host_is_directory_proc(&mut std_src, &sym),
            "adenc_rt_hostReadAllText" => emit_host_read_all_text_proc(&mut std_src, &sym),
            "adenc_rt_hostListChildren" => emit_host_list_children_proc(&mut std_src, &sym),
            "adenc_rt_hostCreateDirectory" => emit_host_create_directory_proc(&mut std_src, &sym),
            "adenc_rt_hostWriteAllText" => emit_host_write_all_text_proc(&mut std_src, &sym),
            "adenc_rt_hostCopyFile" => emit_host_copy_file_proc(&mut std_src, &sym),
            "adenc_rt_hostAllocBytes" => emit_host_alloc_bytes_proc(&mut std_src, &sym),
            "adenc_rt_hostFreeBytes" => emit_host_free_bytes_proc(&mut std_src, &sym),
            "adenc_rt_hostReadByte" => emit_host_read_byte_proc(&mut std_src, &sym),
            "adenc_rt_hostWriteByte" => emit_host_write_byte_proc(&mut std_src, &sym),
            "adenc_rt_hostCopyBytes" => emit_host_copy_bytes_proc(&mut std_src, &sym),
            "adenc_rt_hostReadLong" => emit_host_read_long_proc(&mut std_src, &sym),
            "adenc_rt_hostWriteLong" => emit_host_write_long_proc(&mut std_src, &sym),
            "adenc_rt_hostStringUtf8Length" => emit_host_string_utf8_length_proc(&mut std_src, &sym),
            "adenc_rt_hostAllocUtf8Z" => emit_host_alloc_utf8z_proc(&mut std_src, &sym),
            "adenc_rt_hostStringFromUtf8" => emit_host_string_from_utf8_proc(&mut std_src, &sym),
            "adenc_rt_hostStringFromUtf8Z" => emit_host_string_from_utf8z_proc(&mut std_src, &sym),
            ...
            "adenc_rt_hostCallNative0" => emit_host_call_native0_proc(&mut std_src, &sym),
            "adenc_rt_hostCallNative1" => emit_host_call_native1_proc(&mut std_src, &sym),
            "adenc_rt_hostCallNative2" => emit_host_call_native2_proc(&mut std_src, &sym),
            "adenc_rt_hostCallNative3" => emit_host_call_native3_proc(&mut std_src, &sym),
            "adenc_rt_hostCallNative4" => emit_host_call_native4_proc(&mut std_src, &sym),
            ...
            "adenc_rt_hostRunShellProcess" => emit_host_run_shell_process_proc(&mut std_src, &sym),
            "adenc_rt_arcRetain" => emit_arc_retain_proc(&mut std_src, &sym),
            "adenc_rt_arcRelease" => emit_arc_release_proc(&mut std_src, &sym),
            "adenc_rt_arcCycleYoungPass" => emit_arc_cycle_young_pass_proc(&mut std_src, &sym),
            "adenc_rt_arcCycleFullPass" => {
                emit_arc_cycle_full_pass_proc(&mut std_src, &sym, &object_arc_fields)
            }
            "adenc_rt_arcCycleTick" => emit_arc_cycle_tick_proc(&mut std_src, &sym),
            "adenc_rt_weakNew" => emit_weak_new_proc(&mut std_src, &sym),
            "adenc_rt_weakGet" => emit_weak_get_proc(&mut std_src, &sym),
            "adenc_rt_weakClear" => emit_weak_clear_proc(&mut std_src, &sym),
            "adenc_std_com_aden_math_Math_abs" => emit_math_abs_proc(&mut std_src, &sym),
            "adenc_std_com_aden_math_Math_max" => emit_math_max_proc(&mut std_src, &sym),
    ...
    startup.push_str("extrn adenc_rt_init:proc\n");
    startup.push_str("extrn adenc_rt_shutdown:proc\n");
...
pub(crate) fn emit_masm_full_program_object(
    ...
    emit_runtime_init_proc(&mut source, "adenc_rt_init", WRITE_RAW_SYMBOL);
    source.push('\n');
    emit_runtime_shutdown_proc(&mut source, "adenc_rt_shutdown");
    source.push('\n');
    ...
    source.push('\n');
    emit_string_from_bytes_proc(&mut source, "adenc_rt_stringFromBytes");
    source.push('\n');
    emit_string_equals_proc(&mut source, "adenc_rt_stringEquals");
    source.push('\n');
    emit_write_raw_proc(&mut source, WRITE_RAW_SYMBOL);
    source.push('\n');
    emit_host_path_alloc_proc(&mut source, "adenc_rt_hostPathAlloc");
    source.push('\n');
    emit_fp_to_int_proc(&mut source, "adenc_rt_fpToInt");
    source.push('\n');
    emit_fp_to_long_proc(&mut source, "adenc_rt_fpToLong");
    source.push('\n');
    ...
    source.push('\n');
    emit_ensure_slot_capacity_proc(&mut source, "adenc_rt_ensureSlotCapacity");
    source.push('\n');
    emit_arc_teardown_proc(&mut source, "adenc_rt_arcTeardown", &object_arc_fields);
    source.push('\n');
    emit_startup_entry_proc(
        &mut source,
        "mainCRTStartup",
    ...
    for sym in std_symbols {
        match sym.as_str() {
            ...
            "adenc_rt_consoleReadLine" => emit_console_read_line_proc(&mut source, &sym),
            "adenc_rt_consoleWrite" => {
            ...
            "adenc_rt_panic" => emit_panic_proc(&mut source, &sym),
            DISPATCH_NULL_PANIC_SYMBOL => {
            ...
            "adenc_rt_stringConcat" => emit_string_concat_proc(&mut source, &sym),
            "adenc_rt_stringLength" => emit_string_length_proc(&mut source, &sym),
            "adenc_rt_parseInt" => emit_parse_int_proc(&mut source, &sym),
            "adenc_rt_parseBoolean" => emit_parse_boolean_proc(&mut source, &sym),
            OBJECT_CLASS_NAME_SYMBOL => {}
            OBJECT_HASH_CODE_SYMBOL => {}
            ...
            "adenc_rt_arrayNew" => emit_array_new_proc(&mut source, &sym),
            "adenc_rt_arrayNewMulti" => emit_array_new_multi_proc(&mut source, &sym),
            "adenc_rt_arrayLength" => emit_array_length_proc(&mut source, &sym),
            "adenc_rt_arrayGetInt" => emit_array_get_proc(&mut source, &sym, "rt_arr_i_ptr"),
            "adenc_rt_arraySetInt" => emit_array_set_proc(&mut source, &sym, "rt_arr_i_ptr"),
            "adenc_rt_arrayGetLong" => {
            ...
            "adenc_rt_arrayGetString" => emit_array_get_proc(&mut source, &sym, "rt_arr_s_ptr"),
            "adenc_rt_arraySetString" => emit_array_set_proc(&mut source, &sym, "rt_arr_s_ptr"),
            "adenc_rt_listNew" => emit_list_new_proc(&mut source, &sym),
            "adenc_rt_listSize" => emit_list_size_proc(&mut source, &sym),
            "adenc_rt_listKind" => emit_list_kind_proc(&mut source, &sym),
            "adenc_rt_listClear" => emit_list_clear_proc(&mut source, &sym),
            "adenc_rt_listAddInt" => emit_list_add_proc(&mut source, &sym, "rt_list_i_ptr"),
            "adenc_rt_listAddString" => emit_list_add_proc(&mut source, &sym, "rt_list_s_ptr"),
            "adenc_rt_listGetInt" => emit_list_get_proc(&mut source, &sym, "rt_list_i_ptr"),
            "adenc_rt_listGetString" => emit_list_get_proc(&mut source, &sym, "rt_list_s_ptr"),
            "adenc_rt_mapNew" => emit_map_new_proc(&mut source, &sym),
            "adenc_rt_mapSize" => emit_map_size_proc(&mut source, &sym),
            "adenc_rt_mapClear" => emit_map_clear_proc(&mut source, &sym),
            "adenc_rt_mapContainsKey" => emit_map_contains_proc(&mut source, &sym),
            "adenc_rt_mapPut" => emit_map_put_proc(&mut source, &sym, false),
            "adenc_rt_mapPutInt" => emit_map_put_proc(&mut source, &sym, true),
            "adenc_rt_mapGet" => emit_map_get_proc(&mut source, &sym, false),
            "adenc_rt_mapGetInt" => emit_map_get_proc(&mut source, &sym, true),
            "adenc_rt_hostExists" => emit_host_exists_proc(&mut source, &sym),
            "adenc_rt_hostIsFile" => emit_host_is_file_proc(&mut source, &sym),
            "adenc_rt_hostIsDirectory" => emit_host_is_directory_proc(&mut source, &sym),
            "adenc_rt_hostReadAllText" => emit_host_read_all_text_proc(&mut source, &sym),
            "adenc_rt_hostListChildren" => emit_host_list_children_proc(&mut source, &sym),
            "adenc_rt_hostCreateDirectory" => emit_host_create_directory_proc(&mut source, &sym),
            "adenc_rt_hostWriteAllText" => emit_host_write_all_text_proc(&mut source, &sym),
            "adenc_rt_hostCopyFile" => emit_host_copy_file_proc(&mut source, &sym),
            "adenc_rt_hostAllocBytes" => emit_host_alloc_bytes_proc(&mut source, &sym),
            "adenc_rt_hostFreeBytes" => emit_host_free_bytes_proc(&mut source, &sym),
            "adenc_rt_hostReadByte" => emit_host_read_byte_proc(&mut source, &sym),
            "adenc_rt_hostWriteByte" => emit_host_write_byte_proc(&mut source, &sym),
            "adenc_rt_hostCopyBytes" => emit_host_copy_bytes_proc(&mut source, &sym),
            "adenc_rt_hostReadLong" => emit_host_read_long_proc(&mut source, &sym),
            "adenc_rt_hostWriteLong" => emit_host_write_long_proc(&mut source, &sym),
            "adenc_rt_hostStringUtf8Length" => emit_host_string_utf8_length_proc(&mut source, &sym),
            "adenc_rt_hostAllocUtf8Z" => emit_host_alloc_utf8z_proc(&mut source, &sym),
            "adenc_rt_hostStringFromUtf8" => emit_host_string_from_utf8_proc(&mut source, &sym),
            "adenc_rt_hostStringFromUtf8Z" => emit_host_string_from_utf8z_proc(&mut source, &sym),
            "adenc_rt_hostLoadDynamicLibrary" => {
            ...
            "adenc_rt_hostCallNative0" => emit_host_call_native0_proc(&mut source, &sym),
            "adenc_rt_hostCallNative1" => emit_host_call_native1_proc(&mut source, &sym),
            "adenc_rt_hostCallNative2" => emit_host_call_native2_proc(&mut source, &sym),
            "adenc_rt_hostCallNative3" => emit_host_call_native3_proc(&mut source, &sym),
            "adenc_rt_hostCallNative4" => emit_host_call_native4_proc(&mut source, &sym),
            "adenc_rt_hostRegisterNativeCallback0" => {
            ...
            "adenc_rt_hostRunShellProcess" => emit_host_run_shell_process_proc(&mut source, &sym),
            "adenc_rt_arcRetain" => emit_arc_retain_proc(&mut source, &sym),
            "adenc_rt_arcRelease" => emit_arc_release_proc(&mut source, &sym),
            "adenc_rt_arcCycleYoungPass" => emit_arc_cycle_young_pass_proc(&mut source, &sym),
            "adenc_rt_arcCycleFullPass" => {
                emit_arc_cycle_full_pass_proc(&mut source, &sym, &object_arc_fields)
            }
            "adenc_rt_arcCycleTick" => emit_arc_cycle_tick_proc(&mut source, &sym),
            "adenc_rt_weakNew" => emit_weak_new_proc(&mut source, &sym),
            "adenc_rt_weakGet" => emit_weak_get_proc(&mut source, &sym),
            "adenc_rt_weakClear" => emit_weak_clear_proc(&mut source, &sym),
            "adenc_std_com_aden_math_Math_abs" => emit_math_abs_proc(&mut source, &sym),
            "adenc_std_com_aden_math_Math_max" => emit_math_max_proc(&mut source, &sym),
...
pub(crate) fn shared_runtime_export_symbols_for_ir(ir: &IrProgram) -> Vec<String> {
    ...
    for class in &ir.classes {
        if !class.package_name.starts_with("aden.") {
            continue;
        }
        for method in &class.methods {
            let sig = method
