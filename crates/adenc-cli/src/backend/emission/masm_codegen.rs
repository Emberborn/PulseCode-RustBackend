use super::*;
use adenc_core::intermediate::IrSourceLoc;
...
fn call_result_requires_arc_retain_on_store(
    ...
    if owner == "aden.memory.Memory" && member == "weakGet" {
        return false;
    }

    let owner_is_collection_like = owner.starts_with("aden.collections.")
        || owner == "Array"
        || owner == "ArrayList"
        || owner == "LinkedList"
        || owner == "HashMap"
...
fn raw_runtime_handle_field(owner: &str, member: &str) -> bool {
    matches!(
        ...
        ("Files", "textStore")
            ...
            | ("Files", "modifiedStore")
            | ("Array", "handle")
            | ("aden.io.Files", "textStore")
            | ("aden.io.Files", "fileIndex")
            | ("aden.io.Files", "directoryIndex")
            | ("aden.io.Files", "modifiedStore")
            | ("aden.collections.Array", "handle")
    )
}
...
fn emit_runtime_numeric_coercion(
    ...
    if is_float_scalar_type_name(&source_ty) {
        match target_ty.as_str() {
            ...
            "long" | "ulong" => {
                out.push_str("    mov rcx, rax\n");
                out.push_str("    call adenc_rt_fpToLong\n");
            }
            "byte" | "ubyte" | "short" | "ushort" | "int" | "uint" | "char" => {
                out.push_str("    mov rcx, rax\n");
                out.push_str("    call adenc_rt_fpToInt\n");
...
fn emit_array_new_initialized_value(
    out: &mut String,
    method: &IrMethod,
    value: &adenc_core::intermediate::IrValue,
    element_ty: &str,
    dimensions: usize,
    values: &[IrValueId],
    current_method_is_static: bool,
    ...
    match &value.kind {
        ...
        IrValueKind::ArrayNew { lengths, .. } => {
            if lengths.len() == 1 {
                ...
                out.push_str("    call adenc_rt_arrayNewMulti\n");
            }
            Ok(())
        }
        ...
        IrValueKind::ArrayGet {
            ...
            if native_array_uses_qword_scalar_lane(element_ty) {
                out.push_str("    call adenc_rt_arrayGetLong\n");
            ...
            {
                out.push_str("    call adenc_rt_arrayGetString\n");
            } else {
                out.push_str("    call adenc_rt_arrayGetInt\n");
            }
            Ok(())
        }
        IrValueKind::ArraySet {
            ...
            if native_array_uses_qword_scalar_lane(element_ty) {
                out.push_str("    call adenc_rt_arraySetLong\n");
            } else if is_string_type_name(element_ty)
                || element_ty.ends_with("[]")
                || (!matches!(
                    element_ty.as_str(),
                ...
                ))
            {
                ...
                out.push_str("    call adenc_rt_arraySetInt\n");
            }
            out.push_str("    xor eax, eax\n");
            Ok(())
        }
