...
fn load_stdlib_class_info() -> Result<Vec<(String, ClassInfo)>, SemanticError> {
    ...
    collect_pulse_files(stdlib_root.as_path(), &mut files)?;
    files.sort();
...
fn collect_pulse_files(dir: &Path, files: &mut Vec<PathBuf>) -> Result<(), SemanticError> {
    for entry in fs::read_dir(dir).map_err(|err| {
        ...
        let path = entry.path();
        if path.is_dir() {
            collect_pulse_files(path.as_path(), files)?;
