...
pub(super) fn validate_foreach_stmt(
    ...
    let array_baseline = types_compatible("aden.collections.Array", &iterable_ty.ty, class_index);
    let iterable_baseline = types_compatible("aden.lang.Iterable", &iterable_ty.ty, class_index);

    if !native_array_baseline && !array_baseline && !iterable_baseline {
        return Err(semantic_error(format!(
            "Foreach source in '{}.{}' must be a native array, aden.lang.Iterable, or aden.collections.Array, got '{}'",
    ...
    let inferred_ty = if ty == "var" {
        if native_array_baseline {
            ...
            "aden.lang.Object".to_string()
        } else {
            return Err(semantic_error(format!(
                "Foreach local variable inference in '{}.{}' is currently supported for native arrays and aden.lang.Iterable, got '{}'",
                class.name, method.name, iterable_ty.ty
            )));
        }
    } else {
    ...
    {
        return Err(semantic_error(format!(
            "Foreach over aden.collections.Array currently supports loop variables of type 'int' or 'String', got '{}'",
            inferred_ty
        )));
    }
    ...
    if iterable_baseline
        && !array_baseline
        && !types_compatible(&inferred_ty, "aden.lang.Object", class_index)
    {
        return Err(semantic_error(format!(
            "Foreach over aden.lang.Iterable currently requires loop variable type 'Object' (or an assignable supertype), got '{}'",
            inferred_ty
        )));
    }
