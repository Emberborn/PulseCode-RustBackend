...
impl IrBuilder {
    ...
    pub(super) fn lower_foreach(
        ...
        if ty == "int" || ty == "String" {
            ...
            let length_local = format!("__foreach{}_length", stmt_index);
            self.local_types
                .insert(array_local.clone(), "aden.collections.Array".to_string());

            self.emit(
                current,
                IrInstr::DeclareLocal {
                    name: array_local.clone(),
                    ty: "aden.collections.Array".to_string(),
                    source: source.clone(),
                },
            );
            self.emit(
        ...
        self.emit(
            ...
            IrInstr::DeclareLocal {
                ...
                ty: "aden.lang.Iterator".to_string(),
                source: source.clone(),
            },
        );
        self.local_types
            .insert(iter_local.clone(), "aden.lang.Iterator".to_string());
